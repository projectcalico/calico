...
// SSA regression tests covering every projectcalico.org/v3 CRD kind. The
// BGPConfiguration regression test in serverside_apply_test.go locks in the
// fix for https://github.com/projectcalico/calico/issues/12700 (listType=set
// on object lists). The tests in this file extend that coverage to all
// other v3 kinds, populating each spec list field with at least one element
// so smd actually walks into each list type.
