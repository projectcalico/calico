...
if ! command -v bz >/dev/null 2>&1; then
  ...
  for attempt in $(seq 1 8); do
    ...
    # curl, not wget: the assets endpoint 302-redirects to a signed host. curl drops
    # the Authorization header across hosts; wget forwards the in-URL credentials,
    # leaking the token onward and tripping S3's "only one auth mechanism" 400. The
    # token also stays out of the URL, so it can't surface via -v, a future set -x,
    # or the process arg list — step logs are archived to GCS and served by the viewer.
    if [[ "${BZ_ASSET_ID}" =~ ^[0-9]+$ ]] \
