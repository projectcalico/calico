...
func TestBackendFilterHints_RejectsFlowFilter(t *testing.T) {
	...
	// Goldmane's FilterHints API returns hint values, not source flows, so a
	// flow-level RBAC predicate cannot be applied; supplying one must fail
	// loudly rather than silently returning unfiltered hints.
	b := NewBackend(climocks.NewFlowsClient(t))

	ft := whiskerv1.FilterType(proto.FilterType_FilterTypeSourceNamespace)
	_, _, err := b.FilterHints(context.Background(), whiskerv1.FlowFilterHintsRequest{Type: &ft},
		func(*whiskerv1.FlowResponse) (bool, error) { return true, nil })
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(ContainSubstring("does not support flow filtering"))
}

// TestBackendListMetadataConversion verifies that List and FilterHints carry
// Goldmane's page and result totals through to the apiutil.ListMeta the handler
// serves, and that a response carrying no metadata reads as zero totals.
func TestBackendListMetadataConversion(t *testing.T) {
	RegisterTestingT(t)

	cli := new(climocks.FlowsClient)
	cli.On("List", mock.Anything, mock.Anything).Return(
		&proto.ListMetadata{TotalPages: 5, TotalResults: 212}, []*proto.FlowResult{}, nil).Once()
	cli.On("FilterHints", mock.Anything, mock.Anything).Return(
		&proto.ListMetadata{TotalPages: 3, TotalResults: 17}, []*proto.FilterHint{}, nil).Once()

	b := NewBackend(cli)

	meta, _, err := b.List(context.Background(), whiskerv1.ListFlowsParams{})
	Expect(err).NotTo(HaveOccurred())
	Expect(meta).To(Equal(apiutil.ListMeta{TotalPages: 5, TotalResults: 212}))

	ft := whiskerv1.FilterType(proto.FilterType_FilterTypeSourceNamespace)
	meta, _, err = b.FilterHints(context.Background(), whiskerv1.FlowFilterHintsRequest{Type: &ft}, nil)
	Expect(err).NotTo(HaveOccurred())
	Expect(meta).To(Equal(apiutil.ListMeta{TotalPages: 3, TotalResults: 17}))

	cli.On("List", mock.Anything, mock.Anything).Return(nil, []*proto.FlowResult{}, nil).Once()

	meta, _, err = b.List(context.Background(), whiskerv1.ListFlowsParams{})
	Expect(err).NotTo(HaveOccurred())
	Expect(meta).To(Equal(apiutil.ListMeta{}))
}
