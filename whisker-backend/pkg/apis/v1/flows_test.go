...
func TestListFlowsFilters(t *testing.T) {
	...
	tt := []struct {
		...
		{
			description: "Decoder parses sortBy query param with allowed value",
			request:     mustCreateGetRequest("GET", "/api/v1/flows", map[string][]string{"type": {"FilterTypeDestName"}}),
			expected:    &v1.FlowFilterHintsRequest{Type: v1.FilterType(proto.FilterType_FilterTypeDestName)},
		},
		{
			description: "Decoder parses sortBy query param with allowed value",
			request: mustCreateGetRequest("GET", "/api/v1/flows", map[string][]string{
