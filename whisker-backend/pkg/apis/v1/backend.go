...
type FlowsBackend interface {
	...
	// totals across all pages. includeFlow, when non-nil, gates the source flows
	// the hints are derived from so a user only sees values from flows they are
	// permitted to view (e.g. RBAC); without it an unscoped query could leak
	// names/namespaces/policies from namespaces the user cannot list.
