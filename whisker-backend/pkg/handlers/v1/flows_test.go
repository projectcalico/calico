...
func TestListFlows(t *testing.T) {
	...
	backend.On("List", mock.Anything, mock.Anything).Return(
		apiutil.ListMeta{TotalPages: 5, TotalResults: 212},
...
func TestListFlows_Error(t *testing.T) {
	...
	backend := new(v1mocks.FlowsBackend)
	backend.On("List", mock.Anything, mock.Anything).Return(apiutil.ListMeta{}, nil, context.Canceled)
...
func TestListFilterHints_Error(t *testing.T) {
	...
	backend := new(v1mocks.FlowsBackend)
	backend.On("FilterHints", mock.Anything, mock.Anything, mock.Anything).Return(apiutil.ListMeta{}, nil, context.Canceled)
...
func TestListFlows_AppliesRBACFilter(t *testing.T) {
	...
	backend := new(v1mocks.FlowsBackend)
	backend.On("List", mock.Anything, mock.Anything).Return(
		apiutil.ListMeta{TotalPages: 1, TotalResults: 2}, []whiskerv1.FlowResponse{flowWithPolicy("allowed"), flowWithPolicy("denied")}, nil)
...
func TestListFlows_RBACEmptiedPageReportsZeroPages(t *testing.T) {
	...
	backend := new(v1mocks.FlowsBackend)
	backend.On("List", mock.Anything, mock.Anything).Return(
		apiutil.ListMeta{TotalPages: 1, TotalResults: 1}, []whiskerv1.FlowResponse{flowWithPolicy("denied")}, nil)
	...
	Expect(flows.Meta).Should(Equal(apiutil.ListMeta{}))
}
...
func TestListFilterHints_PassesRBACPredicate(t *testing.T) {
	...
	backend.On("FilterHints", mock.Anything, mock.Anything, mock.MatchedBy(func(f whiskerv1.FlowFilterFunc) bool {
		includeFlow = f
		return true
	})).Return(apiutil.ListMeta{TotalPages: 1, TotalResults: 1}, []whiskerv1.FlowFilterHintResponse{{Value: "foo"}}, nil)
...
func TestFlows_RBACFilterFactoryError(t *testing.T) {
	...
	t.Run("list", func(t *testing.T) {
		sc := setupTest(t)
		backend := new(v1mocks.FlowsBackend)
		backend.On("List", mock.Anything, mock.Anything).Return(apiutil.ListMeta{TotalPages: 1, TotalResults: 1}, []whiskerv1.FlowResponse{flowWithPolicy("ns")}, nil)
