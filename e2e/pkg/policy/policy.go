...
import (
	...
	"github.com/tigera/k8s-e2e/src/utils/calico"
...
var _ = describe.CalicoDescribe(
	...
	func() {
		...
		var checker utils.ConnectionTester
		...
		BeforeEach(func() {
			...
			checker = utils.NewConnectionTester(f)

			cli, err = client.New(f.ClientConfig())
			Expect(err).NotTo(HaveOccurred())

			// Ensure a clean starting environment before each test.
			Expect(calico.CleanDatastore(cli)).ShouldNot(HaveOccurred())
		})

		Context("with a server and client", func() {
			var server1 *utils.Server
			var client1 *utils.Client
			...
			BeforeEach(func() {
				...
				server1 = utils.NewServer("server", f.Namespace, utils.WithServerLabels(map[string]string{"role": "server"}))
				client1 = utils.NewClient("client", f.Namespace)
			...
			framework.ConformanceIt("should be able to connect to the server", func() {
			...
			framework.ConformanceIt("should correctly isolate namespaces using ingress and egress policies [RunsOnWindows]", func() {
				nsA := f.Namespace
				...
				server2 := utils.NewServer("server-b", nsB)
				client2 := utils.NewClient("client-b", nsB)
				...
				allowEgressToDNS.Spec.Egress = []v3.Rule{
					{
						...
						Destination: v3.EntityRule{
							Selector: calico.GetDnsSelector(f),
						},
					},
				}
			...
			framework.ConformanceIt("should enforce access restrictions through a service account selector", func() {
				ns := f.Namespace
				ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
				defer cancel()
			...
			framework.ConformanceIt("should enforce access restrictions through a namespace selector", func() {
				ns := f.Namespace
			...
			framework.ConformanceIt("should allow negative selectors and support for filtering ICMP [RunsOnWindows]", func() {
				ns := f.Namespace

				// Create a new server which will be used for ICMP tests.
				By("Creating a server pod with the label 'icmp:yes'")
				labels := map[string]string{
					"pod-name": "server-icmp",
					"icmp":     "yes",
					"role":     "server",
				}
				icmpAllowedServer := utils.NewServer("server-icmp", ns, utils.WithServerLabels(labels))
				checker.AddServer(icmpAllowedServer)
				checker.Deploy()

				By("Applying policies that a) allow all traffic into and out-of the client, and b) deny ICMP ingress into the server with the 'icmp' label")
				err := cli.Apply(calico.NPFromYAML(
					fmt.Sprintf(`
...
`, ns.Name)))
				...
				// Expect that the client can connect to the server using its clusterIP, but not via ICMP.
				checker.ExpectSuccess(client1, server1.ClusterIPs()...)
				checker.ExpectFailure(client1, server1.ICMP())
				checker.Execute()
...
`, ns.Name,
				...
				defer func() {
					_, err = cli.DeleteByName("networkpolicy", "allow-icmp-access", ns.Name)
					Expect(err).NotTo(HaveOccurred())
					_, err = cli.DeleteByName("networkpolicy", "deny-icmp", ns.Name)
					Expect(err).NotTo(HaveOccurred())
				}()

				By("Pinging the ICMP allowed pod")
				checker.ResetExpectations()
				checker.ExpectSuccess(client1, icmpAllowedServer.ClusterIPs()...)
				checker.ExpectSuccess(client1, icmpAllowedServer.ICMP())
				checker.ExpectSuccess(client1, server1.ClusterIPs()...) // We only blocked ICMP.
				checker.ExpectFailure(client1, server1.ICMP())
				checker.Execute()
			})
		})
