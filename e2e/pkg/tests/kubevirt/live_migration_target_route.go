...
// (~9s). While the stale route is best, the target cannot resolve recursive
// next-hops through the workload and does not advertise its own /32, so
// remote clients keep following the stale route into the source's dead veth.
// The fix raises the local route to preference 150 while Felix's route
// elevation (metric 512 < NormalRoutePriority) is active.
...
func runTargetRouteMigrationTest(ctx context.Context, f *framework.Framework, cli ctrlclient.Client, p targetRouteTestParams) {
	...
	// Remote client: its path to the VM crosses the mesh, so it is the probe
	// that a stale-best route on the target actually disrupts (the target
	// stops advertising its /32 while a BGP route is best). Target client:
	// pinned to migration 2's destination, validating that a client colocated
	// with the arriving VM keeps working via the local veth.
	...
	if p.preflight != nil {
		By("Waiting for the VM to be reachable from both client pods")
