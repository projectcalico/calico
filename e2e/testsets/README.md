...
- a lane still selecting by regex gets a `legacy/` name, which disappears when the lane converts to a config
...
An empty set is a lane running no specs at all. The generator prints `EMPTY` for those rather than failing, since several are lanes focused on specs that only exist in the Enterprise suite.
