...
security profiles
  A security profile is an object that encapsulates a specific set of security
  rules to apply to an endpoint.  Endpoints reference one or more security
  profiles explicitly.  See :ref:`security-profile-data` for more.

security policies
  Similarly, security policies contain a set of security rules to apply.
  However, security policies provide a richer, tiered security model, which
  can override the security profiles directly referenced by an endpoint.

  Security policies are divided into tiers, which are applied in order by
  Calico and each policy within a tier can have a rich Boolean selector
  attached that picks out the endpoints it should apply to.  See
  :ref:`security-policy-data` for more.
...
The 'tags' are a list of classifiers that apply to each endpoint in the
profile. The purpose of the tags is to allow for rules in other policies to
refer to profiles by name, rather than by membership.
...
richer security model that we call "tiered policy". Naturally enough, tiered
policy consists of a series of explicitly ordered "tiers".  Tiers contain
(explicitly ordered) "policies".  Each policy has a Boolean selector expression
attached that decides whether it applies to a given endpoint. Selector
expressions match against an endpoint's labels.

Each tier might have a different owner; for (a hypothetical) example, an
enterprise's NetSec team could install a global black/white list that comes
before rules generated by a Calico plugin::
...
Each tier of policy must approve each packet by using the "next-tier" action
in a rule before is passes to the next.

Calico renders the security policy for each endpoint individually and only the
policies that have matching selectors are rendered.  This ensures that the
number of rules that actually need to be inserted into the kernel is
...
``action``
  ...
  jumps to the next tier and continues processing.
