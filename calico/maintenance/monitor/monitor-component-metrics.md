...
<label:Operator,active:true>
<%
An Operator installation of {{site.prodname}} automatically deploys one or several Typha instances depending on the scale of your cluster. By default metrics for these instances are disabled.
...
```bash
kubectl delete pod -n calico-monitoring prometheus-pod
kubectl apply -f - <<EOF
apiVersion: v1
kind: Pod
metadata:
