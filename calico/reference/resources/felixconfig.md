...
| healthTimeoutOverrides             | Allows the internal watchdog timeouts of individual subcomponents to be overriden; example: "internal-dataplane-main-loop=30s,calculation-graph=2m".  This is useful for working around "false positive" liveness timeouts that can occur in particularly stressful workloads or if CPU is constrained.  For a list of active subcomponents, see Felix's logs. | `"internal-dataplane-main-loop=120s,calculation-graph=90s"` | Comma-delimited key/value pairs where the value is a duration: `90s`, `120s`, `10m` etc. | string | `""` |
