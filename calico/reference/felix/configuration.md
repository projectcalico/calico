...
| `IptablesBackend`                    | `FELIX_IPTABLESBACKEND`                    | This parameter controls which variant of iptables binary Felix uses. Felix uses `Legacy` by default. Set to `NFT` for hosts using a netfilter backend. [Default: `Legacy`] | `Legacy`, `NFT` |
