...
| BPFLogFilters                      / <br/> FELIX_BPFLOGFILTERS                        | BPFLogFilters is a comma-separated list of key=values where the value is a pcap filter expression and the key is an interface name with 'all' denoting all interfaces, 'weps' all workload endpoints and 'heps' all host endpoints. 'ctlb' can take a value of 'on' or 'off' to set whether connect time load balance debug output is enabled when BPFLogLevel is debug. It is off by default.| key=val,key=val,...  | unset - means all debug logs are emitted |
