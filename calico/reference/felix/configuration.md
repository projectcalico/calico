...
| `RPFilterSkipCIDRList`            | `FELIX_RPFILTERSKIPCIDRLIST`            | Comma-delimited list of IPv4 or CIDR that Felix will exclude from strict reverse-path filter check of packets from workload interfaces. This can be used in conjuction with `WorkloadSourceSpoofing` to allow a list of IPs that can be spoofed by workloads, by default. [Default: ""] | string |
