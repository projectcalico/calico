...
import calico.felix.finterface as finterface
...
class TestFinterface(unittest.TestCase):
    def setUp(self):
        pass
    ...
    def test_tap_exists(self):
        ...
        with mock.patch('os.path.exists', return_value=True):
            self.assertTrue(finterface.tap_exists(tap))
            os.path.exists.assert_called_with("/sys/class/net/" + tap)

        with mock.patch('os.path.exists', return_value=False):
            self.assertFalse(finterface.tap_exists(tap))
            os.path.exists.assert_called_with("/sys/class/net/" + tap)
    ...
    def test_add_route(self):
        ...
        with mock.patch('calico.felix.futils.check_call', return_value=retcode):
            finterface.add_route(type, ip, tap, mac)
    ...
    def test_del_route(self):
        ...
        ip = "1.2.3.4"
        with mock.patch('calico.felix.futils.check_call', return_value=retcode):
            finterface.del_route(type, ip, tap)
    ...
    def test_list_tap_ips(self):
        ...
        retcode = futils.CommandOutput("", "")
        with mock.patch('calico.felix.futils.check_call', return_value=retcode):
            ips = finterface.list_tap_ips(type, tap)
