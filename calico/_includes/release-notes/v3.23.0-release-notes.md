...
The Calico VPP dataplane integration is now in beta!

#### Calico networking support in AKS

You can now use the Calico CNI in your AKS clusters to take advantage of all of the Calico networking features.
...
We have added additional configuration options to allow for more control in specific scenarios, such as node-to-node mesh and separate systems running Calico using the same AS.
...
Following the Kubernetes move from FlexVolumes, Calico now has its own CSI driver. Features that used to depend on FlexVolumes have been migrated to instead use CSI.
...
You can now install Calico on Windows HostProcess containers. With HostProcess containers, you can manage your Calico installation using Kubernetes resources, such as DaemonSets and ConfigMaps.
...
##### General
 ...
 - Build components with go 1.17.8 to pick up CVE fixes. [calico #5817](https://github.com/projectcalico/calico/pull/5817) (@doublek)
 ...
 - Update flannel to version v0.16.3 [calico #5578](https://github.com/projectcalico/calico/pull/5578) (@cyclinder)
 ...
 - Rev up etcd to 3.5.1, any project imported calico should upgrade their etcd to over 3.5.
Rev up k8s to v1.23.1/v0.23.1, any project imported calico should upgrade their k8s to over v1.23.0/v0.23.0.
Felix/bpf/proxy should only use endpointSlice to manage endpoints.
Rev up github.com/containernetworking/plugins and github.com/containernetworking/cni to v1.0.1 including multiple interfaces and methods refactoring and renaming.
In github.com/containernetworking/cni pkg/types/current no longer exists. You need to explicitly select a version. ref: https://github.com/containernetworking/cni/blob/v1.0.1/Documentation/spec-upgrades.md [calico #5466](https://github.com/projectcalico/calico/pull/5466) (@Aceralon)
