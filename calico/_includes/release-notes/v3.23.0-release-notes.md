...
To try it out, follow the Calico on AKS [installation instructions]({{site.prodname}}/getting-started/kubernetes/managed-public-cloud/aks#install-aks-with-calico-networking).
...
To try it for yourself, follow the [Calico for Windows installation instructions]({{site.baseurl}}/getting-started/windows-calico/quickstart#install-calico-for-windows-using-hostprocess-containers).
