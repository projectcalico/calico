...
EOF
""")
        kubectl("patch --type=merge bgppeer.crd.projectcalico.org node-extra.peer --patch '{\"spec\": {\"filters\": [\"test-filter-export-1\"]}}'")
        self.add_cleanup(lambda: kubectl("patch --type=merge bgppeer.crd.projectcalico.org node-extra.peer --patch '{\"spec\": {\"filters\": []}}'"))
