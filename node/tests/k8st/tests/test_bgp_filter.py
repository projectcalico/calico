...
class TestBGPFilter(TestBase):
    ...
    def setUpClass(cls):
        ...
        cls.external_node_ip = start_external_node_with_bgp(
            "kube-node-extra",
            bird_peer_config=bird_conf % ("ipv4", cls.egress_node_ip),
        )
        cls.external_node_ip6 = start_external_node_with_bgp(
            "kube-node-extra-v6",
            bird6_peer_config=bird_conf % ("ipv6", cls.egress_node_ip6),
        )
...
""" % cls.external_node_ip6)
    ...
    def _check_route_in_cluster_bird(self, calicoPod, route, peerIP, ipv6=False, globalPeer=False, present=True):
        ...
        def fn():
            birdCmd = "birdcl"  # BIRD3: single unified daemon/CLI for both families
...
""" % (external_route_v4, self.external_node_ip))
            ...
            if ipv6:
                run("""cat <<EOF | docker exec -i kube-node-extra-v6 sh -c "cat > /etc/bird/static-route.conf"
protocol static static1 {
    ipv6;
    route %s via %s;
}
...
""" % (external_route_v6, self.external_node_ip6))
                run("docker exec kube-node-extra-v6 birdcl configure")
                self.add_cleanup(lambda: run("docker exec kube-node-extra-v6 sh -c 'rm /etc/bird/static-route.conf; birdcl configure'"))
