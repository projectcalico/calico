...
""" % cls.external_node_ip6)
    ...
    def tearDownClass(cls):
        ...
        kubectl("delete bgppeer.crd.projectcalico.org node-extra.peer", allow_fail=True)
        kubectl("delete bgppeer.crd.projectcalico.org node-extra-v6.peer", allow_fail=True)
    ...
    def _patch_peer_filters(self, peer, filters):
        ...
        kubectl("patch --type merge bgppeer.crd.projectcalico.org %s --patch '%s'" % (peer, patchStr))
...
""" % (external_route_v6, self.external_node_ip6))
            ...
            if ipv4:
                kubectl("patch --type=merge bgppeer.crd.projectcalico.org node-extra.peer --type json --patch '[{\"op\": \"remove\", \"path\": \"/spec/nodeSelector\"}]'")
                self.add_cleanup(lambda: kubectl("patch --type=merge bgppeer.crd.projectcalico.org node-extra.peer --patch '{\"spec\":{\"nodeSelector\":\"egress == \\\"true\\\"\"}}'"))
            if ipv6:
                kubectl("patch --type=merge bgppeer.crd.projectcalico.org node-extra-v6.peer --type json --patch '[{\"op\": \"remove\", \"path\": \"/spec/nodeSelector\"}]'")
                self.add_cleanup(lambda: kubectl("patch --type=merge bgppeer.crd.projectcalico.org node-extra-v6.peer --patch '{\"spec\":{\"nodeSelector\":\"egress == \\\"true\\\"\"}}'"))
