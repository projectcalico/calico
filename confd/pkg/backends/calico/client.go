...
func (c *client) onUpdates(updates []api.Update, needUpdatePeersV1 bool) {
	...
	// Update the default BGP Config for the password since the secret watcher will pass in nil updates with a flag to trigger updating peers
	// for password changes.
	if len(updates) == 0 && needUpdatePeersV1 {
...
func (c *client) updateNodeMeshPassword() {
	...
	key := model.GlobalBGPConfigKey{Name: globalConfigName}
