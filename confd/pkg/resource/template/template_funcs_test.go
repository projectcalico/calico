...
func Test_BGPFilterBIRDFuncs(t *testing.T) {
	...
	testFilter.Spec = v3.BGPFilterSpec{
		ImportV4: []v3.BGPFilterRuleV4{
			...
			{Action: "Reject", Source: "RemotePeers", MatchOperator: "NotIn", CIDR: "55.4.0.0/16", PrefixLength: v3.BGPFilterPrefixLength{Min: 16, Max: 24}},
		...
		ExportV4: []v3.BGPFilterRuleV4{
			...
			{Action: "Reject", Source: "RemotePeers", MatchOperator: "NotIn", CIDR: "88.7.0.0/16", PrefixLength: v3.BGPFilterPrefixLength{Max: 24}},
		...
		ImportV6: []v3.BGPFilterRuleV6{
			...
			{Action: "Accept", Interface: "eth0", MatchOperator: "NotIn", CIDR: "55.4.0.0/16", PrefixLength: v3.BGPFilterPrefixLength{Min: 20}},
			{Action: "Reject", Interface: "eth0", Source: "RemotePeers"},
			{Action: "Accept", MatchOperator: "NotEqual", CIDR: "7000:1::0/64", PrefixLength: v3.BGPFilterPrefixLength{Max: 96}},
			{Action: "Accept", Source: "RemotePeers"},
			{Action: "Accept", Interface: "extraiface"},
			{Action: "Reject"},
		},
		ExportV6: []v3.BGPFilterRuleV6{
			...
			{Action: "Reject", Interface: "eth0", MatchOperator: "NotIn", CIDR: "55.4.0.0/16", PrefixLength: v3.BGPFilterPrefixLength{Min: 24, Max: 28}},
			{Action: "Accept", Interface: "eth0", Source: "RemotePeers"},
			{Action: "Accept", MatchOperator: "NotIn", CIDR: "9000:1::0/64", PrefixLength: v3.BGPFilterPrefixLength{Min: 96, Max: 120}},
			{Action: "Accept", Source: "RemotePeers"},
			{Action: "Reject", Interface: "extraiface"},
			{Action: "Reject"},
		},
	...
	expectedBIRDCfgStrV6 := []string{
		...
		"  if ((net !~ [ 55.4.0.0/16{20,32} ])&&((defined(ifname))&&(ifname ~ \"eth0\"))) then { accept; }",
		"  if (((defined(source))&&(source ~ [ RTS_BGP ]))&&((defined(ifname))&&(ifname ~ \"eth0\"))) then { reject; }",
		...
		"  if ((net !~ [ 55.4.0.0/16{24,28} ])&&((defined(ifname))&&(ifname ~ \"eth0\"))) then { reject; }",
		"  if (((defined(source))&&(source ~ [ RTS_BGP ]))&&((defined(ifname))&&(ifname ~ \"eth0\"))) then { accept; }",
