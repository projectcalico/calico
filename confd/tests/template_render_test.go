...
func TestMain(m *testing.M) {
	...
	testEnv.ControlPlane.GetAPIServer().Configure().Append("service-cluster-ip-range", "10.101.0.0/16,fd00:96::/112")
...
// deleteK8sServices removes all Services and EndpointSlices in the default namespace.
func deleteK8sServices(t *testing.T, k8s kubernetes.Interface) {
	t.Helper()
	ctx := context.Background()
	if svcs, err := k8s.CoreV1().Services("default").List(ctx, metav1.ListOptions{}); err == nil {
		for _, s := range svcs.Items {
			if s.Name == "kubernetes" {
				continue
			}
			if err := k8s.CoreV1().Services("default").Delete(ctx, s.Name, metav1.DeleteOptions{}); err != nil {
				t.Logf("cleanup: failed to delete Service %s: %v", s.Name, err)
			}
		}
	}
	if eps, err := k8s.DiscoveryV1().EndpointSlices("default").List(ctx, metav1.ListOptions{}); err == nil {
		for _, e := range eps.Items {
			if err := k8s.DiscoveryV1().EndpointSlices("default").Delete(ctx, e.Name, metav1.DeleteOptions{}); err != nil {
				t.Logf("cleanup: failed to delete EndpointSlice %s: %v", e.Name, err)
			}
		}
	}
}
...
// deleteTestResources removes per-test Calico and K8s resources. Uses t.Logf for errors
...
func TestTemplateRendering(t *testing.T) {
	tests := []struct {
		...
		{"mesh/bgp-export", "mesh/bgp-export/input.yaml", "mesh/bgp-export", nil},
		{"mesh/ipip-always", "mesh/ipip-always/input.yaml", "mesh/ipip-always", nil},
		{"mesh/ipip-cross-subnet", "mesh/ipip-cross-subnet/input.yaml", "mesh/ipip-cross-subnet", nil},
		{"mesh/ipip-off", "mesh/ipip-off/input.yaml", "mesh/ipip-off", nil},
		{"mesh/vxlan-always", "mesh/vxlan-always/input.yaml", "mesh/vxlan-always", nil},
		{"mesh/hash", "mesh/hash/input.yaml", "mesh/hash", map[string]string{"CALICO_ROUTER_ID": "hash"}},
		{"mesh/communities", "mesh/communities/input.yaml", "mesh/communities", nil},
		{"mesh/restart-time", "mesh/restart-time/input.yaml", "mesh/restart-time", nil},
		{"mesh/route-reflector-mesh-enabled", "mesh/route-reflector-mesh-enabled/input.yaml", "mesh/route-reflector-mesh-enabled", nil},
		{"mesh/static-routes", "mesh/static-routes/input.yaml", "mesh/static-routes", nil},
		{"mesh/static-routes-exclude-node", "mesh/static-routes-exclude-node/input.yaml", "mesh/static-routes-exclude-node", nil},

		// Explicit peering tests (nodeToNodeMeshEnabled=false).
		{"explicit_peering/global", "explicit_peering/global/input.yaml", "explicit_peering/global", nil},
		{"explicit_peering/global-external", "explicit_peering/global-external/input.yaml", "explicit_peering/global-external", nil},
		{"explicit_peering/global-ipv6", "explicit_peering/global-ipv6/input.yaml", "explicit_peering/global-ipv6", nil},
		{"explicit_peering/specific_node", "explicit_peering/specific_node/input.yaml", "explicit_peering/specific_node", nil},
		{"explicit_peering/selectors", "explicit_peering/selectors/input.yaml", "explicit_peering/selectors", nil},
		{"explicit_peering/route_reflector", "explicit_peering/route_reflector/input.yaml", "explicit_peering/route_reflector", nil},
		{"explicit_peering/route_reflector_v6_by_ip", "explicit_peering/route_reflector_v6_by_ip/input.yaml", "explicit_peering/route_reflector_v6_by_ip", nil},
		{"explicit_peering/keepnexthop", "explicit_peering/keepnexthop/input.yaml", "explicit_peering/keepnexthop", nil},
		{"explicit_peering/keepnexthop-global", "explicit_peering/keepnexthop-global/input.yaml", "explicit_peering/keepnexthop-global", nil},
		{"explicit_peering/local-as", "explicit_peering/local-as/input.yaml", "explicit_peering/local-as", nil},
		{"explicit_peering/local-as-global", "explicit_peering/local-as-global/input.yaml", "explicit_peering/local-as-global", nil},
		{"explicit_peering/local-as-ipv6", "explicit_peering/local-as-ipv6/input.yaml", "explicit_peering/local-as-ipv6", nil},
		{"explicit_peering/local-as-global-ipv6", "explicit_peering/local-as-global-ipv6/input.yaml", "explicit_peering/local-as-global-ipv6", nil},
	}
...
func runConfdTest(t *testing.T, inputYAML, goldenDir string) {
	...
	deleteK8sServices(t, sharedK8sClient)
	...
	// Apply kubectl resources if they exist for this test (Services, EndpointSlices, etc.).
	kubectlPath := filepath.Join(filepath.Dir(inputPath), "kubectl-input.yaml")
	if _, err := os.Stat(kubectlPath); err == nil {
		applyResources(t, sharedCalicoClient, sharedK8sClient, kubectlPath)
	}
