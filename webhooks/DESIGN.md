...
That makes the 403-vs-404 split an existence oracle, and it is an accepted one, not an oversight. A tier-restricted user naming a policy in a tier they cannot read gets 403; naming one that does not exist gets 404, because the decision fell through to RBAC. So they can distinguish "exists, and I may not see it" from "does not exist" for a name they can guess. Closing it would mean answering 403 for names that do not exist, which requires either denying the not-found case (breaking `kubectl get` of a policy the user *is* allowed to read but that has been deleted) or having the webhook fabricate a decision about an object it cannot see. The aggregated API server leaks the same distinction, so this is parity rather than a regression. Do not "fix" this without a decision to change that behavior deliberately.
