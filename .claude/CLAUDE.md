...
- An edit that does belong is normally **one to three lines**, edited into the
  section that already covers the area. Fix the existing sentence before adding
  a new one. A new `###` heading needs a new concept, not a new behaviour. Past
  about five lines you are narrating the change.
...
- Exemptions (no edit at all): bug fix restoring documented behaviour,
