...
func runInteractiveSelection(kubeClient kubernetes.Interface) (selection, bool, error) {
	...
	form := huh.NewForm(
		...
		// 3. Comparison nodes — by-node path: accept suggestions, choose manually,
		// or skip.
		huh.NewGroup(
			huh.NewSelect[string]().
				Title("Include comparison (healthy) nodes?").
				DescriptionFunc(func() string {
					if sug := suggestComparisonNodes(nodes, currentProblemNames(), defaultComparisonNodeCount); len(sug) > 0 {
						return "Suggested (similar role/zone, Ready): " + strings.Join(sug, ", ")
					}
					return "Healthy nodes collected alongside the problem nodes help spot what's different."
				}, &problemNodeSel).
				Options(
