...
func convertK8sResource(convResource unversioned.Resource) (converters.Resource, error) {
	...
	case "networkpolicy":
		...
		if !ok {
			// TODO: When type assertion to *resourceloader.K8sNetworkPolicy fails,
			// an assertion to resourceloader.K8sNetworkPolicy may still work.
			tmp, ok := convResource.(resourceloader.K8sNetworkPolicy)
			if !ok {
				return nil, fmt.Errorf("failed to convert resource to K8sNetworkPolicy")
			}

			k8sNetworkPolicy = &tmp
		}
