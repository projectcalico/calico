...
var _ = Describe("File and directory iteration", func() {
	...
	BeforeEach(func() {
		...
		f, err = os.TempFile(dname, "testfile*.yaml")
		Expect(err).NotTo(HaveOccurred())
		dfname1 = f.Name()
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())

		f, err = os.TempFile(dname, "testfile*.yml")
		Expect(err).NotTo(HaveOccurred())
		dfname2 = f.Name()
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())

		f, err = os.TempFile(dname, "testfile*.json")
		Expect(err).NotTo(HaveOccurred())
		dfname3 = f.Name()
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())

		f, err = os.TempFile(dname, "testfile*.txt")
		Expect(err).NotTo(HaveOccurred())
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())
		...
		f, err = os.TempFile(dname2, "testfile*.yaml")
		Expect(err).NotTo(HaveOccurred())
		df2name1 = f.Name()
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())

		f, err = os.TempFile(dname2, "testfile*.yml")
		Expect(err).NotTo(HaveOccurred())
		df2name2 = f.Name()
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())

		f, err = os.TempFile(dname2, "testfile*.json")
		Expect(err).NotTo(HaveOccurred())
		df2name3 = f.Name()
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())

		f, err = os.TempFile(dname2, "testfile*.txt")
		Expect(err).NotTo(HaveOccurred())
		err = f.Close()
		Expect(err).NotTo(HaveOccurred())
