...
`
		...
		Expect(kcc.Spec.EtcdV3CompactionPeriod).To(Equal("10m30s"))
		...
		Expect(kcc.Spec.Controllers.Policy).
			To(Equal(&api.PolicyControllerConfig{ReconcilerPeriod: "2m"}))
		Expect(kcc.Spec.Controllers.WorkloadEndpoint).
			To(Equal(&api.WorkloadEndpointControllerConfig{ReconcilerPeriod: "3m"}))
		Expect(kcc.Spec.Controllers.ServiceAccount).
			To(Equal(&api.ServiceAccountControllerConfig{ReconcilerPeriod: "4m"}))
		Expect(kcc.Spec.Controllers.Namespace).
			To(Equal(&api.NamespaceControllerConfig{ReconcilerPeriod: "5m"}))
		...
		Expect(kcc.Status.RunningConfig.EtcdV3CompactionPeriod).To(Equal(""))
	})
