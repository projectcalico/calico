...
import (
	...
	"github.com/projectcalico/calico/hack/argoci/report"
)
...
func main() {
	...
	case *cron != "":
		fmt.Print(report.CSV(cronJobs(*cron)))
	case *sem != "":
		fmt.Print(report.CSV(semaphoreJobs(*sem)))
	default:
...
func runDiff(sem, argo []map[string]string) int {
	onlyS, onlyA := report.Diff(sem, argo)
