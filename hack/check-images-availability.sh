...
# 🔍 Search entire home directory for the secrets folder
echo "🔍 Searching entire home for a 'secrets' folder:"
find "$HOME" -type d -name "secrets" 2>/dev/null || echo "🔎 No 'secrets' folder found"

# 🔍 Search entire home directory for the GCP JSON file
echo "🔍 Searching entire home for GCP key file:"
find "$HOME" -type f -name "gcp-registry-pusher-service-account.json" 2>/dev/null || echo "🔎 No GCP key file found"
...
if [[ -f "$HOME/secrets/gcp-registry-pusher-service-account.json" ]]; then
  echo "🔐 Found GCR service account key at ~/secrets/gcp-registry-pusher-service-account.json"
  "$CRANE" auth login gcr.io -u _json_key -p "$(cat "$HOME/secrets/gcp-registry-pusher-service-account.json")"
else
  echo "ℹ️  No GCR service account key found at ~/secrets/gcp-registry-pusher-service-account.json"
  echo "   Skipping crane login — assuming local Docker or cached credentials"
fi
