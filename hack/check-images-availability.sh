...
# Show full resolved paths
echo "🔧 Using yq at: $(realpath "$YQ" 2>/dev/null || echo "$YQ")"
echo "🔧 Using crane at: $(realpath "$CRANE" 2>/dev/null || echo "$CRANE")"

# List contents of bin directories
echo "📂 Contents of $(dirname "$YQ"):"
ls -l "$(dirname "$YQ")" || echo "⚠️  Could not list $(dirname "$YQ")"

echo "📂 Contents of $(dirname "$CRANE"):"
ls -l "$(dirname "$CRANE")" || echo "⚠️  Could not list $(dirname "$CRANE")"

# Check dependencies
for cmd in docker crane; do
  if ! command -v "$cmd" &> /dev/null; then
    echo "❌ Error: Required dependency '$cmd' is not installed or not in PATH." >&2
    echo "🔍 Resolved path attempt: $(realpath "${!cmd}" 2>/dev/null || echo '<unresolvable>')" >&2
    exit 1
  fi
done
