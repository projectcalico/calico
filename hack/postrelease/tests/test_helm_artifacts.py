...
def operator_helm_chart_entry():
    ...
    entries = [e for e in index["entries"]["tigera-operator"] if e["appVersion"] == variables.RELEASE_VERSION]
...
def test_calico_helm_version(operator_helm_chart_entry):
    ...
    assert entry_version == variables.RELEASE_VERSION, f"Chart version ({entry_version}) incorrect in helm index"
...
def test_calico_chart_url(operator_helm_chart_entry):
    ...
    assert entry_chart_url == CHART_URL, f"Chart URL (entry_chart_url) incorrect in helm index"
...
def test_calico_operator_url_exists(operator_helm_chart_entry):
    ...
    assert response.status_code == 200, f"Chart URL returned HTTP {response.status_code}"