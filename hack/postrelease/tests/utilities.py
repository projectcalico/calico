...
def download_url_to_file(url, destination):
    ...
    with rich.progress.wrap_file(response.raw, total=length, description=filename) as infile:
...
def tarfile_members_to_map(tf):
    ...
    file_map = {member.get_info()['name']: member for member in members if not member.get_info()['name'].endswith("/")}
    return file_map
