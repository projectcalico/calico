...
func TestNilPolicyUpdateCompilesNothing(t *testing.T) {
	...
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.1"))
	store.ProcessUpdate("", policyUpdate("policy1", &proto.Policy{
	...
	store.ProcessUpdate("", policyUpdate("policy1", nil))
	store.ProcessUpdate("", profileUpdate("profile1", nil))
...
func TestEndpointCompilation(t *testing.T) {
	...
	store.ProcessUpdate("", endpointUpdate("wep1", ep))
	...
	store.ProcessUpdate("", policyUpdate("policy1", &proto.Policy{}))
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.2"))
	Expect(compiler.endpointCompiles).To(Equal(1))
	...
	store.ProcessUpdate("", endpointUpdate("wep1", ep2))
	...
	store.ProcessUpdate("", endpointRemove("wep1"))
...
func TestEndpointCompilationPerHost(t *testing.T) {
	...
	store.ProcessUpdate("per-host-policies", endpointUpdate("wep1", ep1))
	store.ProcessUpdate("per-host-policies", endpointUpdate("wep2", ep2))
	Expect(store.CompiledEndpoints).To(HaveLen(2))

	store.ProcessUpdate("per-host-policies", endpointRemove("wep1"))
	Expect(store.CompiledEndpoints).To(HaveLen(1))
	Expect(store.CompiledEndpoints).To(HaveKey(ep2))
}
...
func TestIPSetInvalidation(t *testing.T) {
	...
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.1"))
	store.ProcessUpdate("", ipSetUpdate("set-b", "10.0.0.2"))
	...
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.3"))
	...
	store.ProcessUpdate("", ipSetDeltaUpdate("set-a", "10.0.0.4"))
	...
	store.ProcessUpdate("", ipSetRemove("set-b"))
	...
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.5"))
	Expect(compiler.policyCompiles[pID("refs-a")]).To(Equal(3))
	...
	store.ProcessUpdate("", policyRemove("refs-a"))
	store.ProcessUpdate("", policyRemove("refs-b"))
	store.ProcessUpdate("", profileRemove("profile-a"))
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.6"))
	Expect(compiler.policyCompiles[pID("refs-a")]).To(Equal(3))
	Expect(compiler.profileCompiles[prID("profile-a")]).To(Equal(3))
	Expect(store.ipSetPolicyRefs).To(BeEmpty())
...
func TestNilCompilerIsNoOp(t *testing.T) {
	...
	store.ProcessUpdate("", profileUpdate("profile1", &proto.Profile{}))
	store.ProcessUpdate("", ipSetUpdate("set-a", "10.0.0.2"))
	store.ProcessUpdate("", ipSetRemove("set-a"))
...
func TestManagerThreadsCompilerThroughReconnect(t *testing.T) {
	...
	sync := func() {
		m.DoWithLock(func(s *PolicyStore) {
			s.ProcessUpdate("", policyUpdate("policy1", &proto.Policy{}))
		})
		m.OnInSync()
	}
