...
func checkTiers(store *policystore.PolicyStore, ep *proto.WorkloadEndpoint, dir rules.RuleDir, flow Flow) (s status.Status, trace []*calc.RuleID) {
	...
	Policy:
		for i, pID := range policies {
			...
			policyName := getPolicyName(pID.Name)
			switch action {
			case NO_MATCH:
				if tierDefaultActionRuleID == nil {
					tierDefaultActionRuleID = calc.NewRuleID(pID.Kind, tier.GetName(), policyName, policy.GetNamespace(), tierDefaultActionIndex, dir, ruleActionFromStr(tier.DefaultAction))
				}
				continue Policy
			// If the Policy matches, end evaluation (skipping profiles, if any)
			case ALLOW:
				s.Code = OK
				trace = append(trace, calc.NewRuleID(pID.Kind, tier.GetName(), policyName, policy.GetNamespace(), ruleIndex, dir, rules.RuleActionAllow))
				return
			case DENY:
				s.Code = PERMISSION_DENIED
				trace = append(trace, calc.NewRuleID(pID.Kind, tier.GetName(), policyName, policy.GetNamespace(), ruleIndex, dir, rules.RuleActionDeny))
				return
			case PASS:
				trace = append(trace, calc.NewRuleID(pID.Kind, tier.GetName(), policyName, policy.GetNamespace(), ruleIndex, dir, rules.RuleActionPass))
...
//
// TODO: Fix this function up based on new naming conventions.
