...
func (cp *compiledPolicy) check(dir rules.RuleDir, req *requestCache) (Action, int) {
	...
	// policy; requestCache.GetProtocol has already warned about the value,
	// once per request.
	if !validL4Protocol(req.GetProtocol()) {
...
// in the same cheapest-first order the uncompiled path (match) checks them:
// protocol, ports, CIDRs, IP sets, identity, HTTP. A rule's criteria are ANDed
// pure predicates, so the verdict is the same in any order; the order matters
// because most rules are rejected by exactly one criterion and the walk stops
// at it, and because the HTTP matcher panics on a malformed request path, so
// last position confines that to requests a rule otherwise fully matches,
// exactly as the uncompiled path does. A criterion whose fields are empty
// always matches, so it is omitted.
func appendRuleMatchers(ms []ruleMatcher, store *policystore.PolicyStore, r *proto.Rule, policyNamespace string) []ruleMatcher {
	...
	// L4 header: integer comparisons against the flow.
	...
	add(compilePortsMatcher(r.GetSrcPorts(), r.GetNotSrcPorts(),
		...
		(*requestCache).GetSourcePort, (*requestCache).getSrcIPProtoPortStr))
	add(compilePortsMatcher(r.GetDstPorts(), r.GetNotDstPorts(),
		...
		(*requestCache).GetDestPort, (*requestCache).getDstIPProtoPortStr))

	// Addresses: CIDRs (parsed once, here), then IP set lookups.
	...
	// Identity: service account then namespace, source side then destination
	// (matchSrcIdentity, matchDstIdentity). The namespace match depends only on
	// the rule and the policy's namespace, so it is computed once here rather
	// than per flow.
...
// of the not-ranges or not-named-port sets. A named port set holds
// "<IP>,<protocol>:<port>" members rather than bare port numbers (see
// matchPort); namedPortKey supplies that key for the leg being tested, and is
// only consulted once the ranges have failed to decide the criterion.
func compilePortsMatcher(
	ports, notPorts []*proto.PortRange, namedSets, notNamedSets []resolvedIPSet,
	getPort func(*requestCache) int, namedPortKey func(*requestCache) string,
) ruleMatcher {
	...
	return func(req *requestCache) bool {
		...
		if len(ranges) > 0 || len(namedSets) > 0 {
			if !portMatches(port, ranges, namedSets, req, namedPortKey) {
				return false
			}
		}
		if len(notRanges) > 0 || len(notNamedSets) > 0 {
			if portMatches(port, notRanges, notNamedSets, req, namedPortKey) {
				return false
			}
		}
		return true
...
func portMatches(port int32, ranges []portRange, namedSets []resolvedIPSet, req *requestCache, namedPortKey func(*requestCache) string) bool {
	...
	if len(namedSets) > 0 {
		key := namedPortKey(req)
		for _, s := range namedSets {
			if s.set != nil && s.set.Contains(key) {
				return true
			}
		}
	}
...
func compileProtocolMatcher(p, notP *proto.Protocol) ruleMatcher {
	...
	resolve := func(p *proto.Protocol) (int32, bool) {
		if name := p.GetName(); name != "" {
			...
			// Narrowing to the proto's int32: every value in the map fits.
			return int32(n), ok
		}
