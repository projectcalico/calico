...
func checkStoreBothEngines(scope PolicyScope, store *policystore.PolicyStore, ep *proto.WorkloadEndpoint, dir rules.RuleDir, req Flow) *status.Status {
	s, _ := checkTiersBothEngines(scope, store, ep, dir, req)
	return s
}

// checkTiersBothEngines is checkStoreBothEngines for callers that also want
// the rule trace.
func checkTiersBothEngines(scope PolicyScope, store *policystore.PolicyStore, ep *proto.WorkloadEndpoint, dir rules.RuleDir, req Flow) (*status.Status, []*calc.RuleID) {
	s, trace := checkTiers(scope, store, ep, dir, req, nil)
	compileStoreForTest(store)
	compiledS, compiledTrace := checkTiers(scope, store, ep, dir, req, nil)
...
func TestCompilePolicyBadAction(t *testing.T) {
	...
	s := checkStore(EnforcedOnly, store, ep, rules.RuleDirIngress, flow)
...
func TestCompiledIPSetReplacement(t *testing.T) {
	...
	store.ProcessUpdate("", ipSetUpdate("10.0.0.1/32"))
	...
	store.ProcessUpdate("", ipSetUpdate("10.0.0.2/32"))
	s = checkStore(EnforcedOnly, store, ep, rules.RuleDirIngress, flow)
...
func TestCompiledEndpointFollowsPolicyUpdates(t *testing.T) {
	...
	// Removing the policy must stop the endpoint evaluating it: a policy
	// missing from the store fails the evaluation closed (INTERNAL), whichever
	// engine would have evaluated it.
	...
	Expect(checkStore(EnforcedOnly, store, ep, rules.RuleDirIngress, flow).Code).To(Equal(INTERNAL))
...
func TestCompiledRuleIDMemo(t *testing.T) {
	...
	traceFor := func(ep *proto.WorkloadEndpoint) []*calc.RuleID {
		trace, err := Evaluate(EnforcedOnly, rules.RuleDirIngress, store, ep, flow, nil)
...
func TestCompiledRuleIDMemoConcurrent(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			...
			for j := 0; j < 100; j++ {
				trace, err := Evaluate(EnforcedOnly, rules.RuleDirIngress, store, ep, flow, nil)
				if err != nil || len(trace) != 1 || trace[0].Tier != "tier1" || trace[0].Action != rules.RuleActionAllow {
...
// case: the evaluation fails closed.
func TestCompiledStaleEndpointReference(t *testing.T) {
	...
	checkStoreBothEngines(EnforcedOnly, store, ep, rules.RuleDirIngress, flow)
}
