...
func TestMalformedHTTPPathOnlyFailsRulesThatReachIt(t *testing.T) {
	...
	ep := &proto.WorkloadEndpoint{Tiers: []*proto.TierInfo{{
		Name:            "tier1",
		IngressPolicies: []*proto.PolicyID{udpWithPaths, allowsTCP},
		DefaultAction:   "Deny",
	}}}
	...
	Expect(checkStore(store, ep, rules.RuleDirIngress, flow).Code).To(Equal(INVALID_ARGUMENT))
}
...
func TestProtocolResolvedOncePerRequest(t *testing.T) {
	...
	ep := &proto.WorkloadEndpoint{Tiers: []*proto.TierInfo{{
		Name:            "tier1",
		IngressPolicies: []*proto.PolicyID{policy},
		DefaultAction:   "Deny",
	}}}
