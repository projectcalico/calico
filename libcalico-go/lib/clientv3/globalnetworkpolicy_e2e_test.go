	...
	DescribeTable("GlobalNetworkPolicy e2e CRUD tests",
		func(tier, name1, name2 string, spec1, spec2 apiv3.GlobalNetworkPolicySpec, types1, types2 []apiv3.PolicyType) {
			...
			if tier != "" && tier != "default" {
				Expect(names.ValidateTierName(tier)).To(BeTrue())
			}
		...
		// Pass two fully populated GlobalNetworkPolicySpecs in the AdminNetworkPolicy tier, and expect the series of operations to succeed.
		Entry("Two fully populated GlobalNetworkPolicySpecs", tierANP, tierANP+"."+name1, tierANP+"."+name2, spec1, spec2, ingressEgress, ingressEgress),
	)
