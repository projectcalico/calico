...
var _ = testutils.E2eDatastoreDescribe("Node tests (kdd)", testutils.DatastoreK8s, func(config apiconfig.CalicoAPIConfig) {
	...
	It("should delete labels on a node", func() {
		...
		node.Spec.BGP = &internalapi.NodeBGPSpec{IPv4Address: "10.0.0.1"}
...
var _ = testutils.E2eDatastoreDescribe("Node tests (etcdv3)", testutils.DatastoreEtcdV3, func(config apiconfig.CalicoAPIConfig) {
	...
	Describe("nodes", func() {
		It("should clean up weps, IPAM allocations, etc. when deleted", func() {
			...
			// Create a node.
			n, err := c.Nodes().Create(ctx, &internalapi.Node{
	...
	DescribeTable("Node e2e CRUD tests",
		func(name1, name2 string, spec1, spec2 internalapi.NodeSpec, status internalapi.NodeStatus) {
			c, err := clientv3.New(config)
			Expect(err).NotTo(HaveOccurred())
			...
			_, outError := c.Nodes().Update(ctx, &internalapi.Node{
			...
			res1, outError := c.Nodes().Create(ctx, &internalapi.Node{
				ObjectMeta: metav1.ObjectMeta{Name: name1},
				Spec:       spec1,
			}, options.SetOptions{})
			Expect(outError).NotTo(HaveOccurred())
			Expect(res1).To(MatchResource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec1))
			...
			Expect(outError).NotTo(HaveOccurred())
			Expect(res).To(MatchResource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec1))
			...
			Expect(outList.Items).To(ConsistOf(
				testutils.Resource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec1),
			))

			By("Creating a new Node with name2/spec2")
			res2, outError := c.Nodes().Create(ctx, &internalapi.Node{
				ObjectMeta: metav1.ObjectMeta{Name: name2},
				Spec:       spec2,
			}, options.SetOptions{})
			Expect(outError).NotTo(HaveOccurred())
			Expect(res2).To(MatchResource(internalapi.KindNode, testutils.ExpectNoNamespace, name2, spec2))
			...
			Expect(outList.Items).To(ConsistOf(
				testutils.Resource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec1),
				testutils.Resource(internalapi.KindNode, testutils.ExpectNoNamespace, name2, spec2),
			))
			...
			Expect(outError).NotTo(HaveOccurred())
			Expect(res1).To(MatchResource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec2))

			By("Attempting to update the Node without a Creation Timestamp")
			res, outError = c.Nodes().Update(ctx, &internalapi.Node{
			...
			Expect(outError).NotTo(HaveOccurred())
			Expect(res).To(MatchResource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec2))
			...
			Expect(outError).NotTo(HaveOccurred())
			Expect(outList.Items).To(ConsistOf(
				testutils.Resource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec2),
				testutils.Resource(internalapi.KindNode, testutils.ExpectNoNamespace, name2, spec2),
			))
			...
			Expect(outError).NotTo(HaveOccurred())
			Expect(dres).To(MatchResource(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec2))
			...
			Expect(outError).ToNot(HaveOccurred())
			Expect(res).To(MatchResourceWithStatus(internalapi.KindNode, testutils.ExpectNoNamespace, name1, spec1, status))
	...
	Describe("Node watch functionality", func() {
		It("should handle watch events for different resource versions and event types", func() {
			...
			testWatcher1.ExpectEvents(internalapi.KindNode, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes2,
				},
			...
			Expect(err).NotTo(HaveOccurred())
			testWatcher2.ExpectEvents(internalapi.KindNode, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
			...
			if config.Spec.DatastoreType == apiconfig.EtcdV3 {
				...
				testWatcher2_1.ExpectEvents(internalapi.KindNode, []watch.Event{
					{
						Type:   watch.Added,
						Object: outRes1,
					},
			...
			testWatcher3.ExpectEvents(internalapi.KindNode, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes3,
				},
			...
			testWatcher4.ExpectEventsAnyOrder(internalapi.KindNode, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
