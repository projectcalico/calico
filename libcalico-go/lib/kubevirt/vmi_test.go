...
// TestFindVMIMByUID_Found tests that FindVMIMByUID finds a VMIM by UID.
func TestFindVMIMByUID_Found(t *testing.T) {
	fakeClient := kubevirt.NewFakeVirtClient()
	...
	// Verify VMIM by UID
	found, err := kubevirt.FindVMIMByUID(nil, fakeClient, namespace, migrationUID)
	if err != nil {
		t.Errorf("Expected no error, got: %v", err)
	}
	if found == nil {
...
// TestFindVMIMByUID_NotFound tests that FindVMIMByUID returns nil when VMIM is not found.
func TestFindVMIMByUID_NotFound(t *testing.T) {
	fakeClient := kubevirt.NewFakeVirtClient()
	...
	found, err := kubevirt.FindVMIMByUID(nil, fakeClient, namespace, nonExistentUID)
	if err != nil {
		t.Errorf("Expected no error, got: %v", err)
	}
	if found != nil {
...
// TestFindVMIMByUID_EmptyUID tests that FindVMIMByUID returns an error for empty UID.
func TestFindVMIMByUID_EmptyUID(t *testing.T) {
	fakeClient := kubevirt.NewFakeVirtClient()

	namespace := "default"

	// Verify VMIM with empty UID (should return error)
	found, err := kubevirt.FindVMIMByUID(nil, fakeClient, namespace, "")
	if err == nil {
...
// TestFindVMIMByUID_MultipleVMIMs tests that FindVMIMByUID finds the correct VMIM when multiple exist.
func TestFindVMIMByUID_MultipleVMIMs(t *testing.T) {
	fakeClient := kubevirt.NewFakeVirtClient()
	...
	found, err := kubevirt.FindVMIMByUID(nil, fakeClient, namespace, targetUID)
	if err != nil {
		t.Errorf("Expected no error, got: %v", err)
	}
	if found == nil {
