...
func GetPodVMIInfo(pod *corev1.Pod, virtClient VirtClientInterface) (*PodVMIInfo, error) {
	...
	if pod.Labels != nil {
		if migrationUID, ok := pod.Labels[LabelKubeVirtMigrationJobUID]; ok {
			...
			vmim, err := FindVMIMByUID(context.Background(), virtClient, pod.Namespace, migrationUID)
			if err != nil {
...
// FindVMIMByUID finds a VirtualMachineInstanceMigration with the given UID
...
func FindVMIMByUID(ctx context.Context, virtClient VirtClientInterface, namespace, migrationUID string) (*kubevirtv1.VirtualMachineInstanceMigration, error) {
	if migrationUID == "" {
