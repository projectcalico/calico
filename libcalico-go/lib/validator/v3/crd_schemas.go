...
// load parses all embedded CRDs and stores their schemas. Called once via
// r.loadOnce. CEL compilation happens per-Kind on demand.
...
// single CRD Kind. compile() uses sync.Once internally so it is safe to
