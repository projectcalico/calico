...
func newMockListWatchBackend() *mockListWatchBackend {
	return &mockListWatchBackend{
		...
		shouldResync: true,
		syncStrategy: ListThenWatchStrategy{},
	}
}
...
func (m *mockListWatchBackend) OnListSuccess() {
	m.listSuccessCalls++
}

func (m *mockListWatchBackend) ShouldPerformFullResync() bool {
	return m.shouldResync
}

func (m *mockListWatchBackend) GetSyncStrategy() SyncStrategy {
	return m.syncStrategy
}
...
func TestNewGenericListWatcher(t *testing.T) {
	...
	assert.Equal(t, "0", lw.CurrentRevision)
...
func TestListThenWatchStrategy_PerformInitialSync_Success(t *testing.T) {
	handler := newMockEventHandler()
	lw := NewGenericListWatcher(testListOptions(), handler)

	backend := newMockListWatchBackend()
	strategy := ListThenWatchStrategy{}

	err := strategy.PerformInitialSync(context.Background(), lw, backend)
	...
	assert.Len(t, handler.addEvents, 1)
	assert.Equal(t, "100", lw.CurrentRevision)
	assert.Equal(t, 1, backend.listSuccessCalls)
}

func TestListThenWatchStrategy_PerformInitialSync_ListError(t *testing.T) {
	handler := newMockEventHandler()
	lw := NewGenericListWatcher(testListOptions(), handler)
...
func TestListThenWatchStrategy_PerformInitialSync_EmptyRevisionNoItems(t *testing.T) {
	handler := newMockEventHandler()
	lw := NewGenericListWatcher(testListOptions(), handler)
	...
	assert.Error(t, err)
	assert.Equal(t, "0", lw.CurrentRevision) // Should reset for resync
	assert.True(t, lw.RetryBlockedUntil.After(time.Now()))
}

func TestListThenWatchStrategy_PerformInitialSync_ZeroRevisionNoItems(t *testing.T) {
	handler := newMockEventHandler()
	lw := NewGenericListWatcher(testListOptions(), handler)
...
func TestListThenWatchStrategy_PerformInitialSync_ZeroRevisionWithItems_Panics(t *testing.T) {
	handler := newMockEventHandler()
	lw := NewGenericListWatcher(testListOptions(), handler)
	...
	backend.listResult = &model.KVPairList{
		...
		Revision: "0", // Zero revision with items - BUG condition
	}
	...
	assert.Panics(t, func() {
		strategy.PerformInitialSync(context.Background(), lw, backend)
	})
}

func TestWatchListStrategy_PerformInitialSync(t *testing.T) {
	handler := newMockEventHandler()
	lw := NewGenericListWatcher(testListOptions(), handler)
	...
	assert.NoError(t, err)
	// WatchListStrategy always calls OnResyncStarted because with SendInitialEvents=true,
	// the server will re-send all initial events regardless of CurrentRevision
	assert.Equal(t, 1, handler.resyncStartedCt)
...
func TestGenericListWatcher_RunLoopWithBackend_NoResyncNeeded(t *testing.T) {
	...
	backend := newMockListWatchBackend()
	backend.shouldResync = false // No resync needed
