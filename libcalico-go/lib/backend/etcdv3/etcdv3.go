...
func NewEtcdV3Client(config *apiconfig.EtcdConfig) (api.Client, error) {
	...
	// rather than surfacing later on an unrelated read.  Report it as a
	// datastore error, which is how callers distinguish "cannot reach the
	// datastore" from a bad configuration.
