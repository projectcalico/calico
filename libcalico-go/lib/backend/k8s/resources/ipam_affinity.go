...
func (c blockAffinityClient) toV3(kvpv1 *model.KVPair) *model.KVPair {
	var name, cidr, host, state string
	switch val := kvp1.Value.(type) {
	case *model.BlockAffinity:
		name, cidr, host = c.parseKey(kvpv1.Key)
		state = string(val.State)
	case *libapiv3.BlockAffinity:
		name = kvp1.Key.(*model.ResourceKey).Name
		cidr = val.Spec.CIDR
		host = val.Spec.Node
		state = val.Spec.State
	default:
		log.Panic("blockAffinityClient: wrong interface type")
	}
	return &model.KVPair{
