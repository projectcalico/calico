...
var _ = Describe("Custom resource conversion methods (tested using BGPPeer)", func() {
	...
	It("should convert between a KVPair and the equivalent Kubernetes resource", func() {
		...
		r.GetObjectMeta().SetUID("30316465-6365-4463-ad63-3564622d3638")
	...
	It("should convert between a Kubernetes resource and the equivalent KVPair", func() {
		...
		// Converting Calico -> K8s rightfuly removes the UID from the converted resource, as
		// two resources with the same UID are not allowed. Add a new one to simulate the UID
		// generated by Kubernetes for the underlying CR.
		resConverted.GetObjectMeta().SetUID("41cb1fde-57e7-42c1-a73b-0acaf38c7737")

		kvp, err = client.convertResourceToKVPair(resConverted)
		Expect(err).NotTo(HaveOccurred())

		// Assert the UID was translated properly then clear it out to
		// compar the remaining fields.
		expectedUID := types.UID("33303331-3634-4635-ad36-3336352d3434")
		Expect(kvp.Value.(*apiv3.BGPPeer).UID).To(Equal(expectedUID))
		kvp.Value.(*apiv3.BGPPeer).SetUID("")
