...
func (wc defaultWorkloadEndpointConverter) podToDefaultWorkloadEndpoint(pod *kapiv1.Pod) (*model.KVPair, error) {
	...
	var requestedSourceIPs []string
	if annotation, ok := pod.Annotations["cni.projectcalico.org/allowedSourceIPs"]; ok && annotation != "" {
		// Parse Annotation data
		err := json.Unmarshal([]byte(annotation), &requestedSourceIPs)
		if err != nil {
			return nil, fmt.Errorf("failed to parse '%s' as JSON: %s", annotation, err)
		}
	}
	...
	wep.Spec = libapiv3.WorkloadEndpointSpec{
		...
		AllowSpoofedSourceIPs: requestedSourceIPs,
	}
