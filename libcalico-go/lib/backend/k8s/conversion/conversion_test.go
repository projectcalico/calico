	...
	It("should look the source spoofing disabling annotation", func() {
		pod := kapiv1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					"cni.projectcalico.org/podIP":            "192.168.0.1",
					"cni.projectcalico.org/allowedSourceIPs": "[\"8.8.8.8\",\"1.1.1.1\"]",
				},
				ResourceVersion: "1234",
			},
			Spec: kapiv1.PodSpec{
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(c.HasIPAddress(&pod)).To(BeTrue())
		Expect(wep.Value.(*libapiv3.WorkloadEndpoint).Spec.AllowSpoofedSourceIPs).To(ConsistOf([]string{"1.1.1.1", "8.8.8.8"}))
	})
