...
func (d *DeduplicatingBuffer) OnStatusUpdated(status api.SyncStatus) {
	...
	// Statuses are idempotent so skip sending if the latest one in the queue was the
	// same.
	...
	// this prevents us from growing the queue without bound if status is flapping.
	// We can add at most one status to the queue for each non-status update
	// and the number of non-status updates is bounded byt he size of the datastore.
...
// deduplicate in-flight updates to the same keys.  It should only block for short
// periods even if the downstream sink blocks for a long time.
func (d *DeduplicatingBuffer) OnUpdates(updates []api.Update) {
	d.OnUpdatesKeysKnown(updates, nil)
}

// OnUpdatesKeysKnown is like OnUpdates, but it allows for the pre-serialised keys of the
// KV pairs to be passed in.  If an entry in keys is "" or if keys is shorter than updates
// the key will be computed.
//
