...
type TyphaRevision struct {
	// ServerID is unique per invocation of Typha.  Allows the client to spot
	// when it has reconnected to a different Typha instance and the Revision
	// is from a different sequence.
	ServerID string `json:"serverID,omitempty"`
	// Revision contains Typha's current internal per-instance revision.
	Revision string `json:"revision,omitempty"`
	// Timestamp is the time at which Typha generated this revision.
