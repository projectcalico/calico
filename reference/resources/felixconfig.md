...
| ipipMTU                            | The MTU to set on the tunnel device. Zero value means auto-detect. See Configuring MTU {%- if site.include_calicoctl_resource %}[Configuring MTU]({{ site.baseurl }}/networking/mtu){%- endif %} | int | int | `0` |
...
| policySyncPathPrefix               | File system path where Felix notifies services of policy changes over Unix domain sockets. This is only required if you're configuring application layer policy {%- if site.include_calicoctl_resource %}[application layer policy]({{ site.baseurl }}/security/app-layer-policy){%- endif %}. Set to `""` to disable. | string | string | `""` |
...
| serviceLoopPrevention              | When service IP advertisement is enabled {%- if site.include_calicoctl_resource %}[service IP advertisement is enabled]({{ site.baseurl }}/networking/advertise-service-ips){%- endif %}, prevent routing loops to service IPs that are not in use, by dropping or rejecting packets that do not get DNAT'd by kube-proxy.  Unless set to "Disabled", in which case such routing loops continue to be allowed. | `Drop`, `Reject`, `Disabled` | string | `Drop` |
| sidecarAccelerationEnabled         | Enable experimental acceleration between application and proxy sidecar when using application layer policy {%- if site.include_calicoctl_resource %}[application layer policy]({{ site.baseurl }}/security/app-layer-policy){%- endif %}. [Default: `false`] | boolean | boolean | `false` |
...
| wireguardMTU                       | MTU set on the WireGuard interface created by Felix. Zero value means auto-detect. See Configuring MTU {%- if site.include_calicoctl_resource %}[Configuring MTU]({{ site.baseurl }}/networking/mtu){%- endif %}. | int | int | 0 |
...
| bpfEnabled                         | Enable eBPF dataplane mode.  eBPF mode has some limitations, see the guide to enable eBPF {%- if site.include_calicoctl_resource %}[HOWTO guide]({{ site.baseurl }}/maintenance/ebpf/enabling-bpf){%- endif %} for more details. | true, false | boolean | false |
