...
* Use Kubernetes {% include open-new-window.html text='Secrets' url='http://kubernetes.io/docs/user-guide/secrets/' %} to mount the certificates into the pod as files.
...
| `COMPACTION_PERIOD` | Compact the etcd database on this interval. Set to "0" to disable. | {% include open-new-window.html text='duration' url='https://golang.org/pkg/time/#ParseDuration' %} | 10m
