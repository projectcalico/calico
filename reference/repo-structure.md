...
- `calico/node`: the Docker image used to run the Calico Felix agent
  ...
  distributing routes between hosts).  See [`calico_node` directory]({{site.url}}/reference/architecture)
  for details.
- `calicoctl`: the command line tool used for starting a `calico/node`
  ...
  diagnostic administration.  See [`calicoctl` directory]({{site.url}}/reference/calicoctl) for
