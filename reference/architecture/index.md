...
for some or all of the following tasks:
...
into the Linux kernel FIB. This ensures that packets destined for those
...
network just as they'd manage network tools that were built in to the
orchestrator.
...
committing state to the database to cause that state to be programmed
into the network.
...
[BIRD](http://bird.network.cz/), though any BGP client that can draw
