...
- [Secure {{site.prodname}} components with network policy](#secure-calico-components-with-network-policy)
...
1. Install {{site.prodname}} by creating the necessary custom resource. For more information on configuration options available in this manifest, see [the installation reference]({{site.baseurl}}/reference/installation/api).
   ...
   > **Note**: If you are installing {{site.prodname}} on Windows nodes in this cluster, please see the [{{site.prodnameWindows}} for RKE]({{site.baseurl}}/windows-calico/kubernetes/rancher) installation instructions.
   {: .alert .alert-info}
