...
>**Note**: Windows Server 1809 (build 10.0.17763) does not currently support {% include open-new-window.html text='direct server return' url='https://techcommunity.microsoft.com/t5/networking-blog/direct-server-return-dsr-in-a-nutshell/ba-p/693710' %}. This means that policy support is limited to only pod IP addresses.
{: .alert .alert-info}
...
If you are using a more recent Windows Server version, update the manifests to use a {% include open-new-window.html text='servercore image' url='https://hub.docker.com/_/microsoft-windows-servercore' %}
