...
func New(shutdownCtx context.Context, tunnelCert *tls.Certificate, dialer tunnel.SessionDialer, opts ...Option) (Server, error) {
	...
	for _, target := range srv.targets {
		...
		if target.Dest != nil {
			log.Infof("Will route traffic to %s for requests matching %s", target.Dest.Redacted(), target.Path)
		} else {
			log.Infof("Will route traffic for requests matching %s, but destination is not configured", target.Path)
		}
	}
