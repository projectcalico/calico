...
import (
	...
	mocktimeutil "github.com/projectcalico/calico/guardian/pkg/thirdpartymocks/std/timeutil"
...
func TestTunnelOpenConnection(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.description, func(t *testing.T) {
			...
			ctx, cancel := context.WithDeadline(context.Background(), timeutil.Now().Add(time.Second*30))
			defer func() {
				cancel()
				<-tun.WaitForClose()
			}()
...
func TestTunnel_DialRetry(t *testing.T) {
	...
	mockTime := new(mocktimeutil.Interface)
	timeutil.SetTimeInterface(mockTime)
	...
	mockTicker := new(mocktimeutil.Ticker)
	mockTime.On("NewTicker", mock.Anything).Return(mockTicker).Once()
	mockTicker.On("Chan").Return((<-chan time.Time)(livenessTickerChan))
	mockTicker.On("Stop").Return()

	mockTimer := new(mocktimeutil.Timer)
	...
	mockTime.On("NewTimer", mock.Anything).Return(mockTimer)
