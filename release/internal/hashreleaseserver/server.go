...
func HasHashrelease(hash string, cfg *Config) (bool, error) {
	log.WithField("hash", hash).Debug("Checking if hashrelease exists")
	...
	if err != nil {
		if strings.Contains(err.Error(), "exited with status 1") {
			...
			log.WithError(err).Info("Hashrelease does not already exist on server")
			return false, nil
		} else {
			log.WithError(err).Error("Failed to check hashrelease library")
			return false, err
		}
	}
...
func SetHashreleaseAsLatest(rel Hashrelease, productCode string, cfg *Config) error {
	log.Debugf("Updating latest hashrelease for %s stream to %s", rel.Stream, rel.Name)
	if _, err := runSSHCommand(cfg, fmt.Sprintf(`echo "%s/" > %s/latest-%s/%s.txt`, rel.URL(), RemoteDocsPath(cfg.User), productCode, rel.Stream)); err != nil {
		log.WithError(err).Error("Failed to update latest hashrelease and hashrelease library")
		return err
	}
	return nil
}
...
func AddToHashreleaseLibrary(rel Hashrelease, cfg *Config) error {
	log.WithField("hashrelease", rel.Name).WithField("hash", rel.Hash).Debug("Adding hashrelease to library")
...
func CleanOldHashreleases(cfg *Config, maxToKeep int) error {
	log.WithField("maxToKeep", maxToKeep).Info("Cleaning old hashreleases")
	folders, err := listHashreleases(cfg)
	if err != nil {
		log.WithError(err).Error("Failed to list hashreleases")
		return err
	}
	...
	if len(foldersToDelete) == 0 {
		log.Infof("There are %d hashreleases which is less than maxToKeep %d", len(folders), maxToKeep)
		return nil
	}
	if _, err := runSSHCommand(cfg, fmt.Sprintf("rm -rf %s", strings.Join(foldersToDelete, " "))); err != nil {
		log.WithField("folders", strings.Join(foldersToDelete, ", ")).WithError(err).Error("Failed to delete old hashreleases")
		return err
	}
	log.Infof("Deleted %d old hashreleases", len(foldersToDelete))
	log.WithField("folders", strings.Join(foldersToDelete, ", ")).Debug("Deleted hashreleases")
	if err := cleanHashreleaseLibrary(cfg, hashreleaseToDeleteFromLibrary); err != nil {
		log.WithError(err).Warn("Failed to clean hashrelease library")
	}
	return nil
}
...
func listHashreleases(cfg *Config) ([]Hashrelease, error) {
	...
	if err != nil {
		log.WithError(err).Error("Failed to get list of hashreleases")
		return nil, err
	}
	lines := strings.Split(out, "\n")
	log.Debugf("hashrelease server returned %d folders in %s", len(lines), remoteDocsPath)
	...
	for _, line := range lines {
		...
		if err != nil {
			log.WithError(err).Errorf("Failed to parse time from field: %s", fields[1])
			continue
		}
	...
	log.Debugf("Found %d hashreleases", len(releases))
	return releases, nil
}
...
func getHashreleaseLibrary(cfg *Config) ([]string, error) {
	...
	if err != nil {
		log.WithError(err).Error("Failed to get hashrelease library")
		return nil, err
	}
...
func cleanHashreleaseLibrary(cfg *Config, hashreleaseNames []string) error {
	log.WithField("path", remoteReleasesLibraryPath(cfg.User)).Info("Updating hashrelease library")
	library, err := getHashreleaseLibrary(cfg)
	if err != nil {
		return err
	}
	log.Debugf("Hashrelease library currently has %d entries", len(library))
	newLibrary := []string{}
	...
	log.Debugf("Hashrelease library will have %d entries after cleaning", len(newLibrary))
	if _, err := runSSHCommandWithStdin(cfg, fmt.Sprintf("cat > %s", remoteReleasesLibraryPath(cfg.User)), bytes.NewBufferString(strings.Join(newLibrary, "\n"))); err != nil {
		log.WithError(err).Error("Failed to update hashrelease library")
		return err
	}
	return nil
}
