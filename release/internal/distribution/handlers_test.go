...
func TestS3CacheControl(t *testing.T) {
	...
	t.Run("sent when set", func(t *testing.T) {
		f := &fakeRunner{}
		d := S3{URI: "s3://b/x/", CacheControl: RewrittenObjectCacheControl, Runner: f}
		if err := d.Publish(context.Background(), srcPath(t, false)); err != nil {
			t.Fatalf("Publish: %v", err)
		}
		if got := f.valueAfter(cacheControlFlag); got != RewrittenObjectCacheControl {
			t.Errorf("%s = %q, want %q", cacheControlFlag, got, RewrittenObjectCacheControl)
		}
	})
}
