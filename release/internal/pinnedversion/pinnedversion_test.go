...
func TestImageComponents(t *testing.T) {
	...
	if cmp.Diff(expectedComponents, p.ImageComponents()) != "" {
		t.Errorf("expected components to be same, but they differ: %s", cmp.Diff(expectedComponents, p.ImageComponents()))
	}
}
...
func TestRetrieveImageComponents(t *testing.T) {
	...
	if cmp.Diff(expectedComponents, retrievedComponents) != "" {
