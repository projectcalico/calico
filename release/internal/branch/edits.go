...
// ApplyEdits writes each pending edit and collects skip reasons. written lists
// the changed files (deduped) to stage. Re-running is a no-op.
func ApplyEdits(repoRoot string, edits []Edit) (written, skipped []string, err error) {
	...
	for _, e := range edits {
		...
		if writeErr := os.WriteFile(filepath.Join(repoRoot, e.File), updated, 0o644); writeErr != nil {
			return written, skipped, fmt.Errorf("writing %s: %w", e.File, writeErr)
		}
		if !seen[e.File] {
			seen[e.File] = true
			written = append(written, e.File)
		}
	}
