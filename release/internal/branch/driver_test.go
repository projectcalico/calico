...
func localCutSteps(d *Driver, g git, p *CutPlan) []Step {
	...
	return []Step{
		...
		findStep(all, "update-main"),
		findStep(all, "tag-main"),
	}
}
...
func TestValidateFreshCutDirtyTree(t *testing.T) {
	...
	g := gitIn(root)
	// master has no upstream here, so the base sync is inert and only the
...
// TestValidateFreshCutNoValidate: Validate off skips the dirty-tree reject.
// master has no upstream here, so the base check is inert.
