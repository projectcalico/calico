...
func TestCutBranchStepIdempotent(t *testing.T) {
	...
	cut := findStep(d.steps(dg, p), "cut-branch")
...
func TestUpdateDerivedRunsPrepareBranch(t *testing.T) {
	...
	step := findStep(planD.steps(planG, p), "update-derived")
	...
	step = findStep(applyD.steps(applyG, p), "update-derived")
...
func TestPushRefsCheckMatchesRemoteSha(t *testing.T) {
	...
	staleD.Publish = true
	check := findStep(staleD.steps(staleG, p), "push-refs").Done
	done, err := check()
	require.NoError(t, err)
	...
	matchD.Publish = true
	check = findStep(matchD.steps(matchG, p), "push-refs").Done
	done, err = check()
	require.NoError(t, err)
...
func localCutSteps(d *Driver, g git, p *CutPlan) []Step {
	all := d.steps(g, p)
	return []Step{
		findStep(all, "cut-branch"),
		findStep(all, "update-derived"),
		findStep(all, "tag-derived"),
		findStep(all, "update-source"),
		findStep(all, "tag-source"),
	}
}
...
func TestFlowLandsEditOnNewBranch(t *testing.T) {
	...
	planD.PrepareDerived = prepare
	require.NoError(t, planD.runSteps(localCutSteps(planD, planG, newPlan())))
	exists, err := localBranchExists(planG, utils.ReleaseBranchPrefix()+"-v3.33")
	require.NoError(t, err)
	require.False(t, exists, "plan mode must not create the branch")

	// apply mode: branch is created, edit is committed on its head.
	applyD, applyG := stepDriver(root, nil, false, nil)
	applyD.PrepareDerived = prepare
	require.NoError(t, applyD.runSteps(localCutSteps(applyD, applyG, newPlan())))

	exists, err = localBranchExists(applyG, utils.ReleaseBranchPrefix()+"-v3.33")
	require.NoError(t, err)
...
func TestTagDerivedMovesStaleLocalTag(t *testing.T) {
	...
	all := d.steps(g, p)
	cut := findStep(all, "cut-branch")
	update := findStep(all, "update-derived")
	tagDerived := findStep(all, "tag-derived")
...
// The base check runs even with Validate off: a main behind its upstream is rejected.
func TestValidateFreshCutBaseCheckRunsWithoutValidate(t *testing.T) {
	root := initGitRepo(t)
	run := func(dir string, args ...string) {
		_, err := command.GitInDir(dir, args...)
		require.NoError(t, err, "git %v", args)
	}
	remote := t.TempDir()
	...
	runRoot := func(args ...string) { run(root, args...) }
	// Advance origin, then reset local master back so it is behind upstream.
	runRoot("branch", "--set-upstream-to=origin/master", "master")
	runRoot("commit", "-q", "--allow-empty", "-m", "ahead")
	runRoot("push", "-q", "origin", "master")
	runRoot("reset", "-q", "--hard", "HEAD~1")

	d := &Driver{RepoRoot: root, MainBranch: "master", Validate: false}
...
func TestFlowMainOnlyPlanNoDerivedTag(t *testing.T) {
	...
	require.NoError(t, d.runSteps(localCutSteps(d, g, p)))

	// The derived branch exists with its edits commit but no dev tag.
	exists, err := localBranchExists(g, utils.ReleaseBranchPrefix()+"-v3.33")
	require.NoError(t, err)
	require.True(t, exists)
...
func TestUpdateDerivedCommitsHookReturnedFiles(t *testing.T) {
	...
	all := d.steps(g, p)
	local := []Step{findStep(all, "cut-branch"), findStep(all, "update-derived")}
...
// bumpSourcePrepare returns a PrepareSource hook that bumps CALICO_VERSION
// on the source branch and records the branch it ran on.
func bumpSourcePrepare(t *testing.T, root string, ranOn *string) func(string) ([]string, error) {
	t.Helper()
	return func(sourceBranch string) ([]string, error) {
		*ranOn = sourceBranch
		written, skipped, err := ApplyEdits(root, []Edit{
...
// PrepareSource runs on the source branch, so the src bump lands
// there, not on the new branch, and master's dev tag sits on it.
func TestFlowSrcBumpLandsOnSourceBranch(t *testing.T) {
	root := srcBumpRepo(t)
	var ranOn string
	...
	d.PrepareSource = bumpSourcePrepare(t, root, &ranOn)

	require.NoError(t, d.runSteps(localCutSteps(d, g, p)))

	// PrepareSource ran on the source branch, not the new one.
	...
	require.NoError(t, err)
	require.Contains(t, newMeta, "CALICO_VERSION=v3.33.0", "the src bump must not land on the new branch")
	...
	// The empty "Begin development" commit on master's head carries the source edit.
	subject := gitHeadSubject(t, root, "master")
	require.Contains(t, subject, "Begin development")

	// master's dev tag sits on that commit.
...
// Skipping update-source leaves the source pin unbumped, while
// tag-source still tags master's head with an empty commit.
func TestFlowSkipUpdateSourceLeavesPinUnbumped(t *testing.T) {
	root := srcBumpRepo(t)
	var ranOn string
	...
	d, g := stepDriver(root, nil, false, map[string]bool{stepUpdateSource: true})
	d.PrepareDerived = branchEditPrepare(t, root, utils.ReleaseBranchPrefix()+"-v3.33-1")
	d.PrepareSource = bumpSourcePrepare(t, root, &ranOn)

	require.NoError(t, d.runSteps(localCutSteps(d, g, p)))

	// update-source was skipped, so PrepareSource never ran and the pin is unbumped.
	require.Empty(t, ranOn, "update-source must not run when skipped")
	masterMeta, err := command.GitInDir(root, "show", "master:metadata.mk")
	require.NoError(t, err)
	require.Contains(t, masterMeta, "CALICO_VERSION=v3.33.0", "skipping update-source must leave the source pin unbumped")

	// tag-source still tags master's head.
	tagged, err := command.GitInDir(root, "rev-parse", "v3.34.0-1.0-"+utils.DevTagSuffix()+"^{commit}")
	require.NoError(t, err)
	head, err := command.GitInDir(root, "rev-parse", "master")
	require.NoError(t, err)
...
// With no source tag target, update-source and tag-source no-op
// even when a PrepareSource hook is set.
func TestFlowNoSourceTargetSkipsSourceSteps(t *testing.T) {
	root := srcBumpRepo(t)
	var ranOn string
	...
	require.Empty(t, ranOn, "no source target must not run PrepareSource")
	masterMeta, err := command.GitInDir(root, "show", "master:metadata.mk")
	require.NoError(t, err)
	require.Contains(t, masterMeta, "CALICO_VERSION=v3.33.0", "no source target must leave the source pin unbumped")

	// No source dev tag is created.
	_, err = command.GitInDir(root, "rev-parse", "--verify", "--quiet", "refs/tags/v3.34.0-1.0-"+utils.DevTagSuffix())
	require.Error(t, err, "no source target must not create a source dev tag")
}

// A cut with no PrepareSource leaves the source metadata untouched.
func TestFlowNoPrepareSourceLeavesSourceUntouched(t *testing.T) {
	root := srcBumpRepo(t)
	...
	masterMeta, err := command.GitInDir(root, "show", "master:metadata.mk")
	require.NoError(t, err)
	require.Contains(t, masterMeta, "CALICO_VERSION=v3.33.0", "no PrepareSource must not bump the source CALICO_VERSION")
}
...
func TestPostPushStepIsLastSkippableAndPlanned(t *testing.T) {
	...
	require.NoError(t, planD.runSteps([]Step{findStep(planD.steps(planG, p), "post-push")}))
	...
	require.NoError(t, skipD.runSteps([]Step{findStep(skipD.steps(skipG, p), "post-push")}))
	require.Equal(t, 0, ran)

	// local run (Publish false): does not run its network hook.
	localD, localG := stepDriver(t.TempDir(), (&fakeRunner{}).fn(), false, nil)
	localD.PostDerivedPush = post
	require.NoError(t, localD.runSteps([]Step{findStep(localD.steps(localG, p), "post-push")}))
	require.Equal(t, 0, ran)

	// apply: runs the hook once.
...
func TestPushRefsCheckMissingLocalRefIsNotDone(t *testing.T) {
	...
	done, err := findStep(d.steps(g.fn(), p), "push-refs").Done()
...
func TestPostPushNilHookIsDone(t *testing.T) {
	p := &CutPlan{Derived: utils.ReleaseBranchPrefix() + "-v3.33", Remote: "origin"}
	d, g := stepDriver(t.TempDir(), (&fakeRunner{}).fn(), false, nil)
	post := findStep(d.steps(g, p), "post-push")
	done, err := post.Done()
	require.NoError(t, err)
