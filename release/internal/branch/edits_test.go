...
func TestApplyEditsCaptureGroupIdempotent(t *testing.T) {
	...
	// Second run: the expansion is already present, so nothing is written, but
	// the file is still staged for a resume to commit.
	written, _, err = ApplyEdits(root, edits)
	require.NoError(t, err)
	require.Equal(t, []string{"mock-node.yaml"}, written, "an already-applied edit must still be staged")
}
