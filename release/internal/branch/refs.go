...
// A failed `git rev-parse` means "does not exist", not an error.
func localBranchExists(g git, name string) (bool, error) {
	if _, err := g("rev-parse", "--verify", "--quiet", "refs/heads/"+name); err != nil {
		return false, nil
	}
	return true, nil
}
...
// stageAndCommit stages paths and commits with msg, returning whether a commit
// was made (false when nothing was staged).
func stageAndCommit(g git, msg string, paths []string) (bool, error) {
	...
	if _, err := g("diff", "--cached", "--quiet"); err == nil {
		// exit 0 = no staged changes; skip the commit so re-runs don't fail.
		return false, nil
	}
	if out, err := g("commit", "-m", msg); err != nil {
...
// tag reports false, not an error, so a resume knows to create it.
func localTagPointsAt(g git, tagName, branch string) (bool, error) {
	...
	return strings.TrimSpace(tagSha) == strings.TrimSpace(branchSha), nil
}
...
func tagWithEmptyCommit(g git, branch, tagName string) error {
	...
	if _, err := g("commit", "--allow-empty", "-m", "Begin development for "+tagName); err != nil {
		return fmt.Errorf("empty commit on %s: %w", branch, err)
	}
...
func remoteRefMatches(g git, remote, ref, localSha string) (present, matches bool, err error) {
	...
	remoteSha := out[:min(40, len(out))]
	return true, remoteSha == localSha, nil
}
...
func checkBranchCurrent(g git, branch, remote string, requireCurrent, report bool, recovery string) error {
	// An empty remote reads the branch's upstream from .git/config; with no
	// upstream there is nothing to compare, so the branch is current.
