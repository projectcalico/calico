...
func TestLocalBranchExists(t *testing.T) {
	...
	ok, err := localBranchExists(g.fn(), utils.ReleaseBranchPrefix()+"-v3.33-2")
	require.NoError(t, err)
	require.True(t, ok)

	// rev-parse errors -> not exists (not an error)
	g2 := &gitRec{errs: map[string]error{"rev-parse --verify": errFake}}
	ok2, err := localBranchExists(g2.fn(), "nope")
	require.NoError(t, err)
	require.False(t, ok2)
}
...
func TestLocalTagPointsAt(t *testing.T) {
	...
	ok, err := localTagPointsAt(g.fn(), "v3.33.0-"+utils.DevTagSuffix(), utils.ReleaseBranchPrefix()+"-v3.33")
	require.NoError(t, err)
	require.True(t, ok)

	// tag resolves to a different sha than the branch head -> not.
	g2 := &gitRec{out: map[string]string{tagRef: "abc\n", branchRef: "def\n"}}
	ok2, err := localTagPointsAt(g2.fn(), "v3.33.0-"+utils.DevTagSuffix(), utils.ReleaseBranchPrefix()+"-v3.33")
	require.NoError(t, err)
	require.False(t, ok2)

	// tag does not exist -> not an error, not pointing at.
	g3 := &gitRec{errs: map[string]error{tagRef: errFake}}
	ok3, err := localTagPointsAt(g3.fn(), "v3.33.0-"+utils.DevTagSuffix(), utils.ReleaseBranchPrefix()+"-v3.33")
	require.NoError(t, err)
	require.False(t, ok3)
}
...
func TestCheckBranchCurrent(t *testing.T) {
	...
	t.Run("empty remote, no configured upstream: current, no fetch", func(t *testing.T) {
		g := &gitScriptRec{errs: map[string]error{
			"rev-parse --abbrev-ref --symbolic-full-name master@{upstream}": errFake,
		}}
		require.NoError(t, checkBranchCurrent(g.fn(), "master", "", true, false, fix))
		require.False(t, g.ranPrefix("fetch"))
	})
