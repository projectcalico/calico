...
// epNextRelease returns the next release version for an EP version, or false
// for a non-EP version. It bumps EP1's patch and promotes EP2 to GA.
var epNextRelease = func(v *Version) (Version, bool) {
	ver := v.Semver()
	...
	if epVer == 1 {
		...
		return New(fmt.Sprintf("v%d.%d.0-1.%d", ver.Major(), ver.Minor(), minorEPver+1)), true
	}
	// EP2 -> GA
	return New(fmt.Sprintf("v%d.%d.1", ver.Major(), ver.Minor())), true
}
