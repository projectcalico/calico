		...
		if strings.Contains(err.Error(), "already exists") {
			if m := regexp.MustCompile(`(?:^|\s)(https://github\.com/[\w.-]+/[\w.-]+/pull/\d+)(?:$|\s)`).FindStringSubmatch(err.Error()); len(m) > 1 {
				if u, err := url.Parse(m[1]); err != nil {
					pr = fallbackURL
					logrus.WithField("PR", pr).Warn("PR already exists but could not parse URL from error, using fallback PR URL")
					...
					pr = fallbackURL
					logrus.WithField("PR", pr).Warn("PR already exists but error returned an unexpected URL, using fallback PR URL")
