...
func (r *CalicoManager) buildBinaries() error {
	// calicoctl is no longer produced as a side effect of an image build, so
	// build it unconditionally. The Linux cni-plugin is bundled into the
	// calico/calico image and no longer ships standalone binaries. felix
	// binaries are built as part of its image release-build target.
