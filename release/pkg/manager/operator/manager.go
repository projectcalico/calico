...
func (o *OperatorManager) modifyComponentsImagesFile() error {
	dest, err := os.OpenFile(componentImagesFilePath, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0o644)
	if err != nil {
		logrus.WithError(err).Errorf("Failed to open file %s", componentImagesFilePath)
		return err
	}
	defer dest.Close()
	tmpl, err := template.New("pkg/components/images.go").Parse(componentImagesFileTemplate)
	if err != nil {
		logrus.WithError(err).Errorf("Failed to parse template to overwrite %s file", componentImagesFilePath)
		return err
	}
	...
	if err := tmpl.Execute(dest, map[string]string{
		...
		logrus.WithError(err).Errorf("Failed to write to file %s", componentImagesFilePath)
		return err
	}
	return nil
}
