...
func PinnedVersion(cfg *config.Config) {
	...
	pinnedVersionFilePath, err := hashrelease.GeneratePinnedVersionFile(cfg.RepoRootDir, operatorDir, cfg.DevTagSuffix, outputDir)
	if err != nil {
...
func HashreleaseBuild(cfg *config.Config) {
	...
	if err := artifacts.ReleaseWindowsArchive(cfg.RepoRootDir, releaseVersion, hashreleaseOutputDir); err != nil {
		logrus.WithError(err).Fatal("Failed to release windows archive")
	}
	if err := artifacts.HelmArchive(cfg.RepoRootDir, releaseVersion, operatorVersion, hashreleaseOutputDir); err != nil {
		logrus.WithError(err).Fatal("Failed to release helm archive")
	}
	if err := artifacts.GenerateManifests(cfg.RepoRootDir, releaseVersion, operatorVersion, hashreleaseOutputDir); err != nil {
		logrus.WithError(err).Fatal("Failed to generate manifests")
	}

	if err := artifacts.GenerateReleaseInfo(hashreleaseOutputDir, releaseVersion, operatorVersion); err != nil {
