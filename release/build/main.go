...
var (
	...
	// Git owner(org) and repository name
	org          = "projectcalico"
	repo         = "calico"
	operatorOrg  = "tigera"
...
func hashreleaseSubCommands(cfg *config.Config) []*cli.Command {
	...
	return []*cli.Command{
		// The build command is used to produce a new local hashrelease in the output directory.
		{
			...
			Flags: []cli.Flag{
				&cli.StringFlag{Name: orgFlag, Usage: "Git organization", EnvVars: []string{"ORGANIZATION"}, Value: org},
				&cli.StringFlag{Name: repoFlag, Usage: "Git repository", EnvVars: []string{"GIT_REPO"}, Value: repo},
				...
				&cli.StringFlag{Name: operatorOrgFlag, Usage: "Operator git organization", EnvVars: []string{"OPERATOR_GIT_ORGANIZATION"}, Value: operatorOrg},
				&cli.StringFlag{Name: operatorRepoFlag, Usage: "Operator git repository", EnvVars: []string{"OPERATOR_GIT_REPO"}, Value: operatorRepo},
			...
			Action: func(c *cli.Context) error {
				...
				tasks.ReleaseNotes(cfg, c.String(orgFlag), filepath.Join(dir, releaseNotesDir), versions.ProductVersion)
	...
	return []*cli.Command{
		// Build release notes prior to a release.
		{
			...
			Action: func(c *cli.Context) error {
				...
				tasks.ReleaseNotes(cfg, c.String(orgFlag), filepath.Join(cfg.RepoRootDir, releaseNotesDir), ver)
				return nil
			},
		},
