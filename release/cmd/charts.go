...
var chartsBuildAction = func(cfg *Config) func(context.Context, *cli.Command) error {
	return func(_ context.Context, c *cli.Command) error {
		...
		if err != nil {
			...
			return fmt.Errorf("no charts to publish")
		}
		opts, err := chartsBuildOptions(cfg, c, *chart, pinForBuild, c.Bool(helmIndexFlagName))
		if err != nil {
			return err
		}
...
var pinnedChart = func(cfg *Config, c *cli.Command, pin pinned) (*charts.Chart, error) {
	if c.Bool(hashreleaseFlag.Name) {
		...
		return &charts.Chart{
			...
			Names:          charts.All(),
			BaseDir:        filepath.Join(cfg.OutputDir, p.HelmChartVersion()),
		}, nil
	}
