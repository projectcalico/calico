		...
		// The build command is used to produce a new local hashrelease in the output directory.
		{
			...
			Action: func(_ context.Context, c *cli.Command) error {
				...
				if c.String(orgFlag.Name) == utils.TigeraOrg {
					...
					if _, err := outputs.ReleaseNotes(utils.ProjectCalicoOrg, c.String(githubTokenFlag.Name), cfg.RepoRootDir, filepath.Join(hashreleaseDir, outputs.ReleaseNotesDir), releaseVersion); err != nil {
						return err
					}
				}
