...
This is a bug fix release, the major fixes and enhancements include:
- Fix: some policies not working because of / in the labels.
- Fix that ipv4_nat was not working (for OpenStack floating IPs)
...
- Support for ipPools with k8s backend.
- Global init process for backends.
- Add support for policing forwarded traffic via host endpoints.
- Add configuration option to append or insert the iptables rules generated by calico-felix.
- Add configuration option to configure default log prefix.
- Fix: NoneType exception in kubernetes policy controller.
- Fix: kubernetes addons like kube-dns or dashboard not being able to talk to the API server on the same node sometimes.
