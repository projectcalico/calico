...
make fv-bpf FV_NUM_BATCHES=4

# Run a specific test by name (use dots for spaces in regex)
make fv-bpf FV_NUM_BATCHES=4 GINKGO_FOCUS="no.connectivity.to.a.pod"
```

Note: `FV_NUM_BATCHES` must be >= 2 due to the manual sharding implementation.
