			...
			if testOpts.protocol != "udp" { // No need to run these tests per-protocol.
				...
				It("should recover if the BPF programs are removed", func() {
					...
					for _, trigger := range []func(){flapInterface, recreateWEP} {
						...
						progNameFrom := "calico_from_wor"
						progNameTo := "calico_to_workl"
						if testOpts.protocol == "tcp" && testOpts.bpfLogLevel == "debug" {
							progNameTo = "calico_log_filt"
							progNameFrom = "calico_log_filt"
						}
						...
						Eventually(func() string {
							...
							return out
						}, "5s", "200ms").Should(ContainSubstring(progNameFrom),
						...
						Eventually(func() string {
							...
							return out
						}, "5s", "200ms").Should(ContainSubstring(progNameTo),
...
func bpfWaitForPolicyNotExists(felix *infrastructure.Felix, iface, hook, policy string) string {
	search := fmt.Sprintf("Start of policy %s", policy)
	out := ""
	EventuallyWithOffset(1, func() string {
		out = bpfDumpPolicy(felix, iface, hook)
		return out
	}, "5s", "200ms").ShouldNot(ContainSubstring(search))
