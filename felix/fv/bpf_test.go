			...
			Context("with a policy allowing ingress to w[0][0] from all regular workloads", func() {
				...
				Describe("Test advertised IP's with maglev enabled", func() {
					...
					BeforeEach(func() {
						...
						checkEndpointsForSvc(k8sClient, testSvc, 1)
				...
				Context("with test-service configured "+clusterIP+":80 -> w[0][0].IP:8055", func() {
					...
					Context("with test-service port updated", func() {
						...
						BeforeEach(func() {
							...
							Expect(err).NotTo(HaveOccurred())
							checkEndpointsForSvc(k8sClient, testSvcUpdated, 1)
						})
						...
						It("after removing service, should not have connectivity from workloads via a service to workload 0", func() {
							...
							Expect(err).NotTo(HaveOccurred())
							checkEndpointsForSvc(k8sClient, testSvc, 0)
					...
					if !testOpts.connTimeEnabled && testOpts.tunnel == "none" &&
						...
						Context("with small MTU between remote client and cluster", func() {
							...
							BeforeEach(func() {
								...
								Expect(err).NotTo(HaveOccurred())
								checkEndpointsForSvc(k8sClient, svcHostNP, 1)
...
func checkEndpointsForSvc(k8sClient kubernetes.Interface, svc *v1.Service, expectedLen int) {
	Eventually(func() int {
		slices := k8sGetEpsForServiceFunc(k8sClient, svc)()
		cnt := 0
		...
		return cnt
	}, "10s").Should(Equal(expectedLen), "Service endpoints didn't get created? Is controller-manager happy?")
}
