...
type Checker struct {
	...
	// a new one.
	//
	// An expectation given a port by ExpectWithSrcPort keeps it, and so keeps
	// repeating its 5-tuple on every call - the behaviour this option exists to
	// avoid.  Pin a port only where the test needs that exact port, and expect no
	// per-call freshness for that one connection.  A pinned port must also sit
	// outside the range assigned here, or the two could hand the same port to two
	// different connections; that is checked below.
