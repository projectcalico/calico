	...
	Context("with XDP blacklist on felix[1] blocking felixes[0]", func() {
		...
		Context("changing GlobalNetworkSets", func() {
			...
			It("should be reflected in the BPF map", func() {
				getBlacklistFn := func(key []string) func() string {
					return func() string {
						utils.Run("docker", append([]string{"exec", felixes[1].Name, "bpftool", "map", "lookup", "pinned", "/sys/fs/bpf/calico/xdp/eth0_ipv4_v1_blacklist", "key", "hex"}, key...)...)
						return utils.LastRunOutput
					}
				}
				Eventually(getBlacklistFn(host0HexCIDR), "10s").Should(ContainSubstring("value:"))
				...
				Eventually(getBlacklistFn(AdditionalHostUDPHexCIDR), "10s").Should(ContainSubstring("value:"))
				Eventually(getBlacklistFn(AdditionalHostTCPHexCIDR), "10s").Should(ContainSubstring("value:"))
			})
		})
