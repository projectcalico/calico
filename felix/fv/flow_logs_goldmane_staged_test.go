...
var _ = infrastructure.DatastoreDescribe("_BPF-SAFE_ FOCUS goldmane flow log with staged policy tests", []apiconfig.DatastoreType{apiconfig.Kubernetes}, func(getInfra infrastructure.InfraFactory) {
	const (
		wepPort = 8055
		svcPort = 8066
	)
	...
	BeforeEach(func() {
		...
		if !bpfEnabled {
			...
			Eventually(getRuleFunc(tc.Felixes[0], "APE0|GlobalNetworkPolicy/default.ep1-1-allow-all"), "10s", "1s").ShouldNot(HaveOccurred())
			Eventually(getRuleFunc(tc.Felixes[1], "APE0|GlobalNetworkPolicy/default.ep1-1-allow-all"), "10s", "1s").Should(HaveOccurred())
...
var _ = infrastructure.DatastoreDescribe("_BPF-SAFE_ goldmane flow log with staged policies with pending policy tests", []apiconfig.DatastoreType{apiconfig.Kubernetes}, func(getInfra infrastructure.InfraFactory) {
	...
	BeforeEach(func() {
		...
		if !bpfEnabled {
			Eventually(getRuleFunc(tc.Felixes[0], "PPI0|NetworkPolicy/default/tier1.np1-1"), "10s", "1s").ShouldNot(HaveOccurred())
			Eventually(getRuleFunc(tc.Felixes[0], "PPE0|NetworkPolicy/default/tier1.np1-1"), "10s", "1s").ShouldNot(HaveOccurred())
			Eventually(getRuleFunc(tc.Felixes[1], "PPI0|NetworkPolicy/default/tier1.np1-1"), "10s", "1s").ShouldNot(HaveOccurred())
			Eventually(getRuleFunc(tc.Felixes[1], "PPE0|NetworkPolicy/default/tier1.np1-1"), "10s", "1s").ShouldNot(HaveOccurred())
			// When policies are programmed, make sure no staged policy is programmed. Staged policies must be skipped.
			Consistently(getRuleFunc(tc.Felixes[0], "staged"), "5s", "1s").Should(HaveOccurred())
			Consistently(getRuleFunc(tc.Felixes[1], "staged"), "5s", "1s").Should(HaveOccurred())
		} else {
			bpfWaitForPolicy(tc.Felixes[0], ep1_1.InterfaceName, "egress", "NetworkPolicy/default/tier1.np1-1")
			bpfWaitForPolicy(tc.Felixes[0], ep1_1.InterfaceName, "ingress", "NetworkPolicy/default/tier1.np1-1")
			bpfWaitForPolicy(tc.Felixes[1], ep2_1.InterfaceName, "egress", "NetworkPolicy/default/tier1.np1-1")
			bpfWaitForPolicy(tc.Felixes[1], ep2_1.InterfaceName, "ingress", "NetworkPolicy/default/tier1.np1-1")
			// When policies are programmed, make sure no staged policy is programmed. Staged policies must be skipped.
			Consistently(bpfDumpPolicy(tc.Felixes[0], ep1_1.InterfaceName, "ingress"), "5s", "1s").ShouldNot(ContainSubstring("staged"))
			Consistently(bpfDumpPolicy(tc.Felixes[0], ep1_1.InterfaceName, "egress"), "5s", "1s").ShouldNot(ContainSubstring("staged"))
			Consistently(bpfDumpPolicy(tc.Felixes[1], ep2_1.InterfaceName, "ingress"), "5s", "1s").ShouldNot(ContainSubstring("staged"))
	...
	configureStagedAllow := func() {
		...
		if !bpfEnabled {
			...
			bpfWaitForPolicy(tc.Felixes[0], ep1_1.InterfaceName,
				"egress", "NetworkPolicy/default/tier1.np1-1")
			bpfWaitForPolicy(tc.Felixes[0], ep1_1.InterfaceName,
				"ingress", "NetworkPolicy/default/tier1.np1-1")
			bpfWaitForPolicy(tc.Felixes[1], ep2_1.InterfaceName,
				"egress", "NetworkPolicy/default/tier1.np1-1")
			bpfWaitForPolicy(tc.Felixes[1], ep2_1.InterfaceName,
				"ingress", "NetworkPolicy/default/tier1.np1-1")
