...
var _ = infrastructure.DatastoreDescribe(
	...
	func(getInfra infrastructure.InfraFactory) {
		...
		JustAfterEach(func() {
			...
			// repinJumpMaps() preserves the previous Felix's maps under
			// old_jumps/<tmp>/ — dump those too so we can see whether old
			// preambles' tail-call targets still have entries.
			felix.Exec("sh", "-c",
		...
		It("should keep host SSH reachable after the Node is deleted", func() {
			...
			// The delete is synchronous from the test's POV; Felix's
			// reaction (HostMetadataRemove → re-attach with HOST_IP=0) is
			// asynchronous. Consistently with a fast probe catches any
			// drop during the reaction window. 10s is plenty: the apply
			// pass that does the re-attach lands within ~1-2s of the
			// delete event.
