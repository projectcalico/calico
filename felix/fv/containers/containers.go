...
func (c *Container) ExecOutput(args ...string) (string, error) {
	...
	if err != nil {
		...
		if len(out) == 0 {
			return "", fmt.Errorf("command failed with no output %q: %w stderr=%q", cmd, err, stderrStr)
		}
		stdoutStr := string(out)
		return stdoutStr, fmt.Errorf("command failed %q: %w output=%q stderr=%q", cmd, err, stdoutStr, stderrStr)
	}
