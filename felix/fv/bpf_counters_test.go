...
var _ = infrastructure.DatastoreDescribe("_BPF-SAFE_ Felix bpf test counters", []apiconfig.DatastoreType{apiconfig.EtcdV3}, func(getInfra infrastructure.InfraFactory) {
	...
	It("should update generic counters", func() {
		...
		pol.Namespace = "fv"
		pol.Name = "drop-policy"
		pol.Spec.Selector = "all()"
		pol.Spec.Ingress = []api.Rule{{
			Action: "Deny",
			Source: api.EntityRule{
				Selector: fmt.Sprintf("name=='%v'", w[0].Name),
			},
			Destination: api.EntityRule{
				Selector: fmt.Sprintf("name=='%v'", w[1].Name),
			},
		}}
		...
		checkDroppedByPolicyCounters(felixes[0], w[0].InterfaceName, 0, 0)
		...
		checkDroppedByPolicyCounters(felixes[0], w[0].InterfaceName, numberOfpackets, 0)
	})
