...
func (kds *K8sDatastoreInfra) AddNode(felix *Felix, v4CIDR *net.IPNet, v6CIDR *net.IPNet, idx int, needBGP bool) {
	...
	if len(felix.IPv6) > 0 && v6CIDR != nil {
		...
		ipv6CIDR := fmt.Sprintf("%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%04x::/96", v6CIDR.IP[0], v6CIDR.IP[1], v6CIDR.IP[2], v6CIDR.IP[3], v6CIDR.IP[4], v6CIDR.IP[5], v6CIDR.IP[6], v6CIDR.IP[7], v6CIDR.IP[8], v6CIDR.IP[9], idx)
		_, n, err := net.ParseCIDR(ipv6CIDR)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		nodeIn.Spec.PodCIDRs = append(nodeIn.Spec.PodCIDRs, n.String())
