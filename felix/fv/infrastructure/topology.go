...
func StartNNodeTopology(n int, opts TopologyOptions, infra DatastoreInfra) (felixes []*Felix, client client.Interface) {
	...
	for i := 0; i < n; i++ {
		...
		configureTunnelIPInEtcdModeIgnoredInK8s := n > 1 || opts.NeedNodeIP
		if opts.IPIPEnabled {
			infra.SetExpectedIPIPTunnelAddr(felix, i, configureTunnelIPInEtcdModeIgnoredInK8s)
		...
		if !opts.DelayFelixStart && felix.ExpectedIPIPTunnelAddr != "" {
			...
			log.Info("Waiting for felix to log that it has the host IP.")
			// The exact NodeIP depends on various options, just wait for Felix to log any non-empty value.
			w = felix.WatchStdoutFor(regexp.MustCompile(
				`Successfully loaded configuration.*"NodeIP":"[^"]+"`))
		}
		infra.AddNode(felix, i, configureTunnelIPInEtcdModeIgnoredInK8s)
		if w != nil {
