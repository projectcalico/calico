...
func main() {
	...
	tostr := arguments["--timeout"].(string)
	timeout, err := strconv.Atoi(tostr)
...
func (tc *testConn) tryLoopFile(loopFile string, logPongs bool, timeout time.Duration) error {
	...
	for {
		...
		if timeout > 0 {
			tc.protocol.SetReadDeadline(time.Now().Add(200 * time.Millisecond))
	...
	go func() {
		...
		for {
			...
			default:
				...
				tc.protocol.SetReadDeadline(time.Now().Add(100 * time.Millisecond))
				respRaw, err := tc.protocol.Receive()
