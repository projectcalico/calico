...
/* Whether a wrong-interface packet is denied rather than merely accepted with a
 * distrusted ingress record.
 */
static CALI_BPF_INLINE bool rpf_strict_enforced(struct cali_tc_ctx *ctx)
{
	return (GLOBAL_FLAGS & CALI_GLOBALS_RPF_OPTION_ENABLED) &&
		(GLOBAL_FLAGS & CALI_GLOBALS_RPF_OPTION_STRICT);
}
