...
enum cali_globals_flags {
	...
	/* CALI_GLOBALS_IPIP_NO_OUTER_HEADER is set for kernels in which IPIP tunnels acts
	 * like other l3 devices, where only inner ip header is seen by bpf programs. This
	 * behaviour started in kernel 5.14. Before 5.14, bpf programs attached to IPIP tunnel
	 * saw ethernet, outer ip header, inner ip header and payload */
