...
skip_redir_ifindex:
		...
		if (cali_rt_flags_local_workload(dest_rt->flags)) {
			 ...
			 * truncating the key before local_ipv4/local_ipv6.  bpf_skb_set_tunnel_key
			 * then omits it and the kernel selects the source by routing to the remote
			 * VTEP, i.e. the node's real underlay IP.  The overlay tunnel-device IP is
			 * not underlay-routable and must never be the outer source: some fabrics
			 * (e.g. GCP anti-spoof) drop packets sourced from it. */
