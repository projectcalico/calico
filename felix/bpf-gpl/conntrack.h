...
static CALI_BPF_INLINE struct calico_ct_result calico_ct_lookup(struct cali_tc_ctx *ctx)
{
	 ...
	 * lives on the tracking entry, which for a NAT_FWD hit is the reverse
	 * entry rather than the one we looked up. src_to_dst/dst_to_src are
	 * redirected there below; bk is the same redirection for the fields that
	 * hang off the value itself.
	 */
	struct calico_ct_value *bk = v;
	...
	if (tcp_header && !related) {
		...
		if (tcp_header->rst) {
			...
			if (now - bk->rst_seen > 2 * 60 * 1000000000ull || now - bk->rst_seen > (1ull << 63)) {
		...
		if ((src_to_dst->fin_seen && dst_to_src->fin_seen) || tcp_header->rst) {
			qos_connlimit_decrement_for_ct(bk);
		}
	}
	...
	if (syn) {
		...
		if (CALI_F_TO_WEP && INGRESS_CONN_LIMIT_CONFIGURED) {
			__u32 cl = ct_value_get_flags(bk);
