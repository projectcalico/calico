 ...
 */
static CALI_BPF_INLINE void ct_leg_reset_kind(struct cali_tc_ctx *ctx,
					      struct calico_ct_leg *leg, __u32 also_clear)
{
...
static CALI_BPF_INLINE struct calico_ct_result calico_ct_lookup(struct cali_tc_ctx *ctx)
{
		...
		if (CALI_F_TO_HOST) {
			...
			case RPF_RES_STRICT:
				if (!related) {
					 ...
					 */
					ct_leg_reset_kind(ctx, src_to_dst, CALI_CT_LEG_PINNED |
							CALI_CT_LEG_CHECKED);
				}
				break;
	...
	if (CALI_F_TO_HOST) {
		...
		if (!related && src_to_dst->ifindex == ifindex) {
			ct_leg_reset_kind(ctx, src_to_dst, 0);
		}
