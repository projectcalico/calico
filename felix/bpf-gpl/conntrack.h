 ...
 * also_clear at the same time. CALI_F_TUNNEL is compile-time, so this is a
 * single atomic in every program that is not a tunnel program.
 */
static CALI_BPF_INLINE void ct_leg_reset_kind(struct calico_ct_leg *leg, __u32 also_clear)
{
	if (CALI_F_TUNNEL) {
		ct_leg_clear_flags(leg, also_clear);
		ct_leg_set_flags(leg, CALI_CT_LEG_TUNNEL);
	} else {
		ct_leg_clear_flags(leg, also_clear | CALI_CT_LEG_TUNNEL);
	}
}
...
static CALI_BPF_INLINE struct calico_ct_result calico_ct_lookup(struct cali_tc_ctx *ctx)
{
	...
	if (CALI_F_TO_HOST) {
		...
		if (!related && src_to_dst->ifindex == ifindex &&
				ct_leg_flag(src_to_dst, CALI_CT_LEG_TUNNEL) != (CALI_F_TUNNEL ? 1 : 0)) {
