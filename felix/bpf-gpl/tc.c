...
syn_force_policy:
	...
	if (CALI_F_FROM_WEP
		...
		if (r->flags & CALI_RT_NAT_OUT) {
			...
			if (rt_flags_should_perform_nat_outgoing(cali_rt_lookup_flags(&ctx->state->post_nat_ip_dst), exclude_hosts)) {
		...
		if (cali_rt_flags_is_in_pool(r->flags) && !(rt_addr_is_local_host_or_in_pool(&ctx->state->post_nat_ip_dst))) {
	...
	if ((CALI_F_TO_HEP) && (rt_addr_is_local_host(&ctx->state->ip_src)) &&
		!(cali_rt_flags_is_in_pool(cali_rt_lookup_flags(&ctx->state->post_nat_ip_dst)))) {
		CALI_DEBUG("Remote host or outside cluster dest " IP_FMT "", debug_ip(ctx->state->post_nat_ip_dst));
		ctx->state->flags |= CALI_ST_CLUSTER_EXTERNAL;
	}
	if ((CALI_F_FROM_HEP) && (!cali_rt_flags_is_in_pool(cali_rt_lookup_flags(&ctx->state->ip_src))) &&
		(rt_addr_is_local_host_or_in_pool(&ctx->state->post_nat_ip_dst))) {
