...
// ourRouteToKernelRoute converts (only) routes that we own back
// to our kernelRoute/Key structs (returning ok=true).  Other routes
// are ignored and returned with ok = false.
func (r *RouteTable) ourRouteToKernelRoute(route *netlink.Route) (kernKey kernelRouteKey, kernRoute kernelRoute, ok bool) {
	ok = r.routeIsOurs(route)
	if !ok {
		return
	}

	kernKey, kernRoute = r.netlinkRouteToKernelRoute(route)
