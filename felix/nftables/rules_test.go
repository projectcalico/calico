...
func renderRule(rule generictables.Rule, features *environment.Features) *knftables.Rule {
	return generictables.NewNFTRenderer("cali:", false).Render("test", "TEST", rule, features)
}
...
func calculateHashes(chainName string, rules []generictables.Rule) []string {
	...
	return generictables.NewNFTRenderer("cali:", false).RuleHashes(chain, &environment.Features{})
}
