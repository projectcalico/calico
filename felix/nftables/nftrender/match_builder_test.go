...
var _ = Describe("MatchBuilder failure cases", func() {
	...
	It("should panic if ConnMarkMatchesWithMask is passed an invalid mark", func() {
		Expect(func() {
			nftrender.Match().ConnMarkMatchesWithMask(0xf, 0x1)
		}).To(Panic())
	})
	It("should panic if ConnMarkMatchesWithMask is passed a 0 mask", func() {
		Expect(func() {
			nftrender.Match().ConnMarkMatchesWithMask(0x0, 0x0)
		}).To(Panic())
	})
})
...
var _ = DescribeTable("MatchBuilder",
	...
	Entry("ConnMarkMatchesWithMask", nftrender.Match().ConnMarkMatchesWithMask(0x400a, 0xf00f), "ct mark & 0xf00f == 0x400a"),

	// Conntrack.
	Entry("ConntrackState", nftrender.Match().ConntrackState("INVALID"), "ct state invalid"),

	// TCP flags.
	Entry("TCPFlagsSet", nftrender.Match().TCPFlagsSet("RST"), "tcp flags & rst == rst"),
