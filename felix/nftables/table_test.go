	...
	Describe("after inserting a rule", func() {
		...
		Describe("after inserting a rule then updating the insertions", func() {
			...
			It("should update the dataplane", func() {
				rules, err := f.ListRules(context.TODO(), "filter-FORWARD")
				Expect(err).NotTo(HaveOccurred())
				Expect(rules).To(nftables.EqualRules([]knftables.Rule{
				...
				Describe("then extending the chain", func() {
					...
					Describe("then replacing the chain", func() {
						...
						It("should be updated", func() {
							rules, err := f.ListRules(context.TODO(), "cali-foobar")
							Expect(err).NotTo(HaveOccurred())
							Expect(rules).To(nftables.EqualRulesFuzzy([]knftables.Rule{
...
var _ = Describe("Table with flowtable offload enabled", func() {
	...
	// Enabling offload with an empty device set must still create the flowtable. The static
	// FORWARD chain references "flow offload @calico" unconditionally whenever offload is on, so
	// the flowtable has to exist for that rule to load — and an empty flowtable is valid. The
	// previous behavior deleted the flowtable when there were no devices, which failed the whole
	// atomic transaction (delete-of-nonexistent) on a node with no overlay devices and no local
	// workloads.
