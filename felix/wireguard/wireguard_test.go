...
var (
	...
	routekey_cidr_local_throwV6 = fmt.Sprintf("%d-%d-%s", tableIndex, 1, cidrV6_local)
	//routekey_1_throwV6 = fmt.Sprintf("%d-%d-%s", tableIndex, 1, cidrV6_1)
	//routekey_2_throwV6 = fmt.Sprintf("%d-%d-%s", tableIndex, 1, cidrV6_2)
	routekey_3_throwV6 = fmt.Sprintf("%d-%d-%s", tableIndex, 1, cidrV6_3)
	routekey_4_throwV6 = fmt.Sprintf("%d-%d-%s", tableIndex, 1, cidrV6_4)
)
		...
		Describe("create the wireguard link", func() {
			...
			It("should handle status update raising an error", func() {
				...
				if enableV6 {
					...
					sV6.err = errors.New("foobarbaz")
					err := wgV6.Apply()
					Expect(err).To(HaveOccurred())
					Expect(err).To(Equal(sV6.err))
				}
			})
			...
			Describe("set the link up", func() {
				...
				It("should create wireguard client and create private key", func() {
					...
					if enableV6 {
						...
						Expect(link.WireguardPrivateKey).NotTo(Equal(zeroKey))
						Expect(link.WireguardPrivateKey.PublicKey()).To(Equal(link.WireguardPublicKey))
						Expect(sV6.numCallbacks).To(Equal(1))
						Expect(sV6.key).To(Equal(link.WireguardPublicKey))
					}
				})
				...
				It("after endpoint update with incorrect key should program the interface address and resend same key as status", func() {
					...
					if enableV6 {
						...
						Expect(sV6.numCallbacks).To(Equal(1))
						key := link.WireguardPrivateKey
						Expect(sV6.key).To(Equal(key.PublicKey()))
						...
						Expect(link.WireguardPrivateKey).To(Equal(key))
						Expect(link.WireguardPrivateKey.PublicKey()).To(Equal(link.WireguardPublicKey))
						Expect(sV6.numCallbacks).To(Equal(2))
						Expect(sV6.key).To(Equal(key.PublicKey()))
					}
				})
				...
				Describe("create two wireguard nodes with different public keys", func() {
					...
					BeforeEach(func() {
						...
						if enableV6 {
							Expect(sV6.numCallbacks).To(Equal(1))
							wgV6.EndpointWireguardUpdate(hostname, sV6.key, nil)
					...
					It("should contain a throw route for the local CIDR", func() {
						...
						if enableV6 {
							...
							Expect(rtDataplaneV6.AddedRouteKeys).To(HaveKey(routekey_cidr_local_throwV6))
							Expect(rtDataplaneV6.AddedRouteKeys).To(HaveLen(1))
						...
						Describe("create destinations on each peer", func() {
							...
							BeforeEach(func() {
								...
								if enableV6 {
									...
									routekeyV6_1 = fmt.Sprintf("%d-%d-%s", tableIndex, linkV6.LinkAttrs.Index, cidrV6_1)
									routekeyV6_2 = fmt.Sprintf("%d-%d-%s", tableIndex, linkV6.LinkAttrs.Index, cidrV6_2)
									routekeyV6_3 = fmt.Sprintf("%d-%d-%s", tableIndex, linkV6.LinkAttrs.Index, cidrV6_3)
							...
							It("should route to wireguard for peer1 and peer2 routes, but not peer3 routes", func() {
								...
								if enableV6 {
									...
									Expect(rtDataplaneV6.AddedRouteKeys).To(HaveKey(routekey_4_throwV6))
									...
									Expect(rtDataplaneV6.RouteKeyToRoute[routekey_4_throwV6]).To(Equal(netlink.Route{
										LinkIndex: 1,
										Dst:       &ipnetV6_4,
										Type:      syscall.RTN_THROW,
										Protocol:  FelixRouteProtocol,
							...
							It("should handle deletion of nodes 2 and 3", func() {
								...
								if enableV6 {
									...
									Expect(rtDataplaneV6.AddedRouteKeys).NotTo(HaveKey(routekey_3_throwV6))
									Expect(rtDataplaneV6.AddedRouteKeys).NotTo(HaveKey(routekey_4_throwV6))
									Expect(wgDataplaneV6.WireguardConfigUpdated).To(BeTrue())
									Expect(linkV6.WireguardPeers).To(HaveKey(keyV6_peer1))
							...
							Describe("move a route from peer1 to peer2 and a route from peer2 to peer3", func() {
								...
								It("should reprogram the route to the non-wireguard peer only", func() {
									...
									if enableV6 {
										...
										Expect(rtDataplaneV6.AddedRouteKeys).To(HaveKey(routekey_3_throwV6))
										Expect(rtDataplaneV6.RouteKeyToRoute[routekey_3_throwV6]).To(Equal(netlink.Route{
											LinkIndex: 1,
											Dst:       &ipnetV6_3,
											Type:      syscall.RTN_THROW,
											Protocol:  FelixRouteProtocol,
							...
							Describe("enable wireguard on peer 3", func() {
								...
								It("should reprogram the route to peer3 only", func() {
									...
									if enableV6 {
										routekeyV6_4 := fmt.Sprintf("%d-%d-%s", tableIndex, linkV6.LinkAttrs.Index, cidrV6_4)
										Expect(rtDataplaneV6.AddedRouteKeys).To(HaveLen(1))
										Expect(rtDataplaneV6.DeletedRouteKeys).To(HaveLen(1))
										Expect(rtDataplaneV6.DeletedRouteKeys).To(HaveKey(routekey_4_throwV6))
			...
			Describe(desc, func() {
				...
				It("should update on resync", func() {
					...
					if enableV6 {
						...
						// Expect a zero key status update.
						Expect(sV6.key).To(Equal(zeroKey))
						Expect(sV6.numCallbacks).To(Equal(1))
						...
						// Expect an updated public key and the previously failed client to have been re-requested.
						Expect(sV6.key).NotTo(Equal(zeroKey))
						Expect(sV6.numCallbacks).To(Equal(2))
						if failFlags&mocknetlink.FailNextNewWireguardNotSupported != 0 {
							// And if emulating the wireguard failure, we expect a call to that too.
							Expect(wgDataplaneV6.NumNewWireguardCalls).To(Equal(1))
