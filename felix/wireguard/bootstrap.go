...
func FilterTyphaEndpoints(
	...
	if peersToValidate == nil || peersToValidate.Len() == 0 {
	...
	for _, typha := range typhas {
		...
		if typhaNodeName == configParams.FelixHostname {
			...
			logCxt.Infof("Typha endpoint %s is local - include typha endpoint", typha.Addr)
			filtered = append(filtered, typha)
			continue
		}
		...
		if err != nil {
			logCxt.WithError(err).Infof("Unable to determine public key for node %s", typhaNodeName)
			return nil, err
		}

		if typhaNodeKey == "" {
			// There is no key configured, so perhaps this node has wireguard disabled. Include this typha.
			logCxt.Infof("Typha node %s does not have a wireguard key - include typha endpoint", typhaNodeName)
			filtered = append(filtered, typha)
			...
			logCxt.Infof("Typha node %s has a wireguard key that is in the local wireguard routing table - include typha endpoint", typhaNodeName)
			filtered = append(filtered, typha)
		} else {
			// The public key on the typha node is not configured in the local routing table. There is no point in
			// including this typha because routing will not work.
			logCxt.Warningf("Typha node %s has a wireguard key that is not in the local wireguard routing table - exclude typha endpoint", typhaNodeName)
		}
	}
