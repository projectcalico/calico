...
const (
	bootstrapBackoffDuration  = 2 * time.Second
	bootstrapBackoffExpFactor = 2
	bootstrapBackoffMax       = 32 * time.Second
	bootstrapJitter           = 0.2
...
func BootstrapHostConnectivity(
	...
	if err != nil {
		logCtx.Info("Couldn't acquire WireGuard handle, remove any wiregruad configuration")
		return RemoveWireguardForHostEncryptionBootstrapping(configParams, getNetlinkHandle, calicoClient)
	}
	defer func() {
		err = wg.Close()
