...
BPFMapSizePerCPUConntrack determines the size of connectrack map based on the number of CPUs. If set it to a non-zero value, BPFMapSizeConntrack will be override as BPFMapSizePerCPUConntrack * (Number of CPUs). This map must be large enough to hold an entry for each active connection. Warning: changing the size of the conntrack map can cause disruption.
