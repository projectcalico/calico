...
# (re)generate the IPv6 BGP peer protocols into that shared include directory
...
PEER_CONF=$CONF_DIR/ipv6-calico-mesh.conf
...
# Generate the IPv6 peering config into the shared include directory.
mkdir -p $CONF_DIR
> $PEER_CONF
