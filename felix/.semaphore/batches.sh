...
run_batch() {
  ...
  while true; do
    VM_NAME="$vm_name" ${remote_exec} tail -F -n 0 "nohup.out" >> "$log_file" < /dev/null || true
  ...
  while true; do
    local rc
    rc=$(VM_NAME="$vm_name" ${remote_exec} bash -c 'while [ ! -f /tmp/test-rc ]; do sleep 1; done; cat /tmp/test-rc' < /dev/null)
