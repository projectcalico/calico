...
container). The deadlock only manifests in bootstrap mode because
that is the only configuration where Felix routes itself through the
ClusterIP.
...
This is the same defensive shape as #12192 — don't let a transient,
incomplete view erase a pre-existing NAT entry that traffic (here,
Felix's own) depends on.
