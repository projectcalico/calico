...
`TCPResetSeen` and the next recount rebases, which is why that timeout
tracks `nf_conntrack_tcp_timeout_close` rather than being generous.
...
`calico_ct_lookup` records an RST only if its sequence number is the
sender's `snd_nxt`, which is what the peer's own stack demands (RFC
5961), so an out-of-window RST changes nothing. `calico_ct_leg.seqno`
carries the ISN until both legs are established and that leg's
`snd_nxt` afterwards. Advancing it has to count every byte the sender
sent, aggregated segments included: a `snd_nxt` that lags fails a
connection's *own* closing RST, which then holds its slot until
`TCPEstablished` instead of `TCPResetSeen`.
...
- `CONNLIMIT_DEC` is an idempotence latch, **not** a statement that
  ...
  nor on any other marker a pod can set for itself: the recount is
  the only path that can return a slot, so whatever it skips is a
  place to park live connections out of sight.
