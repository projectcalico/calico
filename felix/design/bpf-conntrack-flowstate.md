...
- **`CALI_CT_FLAG_OVERLAY_REPLY` (normal entries).** With
  ...
  *not* tunneled — and would leave the node un-encapsulated with a
  ...
  The flag is set at ingress on the host→workload flow (VXLAN: outer
  source present in `state->tun_ip`; IPIP: the `CALI_F_IPIP_ENCAPPED`
  hook) and read on the reply's workload-egress path, where it forces
  the overlay encap even though the destination route is a bare host.
  It carries no address: a remote-host route already holds the node
  IP as its `next_hop`, which is the tunnel endpoint. This is
  deliberately *not* `tun_ip` — that field's meaning is NodePort/
  service-specific and must stay so.
