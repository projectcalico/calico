...
Code outside the BPF endpoint manager must resolve the option the
same way, through `tc.ResolveAttachType`, rather than compare the
configured value against a mechanism. Bandwidth QoS is the case
that matters: it is only supported where the workload devices are
TCX-driven (`isQoSBandwidthSupported` in `qos_controls.go`), which
`Netkit` satisfies for every device that is not a netkit device.
Comparing the raw option against `TCX` there silently programs no
QoS qdisc at all.
...
**Migrate onto TCX, not TC.** Both netkit attachment and TCX
leave the device's own qdisc space free — `ensureQdisc` is a
no-op under TCX — which is what QoS needs: the bandwidth qdiscs
claim the root and the `ffff:` ingress handle, and BPF QoS is
keyed off that same device. Legacy TC puts clsact on `ffff:` and
takes that space, so QoS controls stop working on the devices
that moved. TCX is always available where netkit is (netkit
needs kernel 6.7, TCX 6.6), so this costs nothing to honour:
`BPFAttachType: TCX` is the migration target, and `TC` is only
for kernels too old to have netkit devices in the first place.
