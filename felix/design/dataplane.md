...
name: a new manager or driver, or a change to the manager/driver
split; a change to the `apply()` ordering or the
...
resync; a change to the `*tables` Table reconciliation, dispatch-chain
structure, mark-bit allocation, IP-set ordering, or route ownership
classification; or a change to the `proto.*` dataplane API. When one
of them does earn an edit, it goes in the relevant section of this
file in the same PR — and in
