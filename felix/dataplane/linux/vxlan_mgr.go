...
func (m *vxlanManager) CompleteDeferredWork() error {
	...
	if m.vtepsDirty {
		...
		for _, u := range m.vtepsByNode {
			...
			addr := u.Ipv4Addr
			parentDeviceIP := u.ParentDeviceIpv4
			if m.ipVersion == 6 {
				addr = u.Ipv6Addr
	...
	for {
		...
		if parent, err := m.getLocalVTEPParent(); err != nil {
			...
			if m.getNoEncapRouteTable() == nil {
				...
				if m.ipVersion == 6 {
					noEncapRouteTable = m.noEncapRTConstruct([]string{"^" + parent.Attrs().Name + "$"}, m.ipVersion, false, m.dpConfig.NetlinkTimeout, m.dpConfig.DeviceRouteSourceAddressIPv6,
						m.noEncapProtocol, false)
				}
...
func (m *vxlanManager) getParentInterface(localVTEP *proto.VXLANTunnelEndpointUpdate) (netlink.Link, error) {
	...
	parentDeviceIP := localVTEP.ParentDeviceIpv4
	if m.ipVersion == 6 {
