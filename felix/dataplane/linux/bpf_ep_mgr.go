...
func (m *bpfEndpointManager) doApplyPolicyToDataIface(iface string) (bpfInterfaceState, error) {
	...
	if err := m.dataIfaceStateFillJumps(tcAttachPoint, &state); err != nil {
		return state, err
	}
...
func (m *bpfEndpointManager) dataIfaceStateFillJumps(ap *tc.AttachPoint, state *bpfInterfaceState) error {
	var err error

	if m.v4 != nil {
		err = m.allocJumpIndicesForDataIface(ap.IfaceName(), &state.v4)
		if err != nil {
			return err
		}
	}
	...
	if m.v6 != nil {
		err = m.allocJumpIndicesForDataIface(ap.IfaceName(), &state.v6)
		if err != nil {
			return err
		}
	}
