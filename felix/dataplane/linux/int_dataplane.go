...
func (d *InternalDataplane) doStaticDataplaneConfig() {
	...
	if d.config.BPFEnabled {
		...
		d.setUpIptablesBPF()
		// } else if d.config.RulesConfig.NFTables {
...
func (d *InternalDataplane) setUpNFTablesNormal() {
	for _, t := range d.rawTables {
		...
		// Connect the nftables base chains to the static raw table chains.
		t.InsertOrAppendRules("prerouting", []generictables.Rule{{
			Match:  d.newMatch(),
			Action: d.actionSet.JumpAction(rules.ChainRawPrerouting),
		}})
	...
	for _, t := range d.filterTables {
		...
		// Connnect the static filter table chains to the root hook chains.
