...
func useNftables(config Config, proxyEnabled bool) bool {
	...
	switch config.RulesConfig.NFTablesMode {
	case "Auto":
	...
	log.WithFields(log.Fields{
		...
		"calicoMode":       config.RulesConfig.NFTablesMode,
		"useNftables":      use,
...
// kubeProxyNftablesFn returns a function that can be used to check if kube-proxy is running
// in nftables mode.
func kubeProxyNftablesFn() (func() (bool, error), error) {
	...
	nft, err := knftables.New(knftables.IPv4Family, "kube-proxy")
	if err != nil {
		log.WithError(err).Warn("Failed to create nftables interface to check kube-proxy mode.")
		return nil, err
	}
	nftv6, err := knftables.New(knftables.IPv6Family, "kube-proxy")
	if err != nil {
	...
	// Return a function that checks both IPv4 and IPv6 tables.
