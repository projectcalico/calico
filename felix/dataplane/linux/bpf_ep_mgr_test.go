...
var _ = Describe("BPF Endpoint Manager", func() {
	...
	BeforeEach(func() {
		...
		countersMap = mock.NewMockMap(counters.MapParameters)
	...
	Describe("counters", func() {
		It("should clean up after restart", func() {
			counters.EnsureExists(countersMap, "12345", bpf.HookEgress)
			counters.EnsureExists(countersMap, "12345", bpf.HookIngress)
			counters.EnsureExists(countersMap, "12345", bpf.HookXDP)
			counters.EnsureExists(countersMap, "54321", bpf.HookEgress)
			counters.EnsureExists(countersMap, "54321", bpf.HookIngress)
			...
			Expect(countersMap.Contents).To(HaveKey(string(counters.NewKey(15, bpf.HookIngress))))
			Expect(countersMap.Contents).To(HaveKey(string(counters.NewKey(15, bpf.HookEgress))))
		})
	})
