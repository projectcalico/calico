...
var _ = Describe("BPF Endpoint Manager", func() {
	...
	Describe("counters", func() {
		It("should clean up after restart", func() {
			...
			Expect(countersMap.Contents).To(HaveKey(string(counters.NewKey(15, bpf.HookIngress).AsBytes())))
			Expect(countersMap.Contents).To(HaveKey(string(counters.NewKey(15, bpf.HookEgress).AsBytes())))
		})
	})
