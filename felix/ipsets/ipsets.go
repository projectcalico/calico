...
func (s *IPSets) ApplyUpdates() {
	...
	for attempt := 0; attempt < 10; attempt++ {
		...
		if err := s.tryUpdates(dirtyIPSets); err != nil {
			if attempt > 5 {
				...
				s.ipSetsRequiringResync.AddAll(dirtyIPSets)
			} else {
				// First few attempts, retry without a resync.  This helps if the
				// error is transient; ipset can return EBUSY and the like.
				s.logCxt.WithError(err).WithField("attempt", attempt).Warning(
					"Failed to update IP sets. First few failures, will retry without resync.")
			}
...
func (s *IPSets) tryUpdates(dirtyIPSets []string) (err error) {
