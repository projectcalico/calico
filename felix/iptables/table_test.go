...
// Table in cleanup-only mode is a table that Felix doesn't program; it only exists so that we can
// remove rules left behind by a previous Felix that used a different dataplane (e.g. the iptables
// tables when running in nftables mode).  If a third party writes native nft rules that
// iptables-nft-save can't render, the save fails permanently.  For a table we own that is fatal
// (see "with iptables returning an nft error" above), but for a cleanup-only table there is nothing
// we could have cleaned up anyway, so we must skip it instead of crash-looping calico-node.
var _ = Describe("Table in cleanup-only mode with incompatible nft rules", func() {
	var dataplane *testutils.MockDataplane
	var table *Table

	BeforeEach(func() {
		dataplane = testutils.NewMockDataplane("filter", map[string][]string{
			// A stale insert left behind by a previous Felix; this is the sort of thing a
			// cleanup-only table exists to remove.
			"FORWARD": {"-m comment --comment \"cali:hecdSCslEjdBPBPo\" --jump DROP"},
			"INPUT":   {},
			"OUTPUT":  {},
		}, "nft")
		// iptables-nft-save emits this line and then exits with RC 0.
		dataplane.Prologue = "# Table `filter' is incompatible, use 'nft' tool.\n"
		featureDetector := environment.NewFeatureDetector(nil)
		featureDetector.NewCmd = dataplane.NewCmd
		featureDetector.GetKernelVersionReader = dataplane.GetKernelVersionReader
		table = NewTable(
	...
	It("should skip the table without retrying the save or writing to it", func() {
		table.Apply()
		// One save attempt only: retrying can't help, since the incompatible rules belong to
		// another program.  And no restore, since we have no idea what's in the table.
	...
	It("should not sleep while backing off", func() {
		table.Apply()
		Expect(dataplane.CumulativeSleep).To(BeZero())
	})

	It("should leave the stale insert in place while the table is unreadable", func() {
		table.Apply()
		Expect(dataplane.Chains["FORWARD"]).To(HaveLen(1))
	})

	It("should recover and clean up once the incompatible rules are removed", func() {
		table.Apply()
		dataplane.Prologue = "# generated by dummy iptables-save\n"
		table.Apply()
		Expect(dataplane.CmdNames).To(ContainElement("iptables-nft-restore"))
