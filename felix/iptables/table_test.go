...
var _ = Describe(
	"Table with an empty dataplane (nft)", func() {
		describeEmptyDataplaneTests("nft")
	},
)

var _ = Describe(
	"Table with an empty dataplane (legacy)", func() {
		describeEmptyDataplaneTests("legacy")

		It(
			"should find the iptables-legacy-* iptables binaries", func() {
				dataplane := testutils.NewMockDataplane(
				...
				table := NewTable(
					"filter", 4, rules.RuleHashPrefix, featureDetector, TableOptions{
				...
				table.InsertOrAppendRules(
					"FORWARD", []generictables.Rule{
...
func describeEmptyDataplaneTests(dataplaneMode string) {
	...
	Describe(
		"with iptables returning an nft error", func() {
			BeforeEach(
				func() {
	...
	It(
		"should have a refresh scheduled at start-of-day", func() {
			Expect(table.Apply()).To(Equal(50 * time.Millisecond))
		},
	)

	It(
		"Should defer updates until Apply is called", func() {
			table.InsertOrAppendRules(
				"FORWARD", []generictables.Rule{
					{Match: Match(), Action: DropAction{}},
				},
	...
	It(
		"should ignore delete of nonexistent chain", func() {
			table.RemoveChains(
				[]*generictables.Chain{
					{Name: "cali-foobar", Rules: []generictables.Rule{{Match: Match(), Action: AcceptAction{}}}},
				},
	...
	It(
		"should police the insert mode", func() {
			Expect(
				func() {
					NewTable(
						"filter", 4, rules.RuleHashPrefix, featureDetector, TableOptions{
	...
	Describe(
		"after inserting a rule", func() {
			...
			It(
				"should acquire the iptables lock", func() {
					Expect(iptLock.WasTaken).To(BeTrue())
				},
			)
			It(
				"should release the iptables lock", func() {
					Expect(iptLock.Held).To(BeFalse())
				},
			)
			It(
				"should be in the dataplane", func() {
					Expect(dataplane.Chains).To(
						Equal(
							map[string][]string{
			...
			It(
				"further inserts should be idempotent", func() {
					table.InsertOrAppendRules(
						"FORWARD", []generictables.Rule{
							{Match: Match(), Action: DropAction{}},
						},
			...
			Describe(
				"after inserting a rule then updating the insertions", func() {
					BeforeEach(
						func() {
							table.InsertOrAppendRules(
								"FORWARD", []generictables.Rule{
					...
					It(
						"should update the dataplane", func() {
							Expect(dataplane.Chains).To(
								Equal(
									map[string][]string{
										"FORWARD": {
			...
			Describe(
				"after another process removes the insertion (empty chain)", func() {
					BeforeEach(
						func() {
					...
					It(
						"should put it back on the next explicit refresh", func() {
					...
					It(
						"should defer recheck of the dataplane until after first recheck time",
						shouldNotBeFixedAfter(49*time.Millisecond),
					)
					It(
						"should recheck the dataplane if time has advanced far enough",
						shouldBeFixedAfter(50*time.Millisecond),
					)
					It(
						"should recheck the dataplane even if one of the recheck steps was missed",
			...
			Describe(
				"after another process replaces the insertion (non-empty chain)", func() {
					BeforeEach(
						func() {
							dataplane.Chains = map[string][]string{
								"FORWARD": {
					...
					It(
						"should put it back on the next refresh", func() {
							table.InvalidateDataplaneCache("test")
							table.Apply()
							Expect(dataplane.Chains).To(
								Equal(
	...
	Describe(
		"after adding a couple of chains", func() {
			BeforeEach(
				func() {
					table.UpdateChains(
						[]*generictables.Chain{
			...
			It(
				"nothing should get programmed due to lack of references", func() {
					Expect(dataplane.Chains).To(
						Equal(
							map[string][]string{
								"FORWARD": {},
			...
			Describe(
				"after adding a reference from another unreferenced chain", func() {
					BeforeEach(
						func() {
							table.UpdateChain(
								&generictables.Chain{
					...
					It(
						"nothing should get programmed due to having no path back to root chain", func() {
							Expect(dataplane.Chains).To(
								Equal(
									map[string][]string{
										"FORWARD": {},
					...
					Describe(
						"after adding an indirect reference from an insert", func() {
							BeforeEach(
								func() {
									table.InsertOrAppendRules(
										"FORWARD", []generictables.Rule{
							...
							It(
								"both chains should be programmed", func() {
									Expect(dataplane.Chains).To(
										Equal(
											map[string][]string{
												"FORWARD": {
							...
							Describe(
								"after deleting the inserted rule", func() {
									BeforeEach(
										func() {
									...
									It(
										"should clean up both chains", func() {
											Expect(dataplane.Chains).To(
												Equal(
													map[string][]string{
														"FORWARD": {},
							...
							Describe(
								"after switching the intermediate rule", func() {
									BeforeEach(
										func() {
											table.UpdateChain(
												&generictables.Chain{
									...
									It(
										"correct chain should be swapped in", func() {
											Expect(dataplane.Chains).To(
												Equal(
													map[string][]string{
														"FORWARD": {
							...
							Describe(
								"after removing the reference", func() {
									BeforeEach(
										func() {
											table.UpdateChain(
												&generictables.Chain{
									...
									It(
										"should clean up referred chain", func() {
											Expect(dataplane.Chains).To(
												Equal(
													map[string][]string{
														"FORWARD": {
			...
			Describe(
				"after adding a reference from another referenced chain", func() {
					BeforeEach(
						func() {
							table.InsertOrAppendRules(
								"FORWARD", []generictables.Rule{
					...
					Describe(
						"after adding a reference from an insert", func() {
							BeforeEach(
								func() {
									table.InsertOrAppendRules(
										"FORWARD", []generictables.Rule{
							...
							It(
								"intermediate chain should be removed", func() {
									Expect(dataplane.Chains).To(
										Equal(
											map[string][]string{
												"FORWARD": {
							...
							Describe(
								"after deleting the intermediate chain", func() {
									BeforeEach(
										func() {
									...
									Describe(
										"after removing the insert", func() {
											BeforeEach(
												func() {
				...
				"after adding a reference from an insert", func() {
					...
					Describe(
						"then updating the chain", func() {
							...
							It(
								"shouldn't get written more than once", func() {
									dataplane.ResetCmds()
									table.Apply()
									Expect(dataplane.CmdNames).To(BeEmpty())
								},
							...
							It(
								"should squash idempotent updates", func() {
									table.UpdateChains(
										[]*generictables.Chain{
											{Name: "cali-foobar", Rules: []generictables.Rule{
												// Same data as above.
					...
					Describe(
						"then extending the chain", func() {
							...
							Describe(
								"then truncating the chain", func() {
									BeforeEach(
										func() {
											table.UpdateChains(
												[]*generictables.Chain{
					...
					Describe(
						"then removing the chain by name", func() {
							BeforeEach(
								func() {
							...
							It(
								"should be gone from the dataplane", func() {
									Expect(dataplane.Chains).To(
										Equal(
											map[string][]string{
												"FORWARD": {
	...
	Describe(
		"after adding a force-programmed chain", func() {
			BeforeEach(
				func() {
					table.UpdateChains(
						[]*generictables.Chain{
			...
			Describe(
				"after adding a reference", func() {
					...
					It(
						"it should still be programmed", func() {
							Expect(dataplane.Chains).To(
								Equal(
									map[string][]string{
										"FORWARD": {
			...
			Describe(
				"after disabling force programming", func() {
					BeforeEach(
						func() {
							table.UpdateChains(
								[]*generictables.Chain{
			...
			Describe(
				"after removing the chain", func() {
					...
					Describe(
						"after adding it back non-forced", func() {
							BeforeEach(
								func() {
									table.UpdateChains(
										[]*generictables.Chain{
	...
	Describe(
		"applying updates when underlying iptables have changed in a approved chain", func() {
			BeforeEach(
				func() {
					table.InsertOrAppendRules(
						"FORWARD", []generictables.Rule{
			...
			Describe(
				"then truncating the chain, with the iptables changed before iptables-restore", func() {
					BeforeEach(
						func() {
	...
	Describe(
		"applying updates when underlying iptables have changed in a non-approved chain", func() {
			BeforeEach(
				func() {
					table.InsertOrAppendRules(
						"FORWARD", []generictables.Rule{
				...
				"then truncating the chain, with the iptables changed before iptables-restore", func() {
					BeforeEach(
						func() {
							...
							table.InsertOrAppendRules(
								"non-cali-chain", []generictables.Rule{
									{Match: Match(), Action: DropAction{}, Comment: []string{"new drop rule"}},
								},
							)
							table.Apply()
	...
	Describe(
		"inserting into a non-Calico chain results in the expected writes", func() {
			BeforeEach(
				func() {
					table.InsertOrAppendRules(
						"FORWARD", []generictables.Rule{
			...
			It(
				"should update the dataplane", func() {
					...
					if dataplaneMode == "nft" {
						Expect(dataplane.CmdNames).To(
							ConsistOf(
								"iptables", "iptables-nft-save", "iptables-nft-restore",
							),
						)
					} else {
			...
			Describe(
				"then inserting the same rules", func() {
					BeforeEach(
						func() {
							table.InsertOrAppendRules(
								"FORWARD", []generictables.Rule{
					...
					It(
						"should result in no inserts", func() {
			...
			Describe(
				"then inserting different rules", func() {
					BeforeEach(
						func() {
							table.InsertOrAppendRules(
								"FORWARD", []generictables.Rule{
					...
					It(
						"should result in modifications", func() {
	...
	Describe(
		"inserting and appending into a non-Calico chain results in the expected writes", func() {
			BeforeEach(
				func() {
					table.AppendRules(
						"FORWARD", []generictables.Rule{
			...
			Describe(
				"then appending the same rules", func() {
					BeforeEach(
						func() {
							table.AppendRules(
								"FORWARD", []generictables.Rule{
			...
			Describe(
				"then inserting and appending different rules", func() {
					BeforeEach(
						func() {
							table.InsertOrAppendRules(
								"FORWARD", []generictables.Rule{
	...
	Describe(
		"only appending rules to a chain", func() {
			BeforeEach(
				func() {
					table.AppendRules(
						"FORWARD", []generictables.Rule{
			...
			It(
				"should avoid spurious insert warnings", func() {
					table.InvalidateDataplaneCache("test")
					table.Apply()
					table.Apply()
					Expect(dataplane.Chains).To(
...
var _ = Describe(
	"Tests of post-update recheck behaviour with refresh timer (nft)", func() {
		describePostUpdateCheckTests(true, "nft")
	},
)

var _ = Describe(
	"Tests of post-update recheck behaviour with no refresh timer (nft)", func() {
		describePostUpdateCheckTests(false, "nft")
	},
)

var _ = Describe(
	"Tests of post-update recheck behaviour with refresh timer (legacy)", func() {
		describePostUpdateCheckTests(true, "legacy")
	},
)

var _ = Describe(
	"Tests of post-update recheck behaviour with no refresh timer (legacy)", func() {
...
func describePostUpdateCheckTests(enableRefresh bool, dataplaneMode string) {
	...
	BeforeEach(
		func() {
			...
			featureDetector.NewCmd = dataplane.NewCmd
			featureDetector.GetKernelVersionReader = dataplane.GetKernelVersionReader
			table = NewTable("filter", 4, rules.RuleHashPrefix, featureDetector, options)
			table.InsertOrAppendRules(
				"FORWARD", []generictables.Rule{
					{Match: Match(), Action: DropAction{}},
				},
	...
	Describe(
		"with slow dataplane", func() {
			...
			Describe(
				"after advancing time 1995ms", func() {
					...
					Describe(
						"after advancing time to 2s", func() {
							...
							Describe(
								"after dataplane speeds up again", func() {
									BeforeEach(
										func() {
											table.InsertOrAppendRules(
												"FORWARD", []generictables.Rule{
									...
									It(
										"should request correct delay", func() {
	...
	Describe(
		"after advancing time 49ms", func() {
			...
			Describe(
				"after advancing time to 999ms", func() {
					...
					if enableRefresh {
						...
						Describe(
							"after advancing time by an hour", func() {
...
var _ = Describe(
	"Table with a dirty dataplane in append mode (nft)", func() {
		describeDirtyDataplaneTests(true, "nft")
	},
)

var _ = Describe(
	"Table with a dirty dataplane in insert mode (nft)", func() {
		describeDirtyDataplaneTests(false, "nft")
	},
)

var _ = Describe(
	"Table with a dirty dataplane in append mode (legacy)", func() {
		describeDirtyDataplaneTests(true, "legacy")
	},
)

var _ = Describe(
	"Table with a dirty dataplane in insert mode (legacy)", func() {
...
func describeDirtyDataplaneTests(appendMode bool, dataplaneMode string) {
	...
	It(
		"should clean up on first Apply()", func() {
			table.Apply()
			Expect(dataplane.Chains).To(
				Equal(
					map[string][]string{
	...
	Describe(
		"with pre-cleanup inserts, appends and updates", func() {
			...
			It(
				"with no errors, it should get to correct final state", func() {
					table.Apply()
					checkFinalState()
			...
			It(
				"with no errors, it shouldn't sleep", func() {
					table.Apply()
			...
			Describe(
				"With a transient iptables-save failure", func() {
					BeforeEach(
						func() {
			...
			Describe(
				"With a transient iptables-save failure and a kill failure", func() {
					BeforeEach(
						func() {
							dataplane.FailNextSaveRead = true
			...
			Describe(
				"With a transient iptables-save pipe-close failure", func() {
					BeforeEach(
						func() {
			...
			Describe(
				"With a transient iptables-save start failure", func() {
					BeforeEach(
						func() {
			...
			Describe(
				"With a transient iptables-save start and pipe-close failure", func() {
					BeforeEach(
						func() {
							dataplane.FailNextStart = true
							dataplane.FailNextPipeClose = true
			...
			Describe(
				"With a persistent iptables-save failure", func() {
					BeforeEach(
						func() {
					...
					It(
						"it should do exponential backoff", func() {
							Expect(
								func() {
									table.Apply()
								},
			...
			It(
				"shouldn't touch already-correct chain", func() {
					table.Apply()
			...
			if dataplaneMode == "legacy" {
				It(
					"shouldn't touch already-correct rules", func() {
						table.Apply()
				...
				It(
					"should rewrite whole chain", func() {
						table.Apply()
						// First two rules are already correct...
			...
			It(
				"with a transient error, it should get to correct final state", func() {
			...
			Describe(
				"with a persistent iptables-restore error", func() {
					BeforeEach(
						func() {
					...
					It(
						"it should do exponential backoff", func() {
							...
							Expect(dataplane.CumulativeSleep).To(
								Equal(
			...
			Describe(
				"with a simulated clobber of chains before first write", func() {
					BeforeEach(
						func() {
					...
					It(
						"should get to correct final state", func() {
							Expect(dataplane.Chains).To(
								Equal(
									map[string][]string{
										"FORWARD": {
					...
					It(
						"should sleep for correct time", func() {
			...
			Describe(
				"with a clobber after initial write", func() {
					...
					It(
						"should get to correct state", func() {
					...
					It(
						"it shouldn't sleep", func() {
							table.Apply()
							Expect(dataplane.CumulativeSleep).To(BeZero())
						},
					)
					It(
						"and pending updates, should get to correct state", func() {
...
var _ = Describe(
	"Table with inserts and a non-Calico chain (legacy)", func() {
		describeInsertAndNonCalicoChainTests("legacy")
	},
)

var _ = Describe(
	"Table with inserts and a non-Calico chain (nft)", func() {
...
func describeInsertAndNonCalicoChainTests(dataplaneMode string) {
	...
	BeforeEach(
		func() {
			...
			featureDetector.GetKernelVersionReader = dataplane.GetKernelVersionReader
			table = NewTable(
				"filter", 6, rules.RuleHashPrefix, featureDetector, TableOptions{
					HistoricChainPrefixes: rules.AllHistoricChainNamePrefixes,
					NewCmdOverride:        dataplane.NewCmd,
					SleepOverride:         dataplane.Sleep,
					NowOverride:           dataplane.Now,
	...
	Describe(
		"after removing the other chain", func() {
			BeforeEach(
				func() {
					dataplane.Chains = map[string][]string{
						"FORWARD": {"-m comment --comment \"cali:hecdSCslEjdBPBPo\" --jump DROP"},
			...
			It(
				"should ignore the deletion", func() {
					Expect(dataplane.Chains).To(
						Equal(
							map[string][]string{
								"FORWARD": {"-m comment --comment \"cali:hecdSCslEjdBPBPo\" --jump DROP"},
			...
			It(
				"should make no changes to the dataplane", func() {
					Expect(dataplane.CmdNames).To(BeEmpty())
				},
			)
			It(
				"should not take the lock", func() {
					Expect(iptLock.WasTaken).To(BeFalse())
				},
			)
		},
...
var _ = Describe(
	"Insert early rules (legacy)", func() {
		describeInsertEarlyRules("legacy")
	},
)

var _ = Describe(
	"Insert early rules (nft)", func() {
...
func describeInsertEarlyRules(dataplaneMode string) {
	...
	It(
		"should insert rules immediately without Apply", func() {
	...
	It(
		"should find out if rules already present", func() {
			rls := []generictables.Rule{
				{Match: Match(), Action: DropAction{}, Comment: []string{"my rule"}},
				{Match: Match(), Action: AcceptAction{}, Comment: []string{"my other rule"}},
			}
