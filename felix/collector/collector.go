...
func (c *collector) evaluatePendingRuleTrace(direction rules.RuleDir, store *policystore.PolicyStore, ep calc.EndpointData, flow TupleAsFlow, ruleIDs *[]*calc.RuleID) {
	...
	if protoEp := c.lookupProtoWorkloadEndpoint(store, ep.GetKey()); protoEp != nil {
		...
		log.WithField("endpoint", ep.GetKey()).Trace("The endpoint is not yet tracked by the PolicyStore")
	}
}
