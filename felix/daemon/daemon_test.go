...
var _ = Describe("Typha address discovery", func() {
	...
	It("should return address if configured", func() {
		...
		typhaAddr, err := discoverTyphaAddrs(configParams, nil, &MockNodesClient{})
		Expect(err).NotTo(HaveOccurred())
	...
	It("should return IP from endpoints", func() {
		typhaAddr, err := discoverTyphaAddrs(configParams, k8sClient, &MockNodesClient{})
		Expect(err).NotTo(HaveOccurred())
	...
	It("should error if no Typhas", func() {
		...
		refreshClient()
		_, err := discoverTyphaAddrs(configParams, k8sClient, &MockNodesClient{})
	...
	It("should choose random Typhas", func() {
		...
		addr, err := discoverTyphaAddrs(configParams, k8sClient, &MockNodesClient{})
		Expect(err).NotTo(HaveOccurred())
		Expect(addr).To(
			ContainElements(
				discovery.Typha{Addr: "10.0.0.2:8156", IP: "10.0.0.2"},
...
func discoverTyphaAddrs(params *config.Config, sClient *fake.Clientset, m *MockNodesClient) ([]discovery.Typha, error) {
	disc := createTyphaDiscoverer(params, sClient, m)
