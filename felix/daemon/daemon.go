...
configRetry:
	for configAttempts := 0; true; configAttempts++ {
		if configAttempts > 0 {
			log.WithField("attempt", configAttempts).Info("Sleeping before retrying to load configuration...")
	...
	loadDatastoreRetry:
		for numDatastoreRetries := 1; true; numDatastoreRetries++ {
			if numDatastoreRetries > 1 {
				time.Sleep(time.Second)
			}
		...
		// sound before we exit the loop.  Note: as a side effect this may trigger Wireguard bootstrap cleanup
		// processing to make sure that wireguard doesn't cut us off from Typha.
		var typhaAddresses []discovery.Typha
		typhaDiscoverer, typhaAddresses, err = discoverTyphaAddrs(configParams, k8sClientSet, v3Client)
		if err != nil {
	...
	if syncer != nil {
		...
		if err != nil {
			time.Sleep(time.Second) // Avoid tight restart loop
...
func discoverTyphaAddrs(configParams *config.Config, k8sClientSet kubernetes.Interface, v3Client client.NodesClient) (*discovery.Discoverer, []discovery.Typha, error) {
	typhaDiscoverer := discovery.New(
		...
		discovery.WithPostDiscoveryFilter(func(typhaAddresses []discovery.Typha) ([]discovery.Typha, error) {
			...
			// asymmetry). If all typhas would be filtered out then wireguard is removed from the node and
			// all typhas are returned (unfiltered).
	...
	addrs, err := typhaDiscoverer.LoadTyphaAddrs()
	return typhaDiscoverer, addrs, err
}
