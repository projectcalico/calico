...
configRetry:
	...
	typhaAddresses, err := bootstrapFilterTyphaForWireguard(configParams, v3Client, typhaAddresses, wireguardPeersToValidate)
	if err != nil {
		time.Sleep(2 * time.Second) // avoid a tight restart loop
		log.WithError(err).Fatal("Couldn't bootstrap WireGuard host connectivity")
	}
	...
	if syncer != nil {
		...
		if err != nil {
			...
			if err != nil {
				// We failed to connect to typha. If we have wireguard peers to validate then that means we could
				// potentially be in a situation where wireguard has broken routing. Just temporarily delete the
				// wireguard configuration before we exit.
				if wireguardPeersToValidate != nil {
					err2 := bootstrapRemoveWireguard(configParams, v3Client)
