...
var _ = Describe("Static with connection state transition logging", func() {
	var rr *DefaultRuleRenderer
	var conf Config
	...
	BeforeEach(func() {
		conf = Config{
			...
			MarkEndpoint:                  0xff000,
			MarkNonCaliEndpoint:           0x1000,
			LogConnectionStateTransitions: true,
			...
			Rules: []generictables.Rule{
				{
					...
					Action: iptables.LogAction{Prefix: "calico-packet-rst"},
				},
				{
					Match:  iptables.Match().TCPFlagsSet("RST"),
				...
				},
				{
					...
					Action: iptables.LogAction{Prefix: "calico-packet-icmp-err"},
				},
				{
					Match:  iptables.Match().Protocol(icmpProto).ConntrackState("RELATED"),
					Action: clearMarkAction,
				...
				{
					Match:  iptables.Match(),
					Action: iptables.LogAction{Prefix: "calico-packet-est"},
				},
				{
					Match:  iptables.Match(),
					Action: clearMarkAction,
	...
	Describe("with a log prefix containing specifiers", func() {
		BeforeEach(func() {
			conf.LogPrefix = "acme-%n"
		})

		It("should strip the specifiers from the static log prefixes", func() {
			chain := findChain(rr.StaticFilterTableChains(4), "cali-log-conn")
			Expect(chain).NotTo(BeNil())
			Expect(chain.Rules[0].Action).To(Equal(iptables.LogAction{Prefix: "acme--rst"}))
		})
	})

	Describe("with a long log prefix", func() {
		BeforeEach(func() {
			conf.LogPrefix = "abcdefghijklmnopqrstuvwxyz0123" // 30 characters.
		})
	...
	Describe("with the feature disabled", func() {
		BeforeEach(func() {
			conf.LogConnectionStateTransitions = false
