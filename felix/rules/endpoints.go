...
func (r *DefaultRuleRenderer) endpointIptablesChain(
	...
	// if _, ok := r.NewMatch().(generictables.NFTMatchCriteria); ok {
	// 	// Nftables includes the table in the prefix, since all logical iptables tables are
	// 	// combined into a single nftables table.
	// 	endpointPrefix = fmt.Sprintf("%s-%s", table, endpointPrefix)
	// }
	chainName := EndpointChainName(endpointPrefix, name)
