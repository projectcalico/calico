...
func (r *DefaultRuleRenderer) endpointIptablesChain(
	...
	if chainType == chainTypeNormal && qosControls != nil {
		log.WithField("qosControls", qosControls).Debug("Rendering QoS controls rules")
		// Add ingress packet rate limit rules if applicable
		if dir == RuleDirIngress && qosControls.IngressPacketRate != 0 {
			log.WithFields(log.Fields{"IngressPacketRate": qosControls.IngressPacketRate, "mark": r.MarkLimitPacketRate}).Debug("Rendering ingress packet rate limit rules")
			if r.NFTables {
				rules = append(rules,
					generictables.Rule{
						Match:   r.NewMatch(),
		...
		if dir == RuleDirEgress && qosControls.EgressPacketRate != 0 {
			log.WithFields(log.Fields{"EgressPacketRate": qosControls.EgressPacketRate, "mark": r.MarkLimitPacketRate}).Debug("Rendering egress packet rate limit rules")
			if r.NFTables {
				rules = append(rules,
					generictables.Rule{
						Match:   r.NewMatch(),
