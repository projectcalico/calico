...
// connStateLogPrefix generates the log prefix for a connection state transition log.
// The chain is shared by all endpoints so, unlike generateLogPrefix, per-policy
// %-specifiers cannot be resolved; they are stripped instead.  The base prefix is
// truncated so that the suffix and the ": " appended by the Log action stay within the
// backend's log prefix limit.
func (r *DefaultRuleRenderer) connStateLogPrefix(suffix string) string {
	base := "calico-packet"
	if len(r.LogPrefix) != 0 {
		base = r.LogPrefix
	}
	base = logPrefixRE.ReplaceAllString(base, "")
