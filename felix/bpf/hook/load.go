			...
			for _, epToHostDrop := range []bool{false, true} {
				...
				for _, fibEnabled := range []bool{false, true} {
					...
					for _, epType := range epTypes {
						epType := epType
						if epToHostDrop && epType != tcdefs.EpTypeWorkload {
						...
						for _, hook := range []Hook{Ingress, Egress} {
							...
							if hook == Egress && epToHostDrop {
								log.Debug("Skipping combination since epToHostDrop only affect from targets")
								continue
							}

							for _, dsr := range []bool{false, true} {
								if dsr && !((epType == tcdefs.EpTypeWorkload && hook == Ingress) ||
									(epType == tcdefs.EpTypeHost)) {
