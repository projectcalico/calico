...
type KubeProxy struct {
	...
	ConsistentHashMap maps.MapWithExistsCheck
...
func StartKubeProxy(k8s kubernetes.Interface, hostname string,
	...
	kp := &KubeProxy{
		...
		ConsistentHashMap: bpfMaps.ConsistentHashMap.(maps.MapWithExistsCheck),
...
func (kp *KubeProxy) run(hostIPs []net.IP) error {
	...
	syncer, err := NewSyncer(kp.ipFamily, withLocalNP, kp.frontendMap, kp.backendMap, kp.ConsistentHashMap, kp.affinityMap,
...
func (kp *KubeProxy) start() error {
	...
	syncer, err := NewSyncer(kp.ipFamily, withLocalNP, kp.frontendMap, kp.backendMap, kp.ConsistentHashMap, kp.affinityMap, kp.rt, kp.excludedCIDRs)
	if err != nil {
		return errors.WithMessage(err, "new bpf syncer")
	}
