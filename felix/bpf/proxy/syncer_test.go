...
var _ = Describe("BPF Syncer API server NAT preservation", func() {
	...
	state := func(apiServerHasEP bool) proxy.DPSyncerState {
		st := proxy.DPSyncerState{
			...
			EpsMap: k8sp.EndpointsMap{
				regularSvcKey: {proxy.NewEndpointInfo("10.2.0.1", 8080, proxy.EndpointInfoOptIsReady(true))},
			},
		}
		if apiServerHasEP {
			st.EpsMap[apiSvcKey] = []k8sp.Endpoint{
				proxy.NewEndpointInfo(apiServerIP.String(), 6443, proxy.EndpointInfoOptIsReady(true)),
			}
		}
		return st
	}
