...
var _ = Describe("BPF Syncer", func() {
	...
	BeforeEach(func() {
		...
		s, _ = proxy.NewSyncer(4, nodeIPs, svcs, eps, mgEps, aff, rt, nil, maglevLUTSize, maglevMaxSvcs)
	...
	It("should make the right test transitions", func() {
		...
		By("inserting non-local eps for a NodePort - no route", makestep(func() {
			...
			s, _ = proxy.NewSyncer(4, append(nodeIPs, net.IPv4(255, 255, 255, 255)), svcs, eps, mgEps, aff, rt, nil, maglevLUTSize, maglevMaxSvcs)
			state.SvcMap[svcKey2] = proxy.NewK8sServicePort(
				net.IPv4(10, 0, 0, 2),
				2222,
				v1.ProtocolTCP,
