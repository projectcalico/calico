...
func printk(b *asm.Block, msg string) {
	...
	for offset := l; offset > 0; offset -= 8 {
		...
		b.Store64(asm.R10, asm.R2, asm.FieldOffset{int16(offset - l - 8), "stack"})
	}
	...
	for i, cbpf := range pcap {
		...
		case bpfClassLd:
			...
			case bpfModeIND:
				...
				continue
			case bpfModeABS:
				b.Load(asm.R1, asm.R7, asm.FieldOffset{int16(K), ""}, asm.OpCode(size))
				if asm.OpCode(size) != asm.MemOpSize8 {
					fromBE(b, size)
				}
				continue
		...
		case bpfClassLdx:
			if cbpf.Code == uint16(bpfClassLdx|bpfModeMSH|bpfSizeB) {
				if K == 14 {
					...
					b.Load8(asm.R1 /* A */, asm.R7 /* pkt */, asm.FieldOffset{int16(K), ""}) // Load pkt[K] to A
