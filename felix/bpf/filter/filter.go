...
// non-matching, mirroring the bounded walk in felix/bpf-gpl/parsing6.h.
// Headers past the first maxData bytes of the packet are out of reach
// regardless: the filter matches against the pre-loaded scratch buffer.
...
func emitInsn(b *asm.Block, i int, cbpf pcap.BPFInstruction, linkType layers.LinkType, label func(target int) string) error {
	...
	case bpfClassLd:
		...
		case bpfModeIND:
			...
			// comparison. Bound the scalar offset against the loaded length
			// (R9 <= maxData) first, then form the pointer, so the verifier
			// can prove the load stays within the scratch buffer.
			b.Mov64(asm.R3 /* tmp */, asm.R2 /* X */) // tmp = X
			if K > 0 {
				b.AddImm64(asm.R3, K) // tmp = X + K
			}
			b.JumpGE64(asm.R3, asm.R9, "exit")                                  // if X+K >= len, out of range
...
func jumpTargets(i int, cbpf pcap.BPFInstruction) []int {
	...
	return []int{i + int(cbpf.Jt) + 1, i + int(cbpf.Jf) + 1}
}
