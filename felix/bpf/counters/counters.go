...
func (c Counters) Read() ([][]uint64, error) {
	...
	for hook, name := range bpf.Hooks {
		...
		if err != nil {
			logrus.Debugf("Failed to read bpf counters. hook=%s err=%v", name, err)
			continue
		}
...
func (c *Counters) Flush() error {
	for hook, name := range bpf.Hooks {
		...
		if err != nil {
			logrus.Debugf("Failed to flush bpf counters for interface=%s hook=%s. err=%v", c.iface, name, err)
			continue
		}
