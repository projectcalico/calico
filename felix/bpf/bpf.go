...
// every loaded object's .rodata.cali_flags section, so the verifier eliminates
...
func loadObject(obj *libbpf.Obj, data libbpf.GlobalData, mapPinOverrides map[string]string, mapsToBePinned ...string) (*libbpf.Obj, error) {
	...
	for m, err := obj.FirstMap(); m != nil && err == nil; m, err = m.NextMap() {
		...
		mapName := m.Name()
		if m.IsMapInternal() {
			// Per-object load-time feature flags (see struct cali_rodata_flags).
			// Its own section so it does not disturb the main globals .rodata.
			if strings.HasSuffix(mapName, ".rodata.cali_flags") {
				if err := m.SetRodataFlags(noTracePrintk); err != nil {
