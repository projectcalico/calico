...
type bpfIPSets struct {
	...
	dpDriver    BPFIPsetsDPDriver
	bpfLogLevel string
}

type BPFIPsetsDPDriver interface {
	AddOrReplaceIPSet(id uint64, ipFamilu int)
	RemoveIPSet(id uint64, ipFamilu int)
}
...
func (m *bpfIPSets) AddOrReplaceIPSet(setMetadata ipsets.IPSetMetadata, members []string) {
	...
	m.markIPSetDirty(ipSet)
	if m.dpDriver != nil {
		m.dpDriver.AddOrReplaceIPSet(ipSet.ID, m.IPVersionConfig.Family.Version())
	}
}
...
func (m *bpfIPSets) RemoveIPSet(setID string) {
	...
	m.markIPSetDirty(ipSet)
	if m.dpDriver != nil {
		m.dpDriver.RemoveIPSet(ipSet.ID, m.IPVersionConfig.Family.Version())
	}
}
