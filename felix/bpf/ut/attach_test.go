...
func TestAttachWithDualStack(t *testing.T) {
	RegisterTestingT(t)

	bpfmaps, err := bpfmap.CreateBPFMaps(true)
	Expect(err).NotTo(HaveOccurred())
	...
	t.Run("restart", func(t *testing.T) {
		...
		bpfEpMgr.OnUpdate(linux.NewIfaceStateUpdate("hostep2", ifacemonitor.StateUp, host2.Attrs().Index))
		bpfEpMgr.OnUpdate(linux.NewIfaceAddrsUpdate("hostep2", "4.3.2.1"))
		bpfEpMgr.OnUpdate(linux.NewIfaceAddrsUpdate("hostep2", "4::1"))
		err = bpfEpMgr.CompleteDeferredWork()
		Expect(err).NotTo(HaveOccurred())
