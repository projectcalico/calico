...
// .rodata.cali_flags no_trace_printk flag before load, the verifier folds the
...
func TestNoTracePrintkFlagStripsTraceHelper(t *testing.T) {
	...
	loadedTraceHelperRefs := func(noTrace bool) int {
		...
		for m, err := o.FirstMap(); m != nil && err == nil; m, err = m.NextMap() {
			if strings.HasSuffix(m.Name(), ".rodata.cali_flags") {
				Expect(m.SetRodataFlags(noTrace)).NotTo(HaveOccurred())
				flagSet = true
			}
		}
		Expect(flagSet).To(BeTrue(), "object should carry a .rodata.cali_flags map")
