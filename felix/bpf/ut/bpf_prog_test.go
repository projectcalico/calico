...
func setupAndRun(logger testLogger, loglevel, section string, rules *polprog.Rules,
	...
	if topts.xdp {
		...
		fileToLoad := "../../bpf-gpl/bin/tc_preamble_th.o"
		if useIngressProgMap {
			fileToLoad = "../../bpf-gpl/bin/tc_preamble_fh.o"
		}
		o, err := objLoad(fileToLoad, bpfFsDir, "preamble", topts, false, false, useIngressProgMap)
		Expect(err).NotTo(HaveOccurred())
		defer o.Close()
	}
	...
	o, err = objLoad(tempObj, bpfFsDir, ipFamily, topts, rules != nil, true, useIngressProgMap)
	Expect(err).NotTo(HaveOccurred())
	defer o.Close()
...
func initMapsOnce() {
	mapInitOnce.Do(func() {
		...
		allMaps = []maps.Map{natMap, natBEMap, natMapV6, natBEMapV6, ctMap, ctMapV6, ctCleanupMap, ctCleanupMapV6, rtMap, rtMapV6, ipsMap, ipsMapV6,
			...
			policyJumpMap[0], policyJumpMap[1], policyJumpMapXDP, ctlbProgsMap[0], ctlbProgsMap[1], ctlbProgsMap[2], qosMap}
...
func tcUpdateJumpMap(obj *libbpf.Obj, progs []int, hasPolicyProg, hasHostConflictProg, useIngressProgMap bool) error {
...
func objLoad(fname, bpfFsDir, ipFamily string, topts testOpts, polProg, hasHostConflictProg, useIngressProgMap bool) (*libbpf.Obj, error) {
	...
	if !forXDP {
		...
		err = tcUpdateJumpMap(obj, tcJumpMapIndexes[ipFamily], false, hasHostConflictProg, useIngressProgMap)
		if err != nil && !strings.Contains(err.Error(), "error updating calico_tc_host_ct_conflict program") {
			goto out
		}
		err = tcUpdateJumpMap(obj, tcJumpMapIndexes[ipFamily], false, false, useIngressProgMap)
	} else {
