		...
		if key.Kind == v3.KindBGPConfiguration && key.Name == "default" {
			...
			if update.Value == nil {
				...
				if node.Spec.BGP != nil {
					...
					if ip4net == nil {
						ip4net = &net.IPNet{} // required for print in event sequencer
					} else {
						ip4net.IP = ip4.IP
					}
					if ip6net == nil {
						ip6net = &net.IPNet{} // required for print in event sequencer
					} else {
						ip6net.IP = ip6.IP
					}
					asnumber := ""
					...
					h.callbacks.OnHostUpdate(hostname, ip4net, ip6net, asnumber, node.Labels)
				} else {
					log.WithField("update", update).Debug("Passing-through Node remove")
					h.callbacks.OnHostRemove(hostname)
				}
