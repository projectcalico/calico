...
var _ = Describe("EndpointLookupsCache tests: endpoints", func() {
	...
	/*TODO (mazdak): this seems to be failing since hostendpoint supports is not added. Check it later.
	It("should process both workload and host endpoints each with multiple IP addresses", func() {
		...
		for _, ipv4 := range remoteWlEp1DualStack.IPv4Nets {
			verifyIpToEndpoint(remoteWlEpKey1, ipv4.IP, true, origRemoteWepLabels)
		}
		for _, ipv6 := range remoteWlEp1DualStack.IPv6Nets {
	...
	/*
		TODO (mazdak): enable this test when staged policies are added
		DescribeTable(
