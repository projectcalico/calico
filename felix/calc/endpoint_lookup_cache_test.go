...
var _ = Describe("EndpointLookupsCache tests: endpoints", func() {
	...
	DescribeTable(
		...
		func(key model.WorkloadEndpointKey, wep *model.WorkloadEndpoint, ipAddr net.IP) {
			...
			Expect(ed.GetKey()).To(Equal(key))

			// test GetEndpointKeys
			keys := ec.GetEndpointKeys()
			Expect(len(keys)).To(Equal(1))
			Expect(keys).To(ConsistOf(ed.GetKey()))
			...
			Expect(keys).NotTo(ConsistOf(ed.GetKey()))
	...
	It("should process local endpoints correctly with no staged policies and one tier per ingress and egress", func() {
		...
		Expect(ed.GetKey()).To(Equal(hostEpWithNameKey))
		Expect(ed.IsLocal()).To(BeTrue())
		Expect(ed.GetGenerateName()).To(Equal(""))
		Expect(ed.GetLabels()).To(Equal(hostEpWithName.Labels))
	...
	DescribeTable(
		...
		func(ingress bool) {
			...
			By("checking endpoint data")
			Expect(ed.GetKey()).To(Equal(localWlEpKey1))
			Expect(ed.IsLocal()).To(BeTrue())
			Expect(ed.GetGenerateName()).To(Equal(localWlEp1.GenerateName))
			Expect(ed.GetLabels()).To(Equal(localWlEp1.Labels))
