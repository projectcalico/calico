...
func outputJSON(params []*config.FieldInfo) {
	...
	// Deterministic must stay: the output is redirected to the checked-in
	// felix/docs/config-params.json file, so non-deterministic map key
	// ordering would produce spurious diffs on every regeneration.
	err := json.MarshalWrite(os.Stdout, OutputJSON{
		Comment: "This file generated by calico-felix-docgen, DO NOT EDIT.",
		Groups:  groups,
	}, json.Deterministic(true), jsontext.EscapeForHTML(false), jsontext.WithIndent("  "))
	if err != nil {
