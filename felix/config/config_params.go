...
type Config struct {
	...
	GoGCThreshold   int `config:"int(-1,);100"`
...
func (config *Config) FieldHasExplicitValue(name string) bool {
	for _, s := range SourcesInDescendingOrder {
		if config.setBy(name, s) {
			return true
		}
	}
	return false
