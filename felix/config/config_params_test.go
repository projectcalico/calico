...
var _ = Describe(
	"FelixConfig vs ConfigParams parity", func() {
		...
		It(
			"FelixConfigurationSpec should contain all Config fields", func() {
		...
		It(
			"Config should contain all FelixConfigurationSpec fields", func() {
...
var _ = Describe(
	"Config override empty", func() {
		...
		It(
			"should allow config override", func() {
			...
			"with a param set", func() {
				...
				It(
					"should allow config override", func() {
						...
						changed, err = cp.UpdateFrom(
							map[string]string{"bpfenabled": "true"}, config.EnvironmentVariable,
						)
		...
		It(
			"should have correct initial IptablesBackend value 'auto'", func() {
				Expect(cp.IptablesBackend).To(Equal("auto"))
			},
		)

		It(
			"should allow IptablesBackend value 'Auto' read from FelixConfiguration resource", func() {
...
var _ = DescribeTable(
	...
	func(key, value string, expected interface{}, errorExpected ...bool) {
		...
		Expect(newVal).To(
			Equal(expected), fmt.Sprintf("Expected %s=%q to parse as %v but got %v", key, value, expected, newVal),
		)
		if len(errorExpected) > 0 && errorExpected[0] {
			...
			Expect(cfg.Err).To(
				HaveOccurred(), fmt.Sprintf("Expected an error to be stored when setting %s=%q", key, value),
			)
		} else {
			Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("Expected no error when setting %s=%q", key, value))
			Expect(cfg.Err).NotTo(
				HaveOccurred(), fmt.Sprintf("Expected no error to be stored when setting %s=%q", key, value),
			)
		}
	},
	...
	Entry(
		"InterfaceExclude one value no regexp", "InterfaceExclude", "kube-ipvs0", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	Entry(
		"InterfaceExclude list no regexp", "InterfaceExclude", "kube-ipvs0,dummy", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
			regexp.MustCompile("^dummy$"),
		},
	),
	Entry(
		"InterfaceExclude one value regexp", "InterfaceExclude", "/kube-ipvs/", []*regexp.Regexp{
			regexp.MustCompile("kube-ipvs"),
		},
	),
	Entry(
		"InterfaceExclude list regexp", "InterfaceExclude", "kube-ipvs0,dummy,/^veth.*$/", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
			regexp.MustCompile("^dummy$"),
	...
	Entry(
		"InterfaceExclude no regexp", "InterfaceExclude", "/^kube.*/,/veth/", []*regexp.Regexp{
			regexp.MustCompile("^kube.*"),
			regexp.MustCompile("veth"),
		},
	),
	Entry(
		"InterfaceExclude list empty regexp", "InterfaceExclude", "kube,//", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	Entry(
		"InterfaceExclude list bad comma use", "InterfaceExclude", "/kube,/,dummy", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	Entry(
		"InterfaceExclude list invalid regexp symbol", "InterfaceExclude", `/^kube\K/`, []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	...
	Entry(
		"IptablesPostWriteCheckIntervalSecs", "IptablesPostWriteCheckIntervalSecs",
	...
	Entry(
		"IptablesLockProbeIntervalMillis", "IptablesLockProbeIntervalMillis",
		"123", 123*time.Millisecond,
	),
	Entry(
		"IptablesLockProbeIntervalMillis garbage", "IptablesLockProbeIntervalMillis",
	...
	Entry(
		"IptablesMangleAllowAction", "IptablesMangleAllowAction",
		"RETURN", "RETURN",
	),
	...
	Entry(
		"IpInIpTunnelAddr", "IpInIpTunnelAddr",
	...
	Entry(
		"EndpointReportingDelaySecs", "EndpointReportingDelaySecs",
		"10", 10*time.Second,
	),
	...
	Entry(
		"FailsafeInboundHostPorts old syntax", "FailsafeInboundHostPorts", "1,2,3,4",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Protocol: "tcp", Port: 2},
			{Protocol: "tcp", Port: 3},
	...
	Entry(
		"FailsafeOutboundHostPorts old syntax", "FailsafeOutboundHostPorts", "1,2,3,4",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Protocol: "tcp", Port: 2},
			{Protocol: "tcp", Port: 3},
	...
	Entry(
		"FailsafeInboundHostPorts new syntax", "FailsafeInboundHostPorts", "tcp:1,udp:2",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeOutboundHostPorts new syntax", "FailsafeOutboundHostPorts", "tcp:1,udp:2",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeInboundHostPorts new cidr syntax", "FailsafeInboundHostPorts", "tcp:0.0.0.0/0:1,udp:0.0.0.0/0:2",
		[]config.ProtoPort{
	...
	Entry(
		"FailsafeOutboundHostPorts new cidr syntax", "FailsafeOutboundHostPorts", "tcp:0.0.0.0/0:1,udp:0.0.0.0/0:2",
		[]config.ProtoPort{
			{Net: "0.0.0.0/0", Protocol: "tcp", Port: 1},
			{Net: "0.0.0.0/0", Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeInboundHostPorts new cidr syntax IPv6", "FailsafeInboundHostPorts", "tcp:[::/0]:1,udp:[::/0]:2",
		[]config.ProtoPort{
	...
	Entry(
		"FailsafeOutboundHostPorts new cidr syntax IPv6", "FailsafeOutboundHostPorts", "tcp:[::/0]:1,udp:[::/0]:2",
		[]config.ProtoPort{
			{Net: "::/0", Protocol: "tcp", Port: 1},
			{Net: "::/0", Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeInboundHostPorts mixed syntax", "FailsafeInboundHostPorts", "1,udp:2",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeOutboundHostPorts mixed syntax", "FailsafeOutboundHostPorts", "1,udp:2",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeInboundHostPorts new mixed syntax", "FailsafeInboundHostPorts", "1,udp:0.0.0.0/0:2",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Net: "0.0.0.0/0", Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeOutboundHostPorts new mixed syntax", "FailsafeOutboundHostPorts", "1,udp:0.0.0.0/0:2",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 1},
			{Net: "0.0.0.0/0", Protocol: "udp", Port: 2},
		},
	...
	Entry(
		"FailsafeInboundHostPorts bad syntax -> defaulted", "FailsafeInboundHostPorts", "foo:1",
		[]config.ProtoPort{
	...
	Entry(
		"FailsafeInboundHostPorts too many parts -> defaulted", "FailsafeInboundHostPorts", "tcp:0.0.0.0/0:1:bar",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 22},
			{Protocol: "udp", Port: 68},
			{Protocol: "tcp", Port: 179},
	...
	Entry(
		"FailsafeInboundHostPorts empty", "FailsafeInboundHostPorts", "",
		[]config.ProtoPort{
			{Protocol: "tcp", Port: 22},
			{Protocol: "udp", Port: 68},
			{Protocol: "tcp", Port: 179},
	...
	Entry(
		"FailsafeOutboundHostPorts empty", "FailsafeOutboundHostPorts", "",
		[]config.ProtoPort{
	...
	Entry(
		"KubeNodePortRanges empty", "KubeNodePortRanges", "",
		[]numorstring.Port{
			{MinPort: 30000, MaxPort: 32767, PortName: ""},
		},
	),
	Entry(
		"KubeNodePortRanges range", "KubeNodePortRanges", "30001:30002,30030:30040,30500:30600",
		[]numorstring.Port{
	...
	Entry(
		"HealthTimeoutOverrides good", "HealthTimeoutOverrides", "a=90s",
		map[string]time.Duration{"a": 90 * time.Second}, false,
	),
	Entry(
		"HealthTimeoutOverrides good x 2", "HealthTimeoutOverrides", "a=90s, b=10ms",
		map[string]time.Duration{"a": 90 * time.Second, "b": 10 * time.Millisecond}, false,
	),
	Entry(
		"HealthTimeoutOverrides good zero", "HealthTimeoutOverrides", "a=0",
		map[string]time.Duration{"a": 0}, false,
	),
	// Not a required parameter so a bad value is translated to nil:
	Entry(
		"HealthTimeoutOverrides non-duration", "HealthTimeoutOverrides", "foo=bar", map[string]time.Duration(nil), false,
	),

	Entry("BPFForceTrackPacketsFromIfaces Empty", "BPFForceTrackPacketsFromIfaces", "", []string{"docker+"}),
	Entry(
		"BPFForceTrackPacketsFromIfaces Single valid entry", "BPFForceTrackPacketsFromIfaces", "docker0",
		[]string{"docker0"},
	),
	Entry(
		"BPFForceTrackPacketsFromIfaces Single valid entry", "BPFForceTrackPacketsFromIfaces", "cali-123",
		[]string{"cali-123"},
	),
	Entry(
		"BPFForceTrackPacketsFromIfaces Single valid wildcard", "BPFForceTrackPacketsFromIfaces", "docker+",
		[]string{"docker+"},
	),
	Entry(
		"BPFForceTrackPacketsFromIfaces Multiple valid entries", "BPFForceTrackPacketsFromIfaces", "docker0,docker1",
		[]string{"docker0", "docker1"},
	),
	Entry(
		"BPFForceTrackPacketsFromIfaces Single invalid entry", "BPFForceTrackPacketsFromIfaces", "cali@123",
		[]string{"docker+"},
	),
	Entry(
		"BPFForceTrackPacketsFromIfaces Multiple invalid entries", "BPFForceTrackPacketsFromIfaces",
		"cali-123,cali@123", []string{"docker+"},
	),
...
var _ = Describe(
	"DatastoreConfig tests", func() {
		var c *config.Config
		Describe(
			"with IPIP enabled", func() {
				BeforeEach(
					func() {
						c = config.New()
						c.DatastoreType = "k8s"
				...
				It(
					"should leave node polling enabled", func() {
		...
		Describe(
			"with the configuration set only from the common calico configuration", func() {
				BeforeEach(
					func() {
						c = config.New()
						c.SetLoadClientConfigFromEnvironmentFunction(
							func() (*apiconfig.CalicoAPIConfig, error) {
				...
				It(
					"sets the configuration options", func() {
		...
		Describe(
			"without setting the DatastoreType and setting the etcdv3 suboptions through the felix configuration",
			func() {
				BeforeEach(
					func() {
						c = config.New()
		...
		Describe(
			"with the configuration set from the common calico configuration and the felix configuration", func() {
				BeforeEach(
					func() {
						c = config.New()
				...
				It(
					"sets the configuration to what the felix configuration is", func() {
						spec := c.DatastoreConfig().Spec
						Expect(spec.DatastoreType).To(Equal(apiconfig.EtcdV3))
						Expect(spec.EtcdEndpoints).To(Equal("http://localhost:1234/"))
						Expect(spec.EtcdKeyFile).To(Equal(testutils.TestDataFile("etcdkeyfile.key")))
...
var _ = DescribeTable(
	...
	Entry(
		"just one TLS setting", map[string]string{
			"TyphaKeyFile": "/usr",
		}, false,
	),
	Entry(
		"TLS certs and key but no CN or URI SAN", map[string]string{
			"TyphaKeyFile":  "/usr",
			"TyphaCertFile": "/usr",
			"TyphaCAFile":   "/usr",
		}, false,
	...
	Entry(
		"TLS certs and key and CN but no URI SAN", map[string]string{
			"TyphaKeyFile":  "/usr",
			"TyphaCertFile": "/usr",
			"TyphaCAFile":   "/usr",
			"TyphaCN":       "typha-peer",
	...
	Entry(
		"TLS certs and key and URI SAN but no CN", map[string]string{
			"TyphaKeyFile":  "/usr",
			"TyphaCertFile": "/usr",
			"TyphaCAFile":   "/usr",
	...
	Entry(
		"all Felix-Typha TLS params", map[string]string{
			"TyphaKeyFile":  "/usr",
			"TyphaCertFile": "/usr",
			"TyphaCAFile":   "/usr",
			"TyphaCN":       "typha-peer",
	...
	Entry(
		"valid OpenstackRegion", map[string]string{
			"OpenstackRegion": "region1",
		}, true,
	),
	Entry(
		"OpenstackRegion with uppercase", map[string]string{
			"OpenstackRegion": "RegionOne",
		}, false,
	),
	Entry(
		"OpenstackRegion with slash", map[string]string{
			"OpenstackRegion": "us/east",
		}, false,
	),
	Entry(
		"OpenstackRegion with underscore", map[string]string{
			"OpenstackRegion": "my_region",
		}, false,
	),
	Entry(
		"OpenstackRegion too long", map[string]string{
			"OpenstackRegion": "my-region-has-a-very-long-and-extremely-interesting-name",
		}, false,
	),
	Entry(
		"valid RouteTableRange", map[string]string{
			"RouteTableRange": "1-250",
		}, true,
	),
	Entry(
		"invalid RouteTableRange", map[string]string{
			"RouteTableRange": "1-255",
		}, false,
	),
	Entry(
		"valid RouteTableRanges", map[string]string{
			"RouteTableRanges": "1-10000",
		}, true,
	),
	// 0xFFFFFFFF + 1
	Entry(
		"overflowing RouteTableRanges", map[string]string{
			"RouteTableRanges": "4294967295-4294967296",
		}, false,
	),
	// exceeds max allowed number of individual tables
	Entry(
		"excessive RouteTableRanges", map[string]string{
			"RouteTableRanges": "1-100000000",
		}, false,
	),
	Entry(
		"invalid RouteTableRanges", map[string]string{
...
var _ = DescribeTable(
	...
	Entry(
		"empty exclude list", "", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	Entry(
		"non-regexp single value", "kube-ipvs0", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	Entry(
		"non-regexp multiple values", "kube-ipvs0,veth1", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
			regexp.MustCompile("^veth1$"),
		},
	),
	Entry(
		"regexp single value", "/^veth.*/", []*regexp.Regexp{
			regexp.MustCompile("^veth.*"),
		},
	),
	Entry(
		"regexp multiple values", "/veth/,/^kube.*/", []*regexp.Regexp{
			regexp.MustCompile("veth"),
			regexp.MustCompile("^kube.*"),
		},
	),
	Entry(
		"both non-regexp and regexp values", "kube-ipvs0,/veth/,/^kube.*/", []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
			regexp.MustCompile("veth"),
			regexp.MustCompile("^kube.*"),
		},
	...
	Entry(
		"invalid non-regexp value", `not.a.valid.interf@e!!`, []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
	Entry(
		"invalid regexp value", `/^kube\K/`, []*regexp.Regexp{
			regexp.MustCompile("^kube-ipvs0$"),
		},
	),
)
...
var _ = Describe(
	"Config copy tests", func() {
		var conf *config.Config

		BeforeEach(
			func() {
				conf = config.New()
				changed, err := conf.UpdateFrom(
					map[string]string{
		...
		It(
			"should copy via protobuf correctly FromConfigUpdate", func() {
				pb := conf.ToConfigUpdate()
...
var _ = DescribeTable(
	...
	func(a, b any, expected bool) {
		Expect(config.SafeParamsEqual(a, b)).To(
			Equal(expected), fmt.Sprintf("SafeParamsEqual(%v, %v) != expected (%v)", a, b, expected),
		)
		Expect(config.SafeParamsEqual(b, a)).To(
			Equal(expected), fmt.Sprintf("SafeParamsEqual(%v, %v) != expected (%v)", b, a, expected),
		)
	},
	Entry("empty", "", "", true),
