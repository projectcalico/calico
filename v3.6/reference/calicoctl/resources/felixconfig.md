...
| interfaceExclude                   | A comma-separated list of interface names that should be excluded when Felix is resolving host endpoints.  The default value ensures that Felix ignores Kubernetes' internal `kube-ipvs0` device. If you want to exclude multiple interface names using a single value, the list supports regular expressions, e.g. to exclude all interfaces that begin with `kube`, simply include an entry `/^kube/`. | string(regex) | string(regex) | `kube-ipvs0` |
