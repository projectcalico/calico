...
func configureLogging() {
	err := log.Configure(log.Options{
		Component:      "mocknode",
		Screen:         &log.ScreenConfig{Level: log.InfoLevel},
		SingleThreaded: true,
		Counters:       log.Counters{WriteErrors: counterLogErrors},
	})
	if err != nil {
		log.WithError(err).Warn("Failed to configure logging")
	}
}
