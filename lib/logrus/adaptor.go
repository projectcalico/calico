...
// Package logrus adapts a *logrus.Logger to the lib/std/log.Logger interface.
//
...
func New(l *logrus.Logger) log.Logger {
	if l == nil {
		panic("logrus.New: logger is nil")
	}
...
// the opposite ordering (Error=2, Warn=3, Info=4, Debug=5) — so a direct
// cast would silently produce wrong answers.
