...
import (
	"errors"
	stdassert "github.com/projectcalico/calico/lib/std/testutils/assert"
	"os"
	"testing"
	"time"
...
func TestFirstAndIntervalLogging(t *testing.T) {
	...
	for _, entry := range entries {
		t.Run(entry.name, func(t *testing.T) {
			...
			stdassert.Equal(t, counter.count, 2)
			assert.Contains(t, counter.entry.Fields(), "a")
			assert.Equal(t, 1, counter.entry.Fields()["a"])
			assert.Contains(t, counter.entry.Fields(), "b")
			assert.Equal(t, 2, counter.entry.Fields()["b"])
			assert.Contains(t, counter.entry.Fields(), "c")
			assert.Equal(t, "3", counter.entry.Fields()["c"])
			assert.NotContains(t, counter.entry.Fields(), "logsSkipped")
			assert.Contains(t, counter.entry.Fields(), "nextLog")
			assert.Contains(t, counter.entry.Fields(), "error")
			...
			assert.Contains(t, counter.entry.Fields(), "c")
			assert.Equal(t, "3", counter.entry.Fields()["c"])
			assert.Contains(t, counter.entry.Fields(), "logsSkipped")
			assert.Equal(t, 2, counter.entry.Fields()["logsSkipped"])
			assert.Contains(t, counter.entry.Fields(), "nextLog")
			assert.Contains(t, counter.entry.Fields(), "error")
			...
			assert.NotContains(t, counter.entry.Fields(), "nextLog")
			assert.NotContains(t, counter.entry.Fields(), "logsSkipped")
