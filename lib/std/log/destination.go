...
// close terminates the background goroutine. Safe only when nothing else is
// sending on the channel.
func (d *destination) close() { close(d.channel) }
...
func (h *backgroundHook) Fire(entry *logrus.Entry) (err error) {
	...
	if entry.Level >= logrus.DebugLevel && h.debugFileNameRE != nil {
		...
		if frame == nil || !h.debugFileNameRE.MatchString(baseName(frame.File)) {
			return nil
		}
	}
