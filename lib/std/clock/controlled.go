...
//
// This clock is thread safe, all calls that read and write from and to the internal state are protected by a mutex.
...
// clock is a helper structure for tests that need control over
type controlledClock struct {
	...
	// timers is a map of all the timers that have been created by this clock. The key is the timer ID. Whenever the
...
func (t *controlledTimer) notifyTimeChange() {
	...
	if t.next <= t.clock.now {
		...
		if t.isTicker {
			...
			// (t.start + d*interval) > t.clock.now is true.
