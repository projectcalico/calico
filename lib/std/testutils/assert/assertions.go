...
// Package assert provides assertion functions for testing without importing third party libraries. This allows us to
// add testing utilities to the modules in the lib folder without creating possibly conflicting dependencies on the
// projects that import them.
//
// The utilities are modeled after the testify assertion library.
...
func ErrorIs(t *testing.T, expectedErr error, err error, msgAndArgs ...interface{}) bool {
	...
	if !errors.Is(err, expectedErr) {
		return Fail(t, fmt.Sprintf("Expected error to be '%v', received '%v'.", expectedErr, err), msgAndArgs...)
	}
	return true
}
