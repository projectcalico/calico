...
var _ = testutils.E2eDatastoreDescribe("FelixConfiguration tests", testutils.DatastoreAll, func(config apiconfig.CalicoAPIConfig) {
	...
	DescribeTable("FelixConfiguration e2e CRUD tests",
		func(name1, name2 string, spec1, spec2 apiv2.FelixConfigurationSpec) {
			...
			res, outError = c.FelixConfigurations().Create(ctx, &apiv2.FelixConfiguration{
			...
			res1, outError := c.FelixConfigurations().Create(ctx, &apiv2.FelixConfiguration{
				ObjectMeta: metav1.ObjectMeta{Name: name1},
				Spec:       spec1,
			}, options.SetOptions{})
			Expect(outError).NotTo(HaveOccurred())
			testutils.ExpectResource(res1, apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec1)
			...
			Expect(outError).NotTo(HaveOccurred())
			testutils.ExpectResource(res, apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec1)
			...
			testutils.ExpectResource(&outList.Items[0], apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec1)

			By("Creating a new FelixConfiguration with name2/spec2")
			res2, outError := c.FelixConfigurations().Create(ctx, &apiv2.FelixConfiguration{
				ObjectMeta: metav1.ObjectMeta{Name: name2},
				Spec:       spec2,
			}, options.SetOptions{})
			Expect(outError).NotTo(HaveOccurred())
			testutils.ExpectResource(res2, apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name2, spec2)
			...
			testutils.ExpectResource(&outList.Items[0], apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec1)
			testutils.ExpectResource(&outList.Items[1], apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name2, spec2)
			...
			res, outError = c.FelixConfigurations().Update(ctx, res1, options.SetOptions{})
			Expect(outError).To(HaveOccurred())
			...
			Expect(outError).NotTo(HaveOccurred())
			testutils.ExpectResource(res, apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec2)
			...
			Expect(outError).NotTo(HaveOccurred())
			Expect(outList.Items).To(HaveLen(2))
			testutils.ExpectResource(&outList.Items[0], apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec2)
			testutils.ExpectResource(&outList.Items[1], apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name2, spec2)
			...
			Expect(outError).NotTo(HaveOccurred())
			testutils.ExpectResource(dres, apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name1, spec2)
			...
			if config.Spec.DatastoreType != apiconfig.Kubernetes {
				...
				_, outError = c.FelixConfigurations().Create(ctx, &apiv2.FelixConfiguration{
					ObjectMeta: metav1.ObjectMeta{Name: name2},
					Spec:       spec2,
			...
			if config.Spec.DatastoreType == apiconfig.Kubernetes {
				...
				Expect(outError).NotTo(HaveOccurred())
				testutils.ExpectResource(dres, apiv2.KindFelixConfiguration, testutils.ExpectNoNamespace, name2, spec2)
			}
	...
	Describe("FelixConfiguration watch functionality", func() {
		It("should handle watch events for different resource versions and event types", func() {
			...
			testWatcher1.ExpectEvents(apiv2.KindFelixConfiguration, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes2,
				},
			...
			Expect(err).NotTo(HaveOccurred())
			testWatcher2.ExpectEvents(apiv2.KindFelixConfiguration, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
			...
			testWatcher2_1.ExpectEvents(apiv2.KindFelixConfiguration, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
			...
			testWatcher3.ExpectEvents(apiv2.KindFelixConfiguration, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes3,
				},
			...
			testWatcher4.ExpectEvents(apiv2.KindFelixConfiguration, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
