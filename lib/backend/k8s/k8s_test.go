...
import (
	...
	capiv3 "github.com/projectcalico/libcalico-go/lib/apis/v3"
...
var (
	...
	calicoAllowPolicyModelSpec = capiv3.GlobalNetworkPolicySpec{
		Order: &zeroOrder,
		Ingress: []capiv3.Rule{
			{
				Action: "allow",
	...
	calicoDisallowPolicyModelSpec = capiv3.GlobalNetworkPolicySpec{
		Order: &zeroOrder,
		Ingress: []capiv3.Rule{
			{
				Action: "deny",
...
var _ = Describe("Test Syncer API for Kubernetes backend", func() {
	...
	It("should handle a Namespace with DefaultDeny (v1beta annotation for namespace isolation)", func() {
		...
		By("Performing a List of Profiles", func() {
			_, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindProfile}, "")
			Expect(err).NotTo(HaveOccurred())
		})

		By("Performing a List of Policies", func() {
			_, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindNetworkPolicy, Namespace: "test-syncer-namespace-default-deny"}, "")
			Expect(err).NotTo(HaveOccurred())
		})

		By("Performing a Get on the Profile and ensure no error in the Calico API", func() {
			_, err := c.Get(ctx, model.ResourceKey{Name: fmt.Sprintf("kns.%s", ns.ObjectMeta.Name), Kind: capiv3.KindProfile}, "")
			Expect(err).NotTo(HaveOccurred())
		})
	...
	It("should handle a Namespace without any annotations", func() {
		...
		By("listing Policies", func() {
			_, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindNetworkPolicy, Namespace: "test-syncer-namespace-no-default-deny"}, "")
			Expect(err).NotTo(HaveOccurred())
		})
	...
	It("should handle a basic NetworkPolicy", func() {
		...
		// Perform a List and ensure it shows up in the Calico API.
		_, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindNetworkPolicy}, "")
		Expect(err).NotTo(HaveOccurred())
	...
	It("should handle a CRUD of Global Network Policy", func() {
		...
		gnpClient := c.getResourceClientFromResourceKind(capiv3.KindGlobalNetworkPolicy)
		kvp1Name := "my-test-gnp"
		kvp1KeyV1 := model.PolicyKey{Name: kvp1Name}
		kvp1a := &model.KVPair{
			Key: model.ResourceKey{Name: kvp1Name, Kind: capiv3.KindGlobalNetworkPolicy},
			Value: &capiv3.GlobalNetworkPolicy{
				TypeMeta: metav1.TypeMeta{
					Kind:       capiv3.KindGlobalNetworkPolicy,
					APIVersion: capiv3.GroupVersionCurrent,
				},
				ObjectMeta: metav1.ObjectMeta{
					Name: kvp1Name,
				},
		...
		kvp2a := &model.KVPair{
			Key: model.ResourceKey{Name: kvp2Name, Kind: capiv3.KindGlobalNetworkPolicy},
			Value: &capiv3.GlobalNetworkPolicy{
				TypeMeta: metav1.TypeMeta{
					Kind:       capiv3.KindGlobalNetworkPolicy,
					APIVersion: capiv3.GroupVersionCurrent,
		...
		By("Getting a Global Network Policy that does noe exist", func() {
			_, err := c.Get(ctx, model.ResourceKey{Name: "my-non-existent-test-gnp", Kind: capiv3.KindGlobalNetworkPolicy}, "")
			Expect(err).To(HaveOccurred())
		})

		By("Listing a missing Global Network Policy", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Name: "my-non-existent-test-gnp", Kind: capiv3.KindGlobalNetworkPolicy}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(0))
		})

		By("Getting an existing Global Network Policy", func() {
			kvp, err := c.Get(ctx, model.ResourceKey{Name: "my-test-gnp", Kind: capiv3.KindGlobalNetworkPolicy}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvp.Key.(model.ResourceKey).Name).To(Equal("my-test-gnp"))
			Expect(kvp.Value.(*capiv3.GlobalNetworkPolicy).Spec).To(Equal(kvp1b.Value.(*capiv3.GlobalNetworkPolicy).Spec))
		})

		By("Listing all Global Network Policies", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindGlobalNetworkPolicy}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(1))
			Expect(kvps.KVPairs[len(kvps.KVPairs)-1].Key.(model.ResourceKey).Name).To(Equal("my-test-gnp"))
			Expect(kvps.KVPairs[len(kvps.KVPairs)-1].Value.(*capiv3.GlobalNetworkPolicy).Spec).To(Equal(kvp1b.Value.(*capiv3.GlobalNetworkPolicy).Spec))
		})
	...
	It("should handle a CRUD of BGP Peer", func() {
		kvp1a := &model.KVPair{
			Key: model.ResourceKey{
				Name: "10-0-0-1",
				Kind: capiv3.KindBGPPeer,
			},
			Value: &capiv3.BGPPeer{
				TypeMeta: metav1.TypeMeta{
					Kind:       capiv3.KindBGPPeer,
					APIVersion: capiv3.GroupVersionCurrent,
				},
		...
		By("Getting a missing BGP Peer", func() {
			_, err := c.Get(ctx, model.ResourceKey{Name: "1-1-1-1", Kind: capiv3.KindBGPPeer}, "")
			Expect(err).To(HaveOccurred())
		})

		By("Listing a missing BGP Peer", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Name: "aa-bb-cc-dd-ee", Kind: capiv3.KindBGPPeer}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(0))
		})

		By("Listing an explicit BGP Peer", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Name: "10-0-0-1", Kind: capiv3.KindBGPPeer}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(1))
			Expect(kvps.KVPairs[0].Key).To(Equal(kvp1b.Key))
			Expect(kvps.KVPairs[0].Value.(*capiv3.BGPPeer).ObjectMeta.Name).To(Equal(kvp1b.Value.(*capiv3.BGPPeer).ObjectMeta.Name))
			Expect(kvps.KVPairs[0].Value.(*capiv3.BGPPeer).Spec).To(Equal(kvp1b.Value.(*capiv3.BGPPeer).Spec))
		})

		By("Listing all BGP Peers (should be 2)", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindBGPPeer}, "")
			Expect(err).ToNot(HaveOccurred())
			...
			for _, k := range kvps.KVPairs {
				...
				vals = append(vals, k.Value.(*capiv3.BGPPeer).Spec)
			}
			Expect(keys).To(ContainElement(kvp1b.Key))
			Expect(keys).To(ContainElement(kvp2b.Key))
			Expect(vals).To(ContainElement(kvp1b.Value.(*capiv3.BGPPeer).Spec))
			Expect(vals).To(ContainElement(kvp2b.Value.(*capiv3.BGPPeer).Spec))
	...
	It("should handle a CRUD of Node BGP Peer", func() {
		...
		defer func() {
			...
			if peers, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindBGPPeer}, ""); err == nil {
		...
		By("Listing all Nodes to find a suitable Node name", func() {
			nodes, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindNode}, "")
			Expect(err).NotTo(HaveOccurred())
		...
		By("Listing Node BGP Peers should contain Node name", func() {
			...
			for _, kvp := range kvps.KVPairs {
				Expect(kvp.Value.(*capiv3.BGPPeer).Spec.Node).To(Equal(nodename))
			}
		})

		By("Listing an explicit Node BGP Peer", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Name: peername1, Kind: capiv3.KindBGPPeer}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(1))
			Expect(kvps.KVPairs[0].Key).To(Equal(kvp1b.Key))
			Expect(kvps.KVPairs[0].Value.(*capiv3.BGPPeer).ObjectMeta.Name).To(Equal(kvp1b.Value.(*capiv3.BGPPeer).ObjectMeta.Name))
	...
	It("should handle a basic Pod", func() {
		...
		By("Performing a List() operation", func() {
			...
			weps, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindWorkloadEndpoint}, "")
			Expect(err).NotTo(HaveOccurred())
		...
		By("Performing a List(Name=wepName) operation", func() {
			...
			weps, err := c.List(ctx, model.ResourceListOptions{Name: wepName, Namespace: "default", Kind: capiv3.KindWorkloadEndpoint}, "")
			Expect(err).NotTo(HaveOccurred())
		...
		By("Performing a Get() operation", func() {
			// Perform a Get and ensure no error in the Calico API.
			wep, err := c.Get(ctx, model.ResourceKey{Name: wepName, Namespace: "default", Kind: capiv3.KindWorkloadEndpoint}, "")
			Expect(err).NotTo(HaveOccurred())
			fmt.Printf("Updating Wep %+v\n", wep.Value.(*capiv3.WorkloadEndpoint).Spec)
	...
	It("should support setting and getting FelixConfig", func() {
		fc := &model.KVPair{
			Key: model.ResourceKey{
				Name: "myfelixconfig",
				Kind: capiv3.KindFelixConfiguration,
			},
			Value: &capiv3.FelixConfiguration{
				TypeMeta: metav1.TypeMeta{
					Kind:       capiv3.KindFelixConfiguration,
					APIVersion: capiv3.GroupVersionCurrent,
				},
				...
				Spec: capiv3.FelixConfigurationSpec{
		...
		By("creating a new object", func() {
			...
			Expect(fc.Value.(*capiv3.FelixConfiguration).GetObjectMeta().GetResourceVersion()).To(Equal(""))
			fc.Value.(*capiv3.FelixConfiguration).GetObjectMeta().SetResourceVersion(updFC.Value.(*capiv3.FelixConfiguration).GetObjectMeta().GetResourceVersion())
			Expect(updFC.Value.(*capiv3.FelixConfiguration)).To(Equal(fc.Value.(*capiv3.FelixConfiguration)))
			...
			fc.Value.(*capiv3.FelixConfiguration).GetObjectMeta().SetResourceVersion("")
		})

		By("getting an existing object", func() {
			updFC, err = c.Get(ctx, fc.Key, "")
			Expect(err).NotTo(HaveOccurred())
			Expect(updFC.Value.(*capiv3.FelixConfiguration).Spec).To(Equal(fc.Value.(*capiv3.FelixConfiguration).Spec))
			Expect(updFC.Key.(model.ResourceKey).Name).To(Equal("myfelixconfig"))
			Expect(updFC.Revision).NotTo(BeNil())
		})

		By("updating an existing object", func() {
			updFC.Value.(*capiv3.FelixConfiguration).Spec.InterfacePrefix = "someotherprefix-"
			updFC, err = c.Update(ctx, updFC)
			Expect(err).NotTo(HaveOccurred())
			Expect(updFC.Value.(*capiv3.FelixConfiguration).Spec.InterfacePrefix).To(Equal("someotherprefix-"))
		})
		...
		By("applying an existing object", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(updFC.Value.(*capiv3.FelixConfiguration).Spec.InterfacePrefix).To(Equal("somenewprefix-"))
		})
	...
	It("should support setting and getting IP Pools", func() {
		By("listing IP pools when none have been created", func() {
			_, err := c.List(ctx, model.ResourceListOptions{Kind: capiv3.KindIPPool}, "")
			Expect(err).NotTo(HaveOccurred())
		})
		...
		By("creating an IP Pool and getting it back", func() {
			...
			pool := &model.KVPair{
				...
				Value: &capiv3.IPPool{
					...
					Spec: capiv3.IPPoolSpec{
						CIDR:     cidr,
						IPIPMode: capiv3.IPIPModeCrossSubnet,
						Disabled: true,
					},
				},
			}
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(receivedPool.Value.(*capiv3.IPPool).Spec.CIDR).To(Equal(cidr))
			Expect(receivedPool.Value.(*capiv3.IPPool).Spec.IPIPMode).To(BeEquivalentTo(capiv3.IPIPModeCrossSubnet))
			Expect(receivedPool.Value.(*capiv3.IPPool).Spec.Disabled).To(Equal(true))
		})
	...
	It("Should support getting, deleting, and listing Nodes", func() {
		...
		By("Listing a specific Node", func() {
			nodes, err := c.List(ctx, model.ResourceListOptions{Name: nodeHostname, Kind: capiv3.KindNode}, "")
			Expect(err).NotTo(HaveOccurred())
		...
		By("Listing a specific invalid Node", func() {
			nodes, err := c.List(ctx, model.ResourceListOptions{Name: "foobarbaz-node", Kind: capiv3.KindNode}, "")
			Expect(err).NotTo(HaveOccurred())
			Expect(nodes.KVPairs).To(HaveLen(0))
		})

		By("Getting a specific nodeHostname", func() {
			n, err := c.Get(ctx, model.ResourceKey{Name: nodeHostname, Kind: capiv3.KindNode}, "")
			Expect(err).NotTo(HaveOccurred())
		...
		By("Getting non-existent Node", func() {
			_, err := c.Get(ctx, model.ResourceKey{Name: "Fake", Kind: capiv3.KindNode}, "")
			Expect(err).To(HaveOccurred())
		})
		...
		By("Updating changes to a node", func() {
			...
			testKvp := model.KVPair{
				Key: model.ResourceKey{
					...
					Kind: capiv3.KindNode,
				},
				Value: &capiv3.Node{
					ObjectMeta: metav1.ObjectMeta{
						Name: kvp.Key.(model.ResourceKey).Name,
					},
					Spec: capiv3.NodeSpec{
						BGP: &capiv3.NodeBGPSpec{
							ASNumber:    &newAsn,
							IPv4Address: ip,
						},
					},
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(*node.Value.(*capiv3.Node).Spec.BGP.ASNumber).To(Equal(newAsn))

			// Also check that Get() returns the changes
			getNode, err := c.Get(ctx, kvp.Key.(model.ResourceKey), "")
			Expect(err).NotTo(HaveOccurred())
			Expect(*getNode.Value.(*capiv3.Node).Spec.BGP.ASNumber).To(Equal(newAsn))
			Expect(getNode.Value.(*capiv3.Node).Spec.BGP.IPv4IPIPTunnelAddr).To(Equal("10.10.10.1"))
		...
		By("Updating a Node", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(node.Value.(*capiv3.Node).Spec.BGP.ASNumber).To(BeNil())

			// Also check that Get() returns the changes
			getNode, err := c.Get(ctx, kvp.Key.(model.ResourceKey), "")
			Expect(err).NotTo(HaveOccurred())
			Expect(getNode.Value.(*capiv3.Node).Spec.BGP.ASNumber).To(BeNil())
		})
