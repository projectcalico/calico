...
// the second image-loading strategy in pkg/kind (the first is
// pkg/kind/imagepuller).
//
// Compared to the imagepuller webhook approach, the local registry:
//
...
//	    DisableImagePuller:      true,  // mutually exclusive with the registry today
//	})
//	if err != nil { t.Fatal(err) }
//
...
type Handle struct {
	cfg     Config
	log     *log.Entry
	created bool // true if Setup created the container (vs reusing an existing one)
}
...
func Setup(ctx context.Context, cfg Config) (*Handle, error) {
	...
	h := &Handle{cfg: cfg, log: log.WithField("component", "local-registry")}
	...
	case !exists:
		...
		h.created = true
	case !running:
		h.log.WithField("name", cfg.Name).Info("starting existing container")
		if err := h.start(ctx); err != nil {
			return nil, err
		}
	default:
		h.log.WithField("name", cfg.Name).Info("reusing already-running container")
	}
...
func (h *Handle) Attach(ctx context.Context, network string) error {
	...
	if err == nil {
		h.log.WithFields(log.Fields{"name": h.cfg.Name, "network": network}).Info("attached to docker network")
		return nil
	}
	...
	case strings.Contains(lower, "not found"), strings.Contains(lower, "no such network"):
		h.log.WithField("network", network).Info("docker network not present yet — skipping attach")
		return nil
	}
	return fmt.Errorf("docker network connect %s %s: %s: %w", network, h.cfg.Name, msg, err)
}

// ContainerdConfigPatches returns the containerd config patches the
// kind cluster needs to route image pulls through this registry. The
// returned slice always includes a `localhost:<Port>` mirror so pods
// that reference local-registry refs explicitly (e.g. via Push +
// LocalRef) work. Pass any upstream registry hostnames (e.g.
// "quay.io", "docker.io") to additionally mirror those upstreams —
// containerd will route pulls for those registries through this
// local registry. Combined with ProxyRemoteURL on Config, this lets
// pods reference upstream image refs directly while transparently
// pulling through this cache.
//
// Each patched mirror endpoint points at `http://<Name>:<5000>`,
// which is reachable inside the kind nodes once the registry is
// attached to the kind docker network (Attach).
func (h *Handle) ContainerdConfigPatches(upstreamMirrors ...string) []string {
	if h == nil {
		return nil
	}
	endpoint := fmt.Sprintf("http://%s:%d", h.cfg.Name, registryInsidePort)
	patches := make([]string, 0, 1+len(upstreamMirrors))
	patches = append(patches, mirrorPatch(fmt.Sprintf("localhost:%d", h.cfg.Port), endpoint))
	for _, registry := range upstreamMirrors {
		patches = append(patches, mirrorPatch(registry, endpoint))
	}
	return patches
}

func mirrorPatch(host, endpoint string) string {
	return fmt.Sprintf(`[plugins."io.containerd.grpc.v1.cri".registry.mirrors.%q]
  endpoint = [%q]`, host, endpoint)
}
...
func (h *Handle) Push(ctx context.Context, sourceRef string) (string, error) {
	...
	h.log.WithFields(log.Fields{"source": sourceRef, "target": target}).Info("push image")
...
func (h *Handle) Stop() error {
	...
	if h.cfg.Persist {
		h.log.WithField("name", h.cfg.Name).Info("leaving registry running (Persist=true)")
		return nil
	}
	h.log.WithField("name", h.cfg.Name).Info("removing registry container")
