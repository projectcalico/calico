...
// Calico can install its own, and the pod subnet the Calico installation
// expects. Callers that want different networking pass a populated
...
func Up(ctx context.Context, c Config) (*Cluster, error) {
	...
	if err := prov.Delete(c.Name, c.KubeconfigPath); err != nil {
		logger.WithError(err).Warn("Failed to delete pre-existing cluster, continuing with creation.")
	}
