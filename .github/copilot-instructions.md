...
key, or bug fix is not by itself any of those. For components
with a design directory (Felix uses `felix/design/`), the edit
goes in the sub-design covering the area, and/or the index itself
when the sub-design table or scope changes.

An edit that does belong is normally **one to three lines**,
edited into the section that already covers the area — not a new
heading, and never a paraphrase of the commit message or PR
description. Exemptions: bug fix restoring documented behaviour,
mechanical refactor, comment/log edits, dependency bump.
...
user; a human-driven agent should instead propose the edit and
wait for approval, per `design/MAINTAINING.md`.) A missing doc
update that the author explicitly considered and skipped is not a
