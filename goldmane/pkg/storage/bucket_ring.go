...
func (r *BucketRing) AddFlow(flow *types.Flow) {
	...
	if log.IsLevelEnabled(log.DebugLevel) {
		log.WithFields(bucket.Fields()).WithField("flowStart", flow.StartTime).WithField("head", r.headIndex).Debug("Adding flow to bucket")
	}
	bucket.AddFlow(flow)
}
...
func (r *BucketRing) flushToStreams() {
	...
	log.WithFields(bucket.Fields()).Debug("Flushing bucket to stream manager")
