...
func AddIPAM(conf types.NetConf, args *skel.CmdArgs, logger log.Entry) (*cniv1.Result, error) {
...
func DeleteIPAM(conf types.NetConf, args *skel.CmdArgs, logger log.Entry) error {
	...
	if conf.IPAM.Type == "host-local" {
		...
		logger.WithFields(log.Fields{"podCidrv4": dummyPodCidrv4,
...
//	  ...
//	}
func ReplaceHostLocalIPAMPodCIDRs(logger log.Entry, stdinData map[string]interface{}, getPodCIDRs func() (string, string, error)) error {
...
func replaceHostLocalIPAMPodCIDR(logger log.Entry, rawIpamData interface{}, getPodCidrs func() (string, string, error)) error {
...
func ReleaseIPAllocation(logger log.Entry, conf types.NetConf, args *skel.CmdArgs) {
