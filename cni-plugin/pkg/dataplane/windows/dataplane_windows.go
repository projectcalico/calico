...
func NewWindowsDataplane(conf types.NetConf, logger log.Logger) *windowsDataplane {
...
func SetupL2bridgeNetwork(networkName string, subNet *net.IPNet, logger log.Logger) (*hcsshim.HNSNetwork, error) {
...
func SetupVxlanNetwork(networkName string, subNet *net.IPNet, vni uint64, logger log.Logger) (*hcsshim.HNSNetwork, error) {
...
func ensureVxlanNetworkExists(networkName string, subNet *net.IPNet, vni uint64, logger log.Logger) (*hcsshim.HNSNetwork, error) {
	var err error
	createNetwork := true
...
func EnsureNetworkExists(networkName string, subNet *net.IPNet, logger log.Logger) (*hcsshim.HNSNetwork, error) {
	var err error
	createNetwork := true
...
func createAndAttachVxlanHostEP(epName string, hnsNetwork *hcsshim.HNSNetwork, subNet *net.IPNet, logger log.Logger) (*hcsshim.HNSEndpoint, error) {
	var err error
...
func CreateAndAttachHostEP(epName string, hnsNetwork *hcsshim.HNSNetwork, subNet *net.IPNet, logger log.Logger) (*hcsshim.HNSEndpoint, error) {
	var err error
	endpointAddress := getNthIP(subNet, 2)
	attachEndpoint := true
...
func chkMgmtIPandEnableForwarding(networkName string, hnsEndpoint *hcsshim.HNSEndpoint, logger log.Logger) (network *hcsshim.HNSNetwork, err error) {
	startTime := time.Now()
...
func enableForwarding(netInterface net.Interface, logger log.Logger) error {
...
func cleanUpEndpointByIP(IP net.IP, logger log.Logger, isDockerV1 bool) error {
	if isDockerV1 {
...
func cleanUpEndpointByName(endpointName string, logger log.Logger, isDockerV1 bool) error {
	if isDockerV1 {
...
func lookupManagementIface(mgmtIP net.IP, logger log.Logger) (net.Interface, error) {
...
func lookupManagementAddr(mgmtIP net.IP, logger log.Logger) (*net.IPNet, error) {
	interfaces, err := net.Interfaces()
	if err != nil {
		logger.WithError(err).Error("Failed to look up host interfaces")
		return nil, err
