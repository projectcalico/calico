...
func CmdAddK8s(ctx context.Context, args *skel.CmdArgs, conf types.NetConf, epIDs utils.WEPIdentifiers, calicoClient calicoclient.Interface, endpoint *libapi.WorkloadEndpoint) (*cniv1.Result, error) {
	...
	case ipAddrs == "" && ipAddrsNoIpam == "":
		result, reusedIPs, err = assignPodIPs(conf, args, endpoint, logger)
		if err != nil {
			return nil, err
		}
...
func assignPodIPs(
	...
	priorIPs := priorSandboxIPs(conf, endpoint, logger)
	if len(priorIPs) != 0 {
...
func priorSandboxIPs(conf types.NetConf, endpoint *libapi.WorkloadEndpoint, logger *logrus.Entry) []net.IP {
