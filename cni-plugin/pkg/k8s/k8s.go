...
func CmdAddK8s(ctx context.Context, args *skel.CmdArgs, conf types.NetConf, epIDs utils.WEPIdentifiers, calicoClient calicoclient.Interface, endpoint *libapi.WorkloadEndpoint) (*cniv1.Result, error) {
	...
	if annotation, ok := annot["cni.projectcalico.org/allowedSourceIPs"]; ok && annotation != "" {
		// Parse Annotation data
		requestedSourceIPs, err := parseIPAddrs(floatingIPs, logger)
		if err != nil {
			releaseIPAM()
			return nil, err
		}
		endpoint.Spec.AllowSpoofedSourceIPs = requestedSourceIPs
	}
