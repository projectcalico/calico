...
var _ = Describe("Kubernetes CNI tests", func() {
	...
	Context("using calico-ipam with the ipAddrs annotation", func() {
		...
		It("hands the requested IP over to a new sandbox of the same pod", func() {
			...
			attrs, handleID, err := calicoClient.IPAM().GetAssignmentAttributes(ctx, cnet.IP{IP: net.ParseIP(podIP)})
			Expect(err).NotTo(HaveOccurred())
			Expect(*handleID).To(ContainSubstring(containerIDY))
			Expect(attrs[ipam.AttributePod]).To(Equal(testPodName))
			Expect(attrs[ipam.AttributeNamespace]).To(Equal(testutils.K8S_TEST_NS))
		...
		It("tolerates kubelet retrying ADD with the same container ID", func() {
			...
			_, handleID, err = calicoClient.IPAM().GetAssignmentAttributes(ctx, cnet.IP{IP: net.ParseIP(podIP)})
			Expect(err).NotTo(HaveOccurred())
			Expect(*handleID).To(Equal(handle))
	...
	Context("after a pod has already been networked once", func() {
		...
		It("a second ADD should assign a new IP when another pod holds the endpoint's address", func() {
			...
			_, handleID, err := calicoClient.IPAM().GetAssignmentAttributes(ctx, oldIP)
			Expect(err).NotTo(HaveOccurred())
			Expect(*handleID).To(Equal(otherHandle))
	...
	Context("when the pod's IP pool has no spare capacity", func() {
		...
		It("rebuilds the sandbox on the pod's own address", func() {
			...
			_, _, err = calicoClient.IPAM().GetAssignmentAttributes(ctx, cnet.IP{IP: net.ParseIP(podIP)})
			Expect(err).To(HaveOccurred())
		})
	})
