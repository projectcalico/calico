...
# Rebuild images with stale stamps, load changed ones onto an existing kind cluster,
# and restart pods. Invalidates a stamp when any tracked source file under the
# component directory is newer than the stamp, then runs kind-build-images which
# only rebuilds stamps that were removed.
.PHONY: kind-reload
kind-reload: $(KUBECTL)
