...
$(REPO_ROOT)/.$(KIND_NAME).created: $(KUBECTL) $(KIND)
	...
	while ! KUBECONFIG=$(KIND_KUBECONFIG) $(KUBECTL) create -f $(REPO_ROOT)/charts/crd.projectcalico.org/templates/; do echo "Waiting for operator CRDs to be created"; sleep 2; done
	while ! KUBECONFIG=$(KIND_KUBECONFIG) $(KUBECTL) create -f $(REPO_ROOT)/charts/crd.projectcalico.org/templates/calico; do echo "Waiting for calico CRDs to be created"; sleep 2; done
	touch $@
