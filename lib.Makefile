...
#
# Usage:
#   make dev-push DEV_IMAGE_REGISTRY=docker.io DEV_IMAGE_PATH=caseydavenport DEV_IMAGE_TAG=my-feature
###############################################################################
DEV_IMAGE_TAG ?= $(GIT_VERSION)
...
ifndef DEV_IMAGE_PATH
	...
	@for img in $(KIND_CALICO_IMAGES); do \
	  base=$${img%%:*}; \
	  name=$${base#calico/}; \
	  docker tag $$base:latest-$(ARCH) $(DEV_IMAGE_PATH)/$$name:$(DEV_IMAGE_TAG); \
	  echo "Tagged $(DEV_IMAGE_PATH)/$$name:$(DEV_IMAGE_TAG)"; \
	done
	cd $(KIND_INFRA_DIR) && \
	  BRANCH=$(OPERATOR_BRANCH) \
	  DEV_IMAGE_TAG=$(DEV_IMAGE_TAG) \
	  ...
	  ./build-operator.sh
	@echo "All images tagged for $(DEV_IMAGE_PATH) with tag $(DEV_IMAGE_TAG)"
...
dev-push: dev-image
	...
	echo "Pushing $(DEV_OPERATOR_IMAGE)"
	docker push $(DEV_OPERATOR_IMAGE)
	@echo "All images pushed to $(DEV_IMAGE_PATH) with tag $(DEV_IMAGE_TAG)"
