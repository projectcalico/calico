...
func init() {
	...
	DescribeTable("NumOrStringJSONUnmarshaling",
		...
		Entry("should accept foo-bar as named port", "\"foo-bar\"", portType, namedPortFromString("foo-bar")),
	...
	DescribeTable("NumOrStringJSONMarshaling",
		...
		Entry("should marshal named port", namedPortFromString("foobar"), `"foobar"`),
...
func namedPortFromString(s string) numorstring.Port {
	p, _ := numorstring.NamedPort(s)
	return p
}
