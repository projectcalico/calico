...
type FelixConfigurationSpec struct {
	...
	// GoGCThreshold overrides the default Go garbage collection threshold percentage.
	// This is useful for tuning Felix's memory usage.  Go's default is to trigger GC after the heap grows by 100%.
	// Lower values will trigger GC more frequently (and hence use more CPU), higher values will trigger GC less frequently,
	// (using less CPU).  The relationship is non-linear; setting the value too small can trigger GC thrashing, where
	// the runtime spends more time garbage collecting than executing the application.  A special value of -1 can be used
	...
	// GoMemoryLimitMB sets a (soft) memory limit for the Go runtime.  The Go runtime will try to keep its memory usage
	// under the limit by triggering GC as needed.  To avoid thrashing, it will exceed the limit if GC starts to take
	// more than 50% of the process's CPU time.
	//
	...
	// +optional
	GoMemoryLimitMB *int `json:"goMemoryLimitMB,omitempty" validate:"omitempty,gte=0"`
}
