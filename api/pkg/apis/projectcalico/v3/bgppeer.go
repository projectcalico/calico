...
// +kubebuilder:validation:XValidation:rule="size(self.node) == 0 || size(self.nodeSelector) == 0",message="node and nodeSelector cannot both be set"
// +kubebuilder:validation:XValidation:rule="size(self.peerIP) == 0 || size(self.peerSelector) == 0",message="peerIP and peerSelector cannot both be set"
// +kubebuilder:validation:XValidation:rule="size(self.peerSelector) == 0 || !has(self.asNumber) || self.asNumber == 0",message="asNumber must be empty when peerSelector is set"
// +kubebuilder:validation:XValidation:rule="size(self.localWorkloadSelector) == 0 || size(self.peerIP) == 0",message="peerIP must be empty when localWorkloadSelector is set"
// +kubebuilder:validation:XValidation:rule="size(self.localWorkloadSelector) == 0 || size(self.peerSelector) == 0",message="peerSelector must be empty when localWorkloadSelector is set"
// +kubebuilder:validation:XValidation:rule="size(self.localWorkloadSelector) == 0 || (has(self.asNumber) && self.asNumber != 0)",message="asNumber is required when localWorkloadSelector is set"
