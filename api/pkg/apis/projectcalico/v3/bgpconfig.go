...
type BGPConfigurationSpec struct {
	...
	// The routes that BIRD does not program here are expected to be programmed by Felix instead.
	//
	// - Disabled: BIRD programs no cluster routes.
	// - EnabledNoEncapOnly: BIRD programs cluster routes for unencapsulated IP Pools, i.e. those with
	//   ipipMode: Never and vxlanMode: Never.
	// - EnabledIPIPOnly: BIRD programs cluster routes for IP Pools with ipipMode: Always or CrossSubnet.
	// - Enabled: BIRD programs cluster routes for both IPIP and unencapsulated IP Pools.
	//
