...
const (
	// ServiceLoadBalancerAggregationEnabled means individual /32 routes will be advertised for each LoadBalancer service
	ServiceLoadBalancerAggregationEnabled ServiceLoadBalancerAggregation = "Enabled"
	// ServiceLoadBalancerAggregationDisabled means the full CIDR range will be advertised (default behavior)
...
type BGPConfigurationSpec struct {
	...
	// When set to "Enabled", individual /32 routes are advertised for each service instead of
	// the full CIDR range. This is useful for anycast failover mechanisms where failed service
	// routes need to be withdrawn. [Default: Disabled]
