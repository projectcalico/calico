...
func TestControllers(t *testing.T) {
	...
	RunSpecs(t, "pkg/enterprise/controller Suite")
}
...
var _ = Describe("Controllers", func() {
	DescribeTable("contributes the Enterprise-only controllers",
		func(variant operatorv1.ProductVariant) {
			names := []string{}
			for _, c := range entcontroller.Controllers(variant) {
				Expect(c.Add).NotTo(BeNil())
				names = append(names, c.Name)
			}
			Expect(names).To(ConsistOf("Monitor", "ApplicationLayer", "EgressGateway", "PacketCapture", "OpenTelemetryCollector", "IntrusionDetection", "LogCollector", "LogStorage", "IstioWaypoint",
	...
	It("contributes nothing for Calico", func() {
		Expect(entcontroller.Controllers(operatorv1.Calico)).To(BeEmpty())
	})
})
