...
func (r *ReconcileInstallation) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if _, err := managedfields.Apply(ctx, r.fieldManager, r.declareFelixConfiguration(ctx, defaulted, needsNamespaceMigration)); err != nil {
	...
	felixConfiguration, err := managedfields.Apply(ctx, r.fieldManager, r.declareBPFEnabled(ctx, defaulted, needsNamespaceMigration))
	if err != nil {
		if !managedfields.IsFieldConflict(err) {
			r.status.SetDegraded(operatorv1.ResourceUpdateError, "Error updating FelixConfiguration", err, reqLogger)
			return reconcile.Result{}, err
	...
	if _, err := managedfields.Apply(ctx, r.fieldManager, r.declareBGPConfiguration(defaulted)); err != nil {
		if !managedfields.IsFieldConflict(err) {
	...
	if _, err := managedfields.Apply(ctx, r.fieldManager, r.declareBPFEnabled(ctx, defaulted, needsNamespaceMigration)); err != nil {
		if !managedfields.IsFieldConflict(err) {
