...
admission policy installation; once an Installation exists it is the authority on the variant.`,
	...
	ctrl.SetLogger(zap.New(zap.WriteTo(os.Stdout), zap.UseFlagOptions(&zapOpts)))
	...
	if !opts.acceptsVariant(operatortigeraiov1.ProductVariant(bootstrapVariant)) {
		fmt.Printf("Invalid -variant %q\n", bootstrapVariant)
		os.Exit(1)
	}

	if handled, err := opts.afterParse(); err != nil {
		fmt.Println(err)
		os.Exit(1)
	} else if handled {
		os.Exit(0)
	...
	if printImages != "" {
		cmpnts, ok := opts.imageSet(strings.ToLower(printImages))
		if !ok {
	...
	extensionRegistry, err := extensions.Build(ctx, opts.Extensions, variant, clientset, extensions.BuildOptions{ManageCRDs: manageCRDs, UseV3CRDs: v3CRDs})
	if err != nil {
...
// ShowCRDs prints the CRDs the operator has bundled for a variant, for a caller
// whose own print-and-exit flag covers the CRDs it registered.
