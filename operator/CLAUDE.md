...
- **`pkg/components/`** — Component image references. `calico.go` and `enterprise.go` are hand-written and take every version from ldflags; see `CALICO_LDFLAGS` in the Makefile.
