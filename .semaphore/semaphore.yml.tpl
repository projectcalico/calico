    ...
    jobs:
      ...
      - name: 'Mainline ST (DevStack + Tempest) on Ussuri'
        commands:
          ...
          # We would normally want to test with the latest Calico master code (i.e. for Felix and
          # networking-calico), but we recently lost support for Ubuntu Bionic, so for Ubuntu Bionic
          # we need to use a previous Calico release (v3.25).  Does that mean that this testing
          # doesn't belong on the master branch of the Calico repo?  Yes.  I'm just doing this here
          # for now in order to validate the theory.
          - export CALICO_PPA=calico-3.25
