  ...
  task:
    ...
    epilogue:
      always:
        ...
        - |
          for f in $(find report -name *.xml); do
            batch=$(basename $(dirname $f))
            test-results compile --name felix-$batch $f test-results-$batch.json || true
          done
          test-results combine test-results-*.json report.json || true
          artifact push job report.json -d test-results/junit.json || true
          artifact push workflow report.json -d test-results/${SEMAPHORE_PIPELINE_ID}/${SEMAPHORE_JOB_ID}.json || true
    secrets:
