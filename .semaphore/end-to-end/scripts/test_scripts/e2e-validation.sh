...
do
    echo [INFO] Checking "$file"
    # SC2148 is ignored because .semaphore/end-to-end/scripts/global_prologue.sh isn't actually a proper shell script
    # SC2155 is ignored because .semaphore/end-to-end/scripts/global_prologue.sh isn't actually a proper shell script and we need commands on a single line.
    # if shellcheck-v0.11.0/shellcheck --severity=warning -e SC2148 -e SC2155 "$file"; then
