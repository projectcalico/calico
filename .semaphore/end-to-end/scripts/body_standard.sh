...
if [[ "${HCP_ENABLED}" == "true" ]]; then
  ...
  if [[ ${MCM_STAGE:-} != *-mgmt* ]] && [[ ${HCP_STAGE:-} != *-hosting* ]]; then
    ...
    if [[ -n "$RUN_LOCAL_TESTS" ]]; then
      ...
      KUBECONFIG=${BZ_LOCAL_DIR}/kubeconfig PRODUCT=calico ginkgo -procs=${E2E_PROCS:-4} ./e2e/bin/k8s/e2e.test -- ${K8S_E2E_FLAGS} |& tee -a >(gzip --stdout > ${BZ_LOGS_DIR}/${TEST_TYPE}-tests.log.gz)
      popd
    else
