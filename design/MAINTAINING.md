...
**A warranted edit lands in the same PR as the code**, in the doc
covering the area — for a component with a design directory, the
sub-design rather than the index. Not a follow-up: the doc and the
code are reviewed together, or the doc is stale the day it merges.
...
User-facing install and upgrade instructions — the chart READMEs — are
**not** design docs; see
[`helm-charts.instructions.md`](../.github/instructions/helm-charts.instructions.md).
They must track every documented step, and there the safe default is
the opposite one: update.

The classic exemption list — bug fix restoring documented behaviour,
mechanical refactor, comment or log-message edits, dependency bump —
needs no separate statement. None of those falsifies a sentence or
introduces an invariant, so the default already covers them.
