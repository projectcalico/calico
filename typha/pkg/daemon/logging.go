...
func ConfigureEarlyLogging() {
	// Typha logs go to stdout (preserved from typha/pkg/logutils on
	// master). lib/std/log now defaults to stderr so binaries with
	// structured stdout don't get corrupted; long-running daemons whose
	// stdout is consumed (kubelet pod logs, FV stdout grep) opt in.
