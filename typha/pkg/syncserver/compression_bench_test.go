...
func gobEncodeUpdates(updates []api.Update) []byte {
	...
	// The algorithm value here doesn't affect compression — it's just
	// metadata inside the gob stream — so we use an empty string.
	envelope := syncproto.Envelope{
...
// TestCompressionComparison is an analysis helper (not a benchmark) that prints
// a human-readable comparison of snappy vs zstd compression on realistic Typha
// snapshot data at various sizes. It is skipped by default because it does
// substantial work and asserts nothing.
// Run explicitly with: COMPRESSION_ANALYSIS=true go test -v -run TestCompressionComparison ./typha/pkg/syncserver/
func TestCompressionComparison(t *testing.T) {
	if os.Getenv("COMPRESSION_ANALYSIS") == "" {
		t.Skip("skipping compression comparison analysis (set COMPRESSION_ANALYSIS=true to run)")
	}
