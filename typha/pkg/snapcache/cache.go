...
func New(config Config) *Cache {
	...
	c.counterUpdatesTotal = mustGetCounter(counterVecUpdatesTotal, config.Name)
	c.counterUpdatesSkipped = mustGetCounter(counterVecUpdatesSkipped, config.Name)
	c.gaugeCurrentSequenceNumber = mustGetGauge(gaugeVecCurrentSequenceNumber, config.Name)
	c.counterBreadcrumbNonBlock = mustGetCounter(counterVecBreadcrumbNonBlock, config.Name)
	c.counterBreadcrumbBlock = mustGetCounter(counterVecBreadcrumbBlock, config.Name)
...
func mustGetCounter(vec *prometheus.CounterVec, name string) prometheus.Counter {
	c, err := vec.GetMetricWithLabelValues(name)
	if err != nil {
		log.WithError(err).Panic("Bug: failed to get Prometheus counter.")
	}
	return c
}

func mustGetGauge(vec *prometheus.GaugeVec, name string) prometheus.Gauge {
	c, err := vec.GetMetricWithLabelValues(name)
	if err != nil {
		log.WithError(err).Panic("Bug: failed to get Prometheus counter.")
	}
