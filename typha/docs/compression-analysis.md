...
cd typha && go test -v -run TestCompressionComparison ./pkg/syncserver/

# Go benchmarks for throughput measurement
cd typha && go test -bench="Benchmark(Snappy|Zstd)" -benchmem -count=1 -benchtime=3s ./pkg/syncserver/

# Round-trip correctness verification
cd typha && go test -v -run TestCompressionRoundTrip ./pkg/syncserver/
```
