...
func (denyAllAuthorizer) AuthorizeTierOperation(_ context.Context, policyName, _ string) error {
	return k8serrors.NewForbidden(v3.Resource("stagednetworkpolicies"), policyName, errors.New("denied"))
}
...
func deleteCollectionContext() context.Context {
	...
	ctx = genericapirequest.WithRequestInfo(ctx, &genericapirequest.RequestInfo{
		...
		Namespace:         "ns1",
		Path:              "/apis/projectcalico.org/v3/namespaces/ns1/stagednetworkpolicies",
	})
	return ctx
}

// Regression test: bulk delete on namespaced StagedNetworkPolicy must fail
// with Forbidden when the caller has no tier authorization. Embedded
// *registry.Store is intentionally left nil.
