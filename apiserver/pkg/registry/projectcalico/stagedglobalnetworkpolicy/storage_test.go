...
func (denyAllAuthorizer) AuthorizeTierOperation(_ context.Context, policyName, _ string) error {
	return k8serrors.NewForbidden(v3.Resource("stagedglobalnetworkpolicies"), policyName, errors.New("denied"))
}
...
func deleteCollectionContext() context.Context {
	...
	ctx = genericapirequest.WithRequestInfo(ctx, &genericapirequest.RequestInfo{
		...
		Path:              "/apis/projectcalico.org/v3/stagedglobalnetworkpolicies",
	})
	return ctx
}

// Regression test: bulk delete on cluster-scoped StagedGlobalNetworkPolicy
// must fail with Forbidden when the caller has no tier authorization.
// Embedded *registry.Store is intentionally left nil.
