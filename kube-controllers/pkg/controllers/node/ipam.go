...
import (
	...
	"github.com/projectcalico/calico/api/pkg/apis/projectcalico/v3"
...
func (c *ipamController) handleClusterInformationUpdate(kvp model.KVPair) {
	if kvp.Value != nil {
		...
		// if there's no ClusterInformation CR object, should we still make the GC run ?
...
func (c *ipamController) syncIPAM() error {
	if !c.datastoreReady {
		return fmt.Errorf("datastore is locked, skipping ipam sync")
	}
