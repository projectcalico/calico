...
var _ = Describe("IPAM controller UTs", func() {
	...
	It("should not get stuck when node cleanup fails (issue #8643)", func() {
		...
		// The tunnel IP should be GC'd even though node cleanup fails.
		Eventually(func() bool {
			return fakeClient.handlesReleased[tunnelHandle]
		}, assertionTimeout, 100*time.Millisecond).Should(BeTrue(), "Tunnel IP handle should be released")

		// The node affinity should NOT be released because cleanup is failing.
		Consistently(func() bool {
			return fakeClient.affinityReleased("stuck-node")
		}, 2*time.Second, 100*time.Millisecond).Should(BeFalse(), "Node affinity should NOT be released when cleanup fails")

		// Clear the error — the retry controller will schedule another sync with backoff,
		// and the node should eventually be cleaned up.
		cli.(*FakeCalicoClient).SetReleaseHostAffinityError(nil)

		Eventually(func() bool {
			return fakeClient.affinityReleased("stuck-node")
		}, assertionTimeout, 100*time.Millisecond).Should(BeTrue(), "Node affinity should be released after error is cleared")
	})
})
