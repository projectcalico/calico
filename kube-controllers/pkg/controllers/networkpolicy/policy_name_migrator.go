...
func NewMigratorController(ctx context.Context, cs kubernetes.Interface, cli clientv3.Interface) controller.Controller {
	...
	ipamMigratorCtrl := &policyMigrator{
		ctx:           ctx,
		cli:           cli,
		cs:            cs,
	...
	ipamMigratorCtrl.syncer = watchersyncer.New(cli.(accessor).Backend(), resourceTypes, ipamMigratorCtrl)

	return ipamMigratorCtrl
}
...
func (c *policyMigrator) Run(stop chan struct{}) {
	logrus.Info("Starting IPAM Migrator Controller")
	...
	<-stop
	logrus.Info("Stopping IPAM Migrator Controller")
}
...
func (c *policyMigrator) processPendingWork() error {
	...
	for key := range c.pendingWork.All() {
		...
		if err != nil && !errors.IsAlreadyExists(err) {
			...
			return fmt.Errorf("error creating new policy %s: %v", newKey, err)
		}
		...
		if err != nil && !errors.IsNotFound(err) {
			...
			return fmt.Errorf("error deleting old policy %s: %v", k, err)
		}
