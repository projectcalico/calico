...
var _ = Describe("kube-controllers metrics and pprof FV tests", func() {
	...
	getFromContainerNS := func(url string) error {
		// Defensive check: kubectrls.Name is generated by the test harness, but
		// validate it matches a safe character set before interpolating into
		// the docker --network argument.
		if !regexp.MustCompile(`^[a-zA-Z0-9_.-]+$`).MatchString(kubectrls.Name) {
			return fmt.Errorf("unexpected container name: %q", kubectrls.Name)
		}
		cmd := exec.Command("docker", "run", "--rm",
			"--network=container:"+kubectrls.Name,
			// Restrict the sidecar's privileges: drop all Linux capabilities
			// and use a read-only root filesystem.  wget needs neither.
			"--cap-drop=ALL", "--read-only",
			"busybox", "wget", "-q", "-O", "/dev/null", "-T", "2", url)
