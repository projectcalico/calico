...
> **Note**: Typha uses **port 9091** TCP by default to publish its metrics. However, if {{site.prodname}} is installed using {% include open-new-window.html text='Amazon yaml file' url='https://github.com/aws/amazon-vpc-cni-k8s/blob/b001dc6a8fff52926ed9a93ee6c4104f02d365ab/config/v1.5/calico.yaml#L535-L536' %} this port will be 9093 as its set manually via **TYPHA_PROMETHEUSMETRICSPORT** environment variable.
   {: .alert .alert-warning}
...
> **Note**: Kubernetes namespaces guide can be {% include open-new-window.html text='found at this link' url='https://kubernetes.io/docs/tasks/administer-cluster/namespaces/' %}.
   {: .alert .alert-info}
...
> **Note**: A comprehensive guide to user roles and authentication can be {% include open-new-window.html text='found at this link' url='https://kubernetes.io/docs/reference/access-authn-authz/rbac/' %}.
   {: .alert .alert-info}
...
> **Note**: A comprehensive guide about configuration file can be {% include open-new-window.html text='found at this link' url='https://prometheus.io/docs/prometheus/latest/configuration/configuration/' %}.
   {: .alert .alert-info}
