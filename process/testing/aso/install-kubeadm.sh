...
function setup_kubeadm_cluster() {
  ...
  echo "Using internal IP ${LOCAL_IP_ENV} for cluster communication"
  echo "Adding external IP ${EXTERNAL_IP_ENV} to API server certificate"
  ${MASTER_CONNECT_COMMAND} "~/init-cluster.sh ${LOCAL_IP_ENV} 10.244.0.0/16 10.96.0.0/12 ${EXTERNAL_IP_ENV}"
