...
while [[ $i -lt $vmss_length ]]; do
  ...
  name=$(echo "$vm" | cut -d" " -f 1)
  is_linux=$(echo "$vm" | cut -d" " -f 2)
  if [[ $is_linux == "true" ]]; then
    LINUX_NAMES="$LINUX_NAMES $(echo "$name" | tr -d '"')"
  fi
  is_windows=$(echo "$vm" | cut -d" " -f 3)
...
CAPZ_WINDOWS_IPS=""
for name in $CAPZ_WINDOWS_IPS; do
  ip=$(az vmss nic list -g "$AZURE_RESOURCE_GROUP" --vmss-name "$name" | jq -r .[].ipConfigurations[].privateIPAddress)
...
CAPZ_K8S_VERSION_MAJOR=$(${KUBECTL} version -o json | jq -r ".serverVersion.major")
CAPZ_K8S_VERSION_MINOR=$(${KUBECTL} version -o json | jq -r ".serverVersion.minor")

if [ "$CAPZ_K8S_VERSION_MAJOR" -eq 1 ] && [ "$CAPZ_K8S_VERSION_MINOR" -le 29 ]; then
