...
# from the cross-block GCS cache as :latest-amd64; we re-tag them to test-build
# to match what the operator renders. calico/operator:test-build is built by
# run-win-fv.sh before this runs. The Windows images are built here and ctr
# imported onto the Windows node.
#
# All refs land as docker.io/calico/<name>:test-build, which is what the
# operator requests (registry docker.io, path calico, tag test-build); the
# Installation uses imagePullPolicy: IfNotPresent so kubelet uses the imported
# images without a registry pull.
