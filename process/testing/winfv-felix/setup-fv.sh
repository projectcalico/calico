...
function upload_calico_images(){
  ...
  #Import calico-node-windows image from locally built image
  ${ASO_DIR}/scp-to-windows.sh 0 'ctr --namespace k8s.io images import --base-name calico/node-windows c:\calico-node-windows.tar --all-platforms'
  ${ASO_DIR}/scp-to-windows.sh 0 'ctr --namespace k8s.io images import --base-name calico/cni-windows c:\calico-cni-plugin-windows.tar --all-platforms'
