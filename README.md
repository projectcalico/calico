...
The `calico-master` container needs to run in one place in your cluster.  It keeps track of all workloads and endpoints and distributes information to `calico-node` containers that run on each Docker host you'll use with Calico.  If you have read the Calico architecture, the `calico-master` service instantiates both the
