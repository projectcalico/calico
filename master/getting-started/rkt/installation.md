...
It is recommended to use systemd to run the `calico/node` container in production.  Use the following [sample `systemd` unit file]({{site.baseurl}}/getting-started/rkt/systemd/calico-node.service) to manage the `calico/node` container using rkt.
