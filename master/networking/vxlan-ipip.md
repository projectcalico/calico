...
If you use only VXLAN pools, BGP networking is not required. You can disable BGP to reduce the moving parts in your cluster by [Customizing the manifests]({{site.baseurl}}/{{page.version}}/getting-started/kubernetes/installation/customizing-the-manifests). Set the `calico_backend` setting to `vxlan`, and disable the BGP readiness check.
