...
Traffic cannot be routed between pods and destinations that aren’t also in the {{site.prodname}} cluster without [Allow workload access outside cluster]({{site.baseurl}}/{{page.version}}/network/), or [peering with the infrastructure](#unencapsulated-peered-with-physical-infrastructure).
