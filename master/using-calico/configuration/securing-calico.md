...
Calico's host endpoint support (see [this document]({{site.url}}/{{page.version}}/getting-started/bare-metal/bare-metal)) can be used to
enforce such policy.
...
-   Unless your Calico system uses `calicoctl node` to install and
    ...
    >  Cluster]({{site.url}}/{{page.version}}/reference/advanced/etcd-secure)
...
parameters; see [here]({{site.url}}/{{page.version}}/using-calico/configuration).
