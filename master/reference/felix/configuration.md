...
| IptablesPostWriteCheckIntervalSecs      | FELIX_IPTABLESPOSTWRITECHECKINTERVALSECS  | 30                                 | Period, in seconds, after Felix has done a write to the dataplane that it schedules an extra read back in order to check the write was not clobbered by another process.  This should only occur if another application on the system doesn't respect the iptables lock.  |
...
| IptablesLockTimeoutSecs                 | FELIX_IPTABLESLOCKTIMEOUTSECS           | 30                                   | Time, in seconds, that Felix will wait for the iptables lock.  It is unlikely that this value would need to be adjusted unless other processes are regularly holding the lock for long periods.  |
