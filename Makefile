...
kubeval:
	...
	docker run -v $$PWD:/calico --entrypoint /bin/sh -ti garethr/kubeval:0.1.1 -c 'for f in `find /calico/_site/master -name "*.yaml" |grep -v "\(config\|allow-istio-pilot\).yaml"`; do echo Running kubeval on $$f; /kubeval $$f; done'
