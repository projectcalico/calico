...
###############################################################################
E2E_PROCS ?= 4
E2E_TEST_CONFIG ?= e2e/config/kind-felix-routing.yaml
...
e2e-run:
	...
	KUBECONFIG=$(KUBECONFIG) go run github.com/onsi/ginkgo/v2/ginkgo --dry-run -procs=$(E2E_PROCS) --junit-report=$(E2E_JUNIT_REPORT) --output-dir=$(E2E_OUTPUT_DIR)/ ./e2e/bin/k8s/e2e.test -- --ginkgo.dry-run --calico.test-config=$(abspath $(E2E_TEST_CONFIG))
