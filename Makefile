...
kubeval:
	...
	docker run -v $$PWD:/calico --entrypoint /bin/sh -ti garethr/kubeval:0.1.1 -c 'ok=true; for f in `find /calico/_site/master -name "*.yaml" |grep -v "\(config\|allow-istio-pilot\).yaml"`; do echo Running kubeval on $$f; /kubeval $$f || ok=false; done; $$ok'
