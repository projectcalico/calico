...
gen-local-deps-files:
	$(MAKE) -j $(LOCAL_DEP_FILES)

$(LOCAL_DEP_FILES): go.mod go.sum Makefile hack/cmd/deps/*
	@{ \
	  echo "# Generated by hack/cmd/deps - DO NOT EDIT" && \
	  echo "# Run 'make gen-local-deps-files' to regenerate." && \
	  $(DOCKER_GO_BUILD) sh -c "go run ./hack/cmd/deps local-dirs-main-only $(patsubst %/,%,$(dir $@))" | sed 's|^/||'; \
	} > $@

check-local-deps: gen-local-deps-files
	@if ! git diff --quiet -- $(LOCAL_DEP_FILES); then \
	  echo "ERROR: local-deps.txt files are out of date. Run 'make gen-local-deps-files' and commit." && \
	  git diff --stat -- $(LOCAL_DEP_FILES) && \
	  exit 1; \
	fi
