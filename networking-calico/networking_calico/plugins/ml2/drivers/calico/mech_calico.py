...
from networking_calico import resync
...
class CalicoMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def _post_fork_init(self, voting=False):
        ...
        with self._init_lock:
            ...
            # Build the endpoint writer used by the port and SG postcommit hooks.
            self.endpoint_writer = EndpointWriter(self.db)
        ...
        try:
            ...
            if calico_status == datamodel_v1.ENDPOINT_STATUS_UP:
                ...
                if migrating_to == hostname:
                    ...
                    namespace = self.endpoint_writer.namespace
    ...
    def update_existing_ports(self, ports, plugin_context, reason):
        ...
        for p in ports:
            if _port_is_endpoint_port(p):
                self.endpoint_writer.write_endpoint(p, plugin_context, must_update=True)
    ...
    def create_port_postcommit(self, context):
        ...
        with self._txn_from_context(plugin_context, tag="create-port"):
            self.endpoint_writer.write_endpoint(port, plugin_context)
    ...
    def update_port_postcommit(self, context):
        ...
        with self._txn_from_context(plugin_context, tag="update-port"):
            ...
            if orig_migrating_to is not None and curr_migrating_to is None:
                ...
                self.endpoint_writer.delete_live_migration(dest_wep_name)
                ...
                if port["binding:host_id"] == original["binding:host_id"]:
                    ...
                    self.endpoint_writer.delete_endpoint(dest_port)
                else:
                    LOG.info(
            ...
            if original["binding:host_id"] != port["binding:host_id"]:
                ...
                self.endpoint_writer.delete_endpoint(original)
            ...
            if port.get("binding:profile", {}).get("migrating_to") is not None:
                ...
                migration_uid = self.endpoint_writer.write_live_migration(
                    port, dest_port
                )
                ...
                self.endpoint_writer.write_endpoint(
    ...
    def delete_port_postcommit(self, context):
        ...
        # Delete source WEP.
        self.endpoint_writer.delete_endpoint(port)
    ...
    def security_groups_rule_updated(self, context):
        ...
        with self._txn_from_context(context.plugin_context, tag="sg-update"):
            policy.write_sgs_to_etcd(context.sgids, self.db, context.plugin_context)
    ...
    def _update_port(self, plugin_context, port):
        ...
        if port_bound(port):
            ...
            self.endpoint_writer.write_endpoint(port, plugin_context, must_update=True)
        else:
    ...
    def _run_startup_resync(self):
        ...
        We pass the driver's own endpoint writer into the runner rather than letting it
        build a fresh one, so the same Keystone client and project-data cache are
        reused.  Subnet and policy resync state is owned by the resync package; the
        runner builds fresh SubnetSyncer and PolicySyncer instances each time.
        ...
        result = resync.Scope(
            self.db,
            endpoint_writer=self.endpoint_writer,
        ).run()
        if result.ok:
