...
class CalicoMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def _post_fork_init(self, voting=False):
        ...
        with self._init_lock:
            ...
            if voting:
                ...
                # Start our long-running threads.  Just in case we ever get two same
                # threads running, use an epoch counter to tell the old thread to die.
                # We deliberately do this last, to ensure that all of the setup above is
                # complete before we start running.
                self._epoch += 1
    ...
    def update_network_postcommit(self, context):
        ...
        # qos_policy_id.  We do NOT wrap this in a writer/reader context: each
        # upstream call (``get_ports``, ``get_security_group_rules`` via the
        # endpoint syncer, etc.) is already decorated with
        # ``@db_api.retry_if_session_inactive`` and manages its own transaction
        # plus retry.  Holding an outer writer here disables that retry --
        # documented as an anti-pattern in Neutron's contributor devref:
        # "the retry context would be always called from inside an active
        # transaction making it useless."  See also CORE-12037 / PR #12898.
    ...
    def create_subnet_postcommit(self, context):
        ...
        # Re-read the subnet from the DB so we pick up the latest state,
        # rather than the (potentially slightly stale) ``context.current``
        # snapshot taken in the precommit phase.  ``self.db.get_subnet`` is
        # ``@retry_if_session_inactive``-decorated and manages its own
        # reader transaction; we deliberately do NOT wrap it in our own
        # writer/reader -- see update_network_postcommit for rationale.
    ...
    def update_subnet_postcommit(self, context):
        ...
        # Re-read the subnet (see create_subnet_postcommit for the rationale
        # behind the re-read and against wrapping in a writer context).
        subnet = context.current
        plugin_context = context._plugin_context
        subnet = self.db.get_subnet(plugin_context, subnet["id"])
        if subnet["enable_dhcp"]:
    ...
    def create_port_postcommit(self, context):
        ...
        Process this event by writing the corresponding WorkloadEndpoint
        (and any side data such as security-group policies) to etcd.  We
        deliberately do not wrap this in a writer/reader context: the inner
        calls into ``self.db.get_*`` are already
        ``@db_api.retry_if_session_inactive``-decorated and manage their
        own transactions plus retry behaviour.  Holding an outer writer
        here disables that retry -- see ``update_network_postcommit`` for
        the devref reference and CORE-12037 / PR #12898 for the regression
        history this avoids.
        """
    ...
    def update_port_postcommit(self, context):
        ...
        # Re-read the port to pick up the latest available data rather than
        # relying on ``context._port`` which is a snapshot taken earlier in
        # the API call.  ``self.db.get_port`` is
        # ``@db_api.retry_if_session_inactive``-decorated and manages its own
        # reader transaction; we deliberately do NOT wrap this body in our
        # own writer/reader context -- see ``create_port_postcommit`` for
        # rationale and CORE-12037 / PR #12898 for the regression history.
        #
        # The re-read is a best-effort hedge against two fast-paired updates
        # to the same port being routed to different API workers and arriving
        # at postcommit in the opposite order to the API call order.  If the
        # second-in-time update has already committed to the Neutron DB by
        # the time we get here, this re-read picks up both changes and we
        # write a consistent superset to etcd.  If the other order obtains
        # (we re-read before the other worker's DB commit, then race on the
        # etcd write), the etcd state can transiently revert to the older
        # update's view.  A writer transaction here would not help: a
        # Neutron writer txn in our session does not row-lock the port and
        # does not order against other workers' sessions or etcd writes,
        # and the etcd write below is not CAS-guarded (``mod_revision`` is
        # ``None`` in ``endpoints.write_endpoint`` for the dynamic path).
        # Persistent drift, if it happens, is repaired on the next
        # neutron-server restart by the startup resync; there is no longer
        # a periodic resync.  Tightening this -- e.g. CAS-against-mod_revision
        # on dynamic writes with retry-on-conflict -- is a known follow-up.
        plugin_context = context._plugin_context
    ...
    def update_floatingip(self, plugin_context):
        ...
        # No outer writer/reader context here -- see create_port_postcommit
        # for rationale.
    ...
    def security_groups_rule_updated(self, context):
        ...
        # No outer writer/reader context here -- see create_port_postcommit
        # for rationale.  ``write_sgs_to_etcd`` calls
        # ``self.db.get_security_group_rules``, which is
        # ``@retry_if_session_inactive``-decorated and triggers upstream
        # ``_ensure_default_security_group``'s race recovery when called
        # without an outer writer.
