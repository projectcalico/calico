...
class TestPluginEtcdBase(_TestEtcdBase):
    ...
    def _trigger_resync(self):
        ...
        Wraps the same ``run_resync`` entry point used by the driver's start-of-day path
        and the ``calico-resync`` CLI, but reuses the syncers already wired on
        ``self.driver`` so the mocked DB and Keystone come along for free.  Used by
        tests that previously triggered a periodic resync by advancing simulated time.

        """
        return resync_scope.Scope(
            self.driver.db,
            admin_context=mech_calico.ctx.get_admin_context(),
            endpoint_writer=self.driver.endpoint_writer,
        ).run()

    def _trigger_narrow_resync(self, **scope_kwargs):
        """Drive a narrow resync via the resync runner.
        ...
        If the driver has been initialised (post _post_fork_init) we reuse
        its EndpointWriter so the same primed project cache is in play.
        Otherwise we let the runner build a fresh one against the mocked
        DB and Keystone.
        """
        return resync_scope.Scope(
            self.db,
            admin_context=mech_calico.ctx.get_admin_context(),
            endpoint_writer=getattr(self.driver, "endpoint_writer", None),
            **scope_kwargs,
        ).run()
...
class TestNarrowResync(TestPluginEtcdBase):
    ...
    def test_port_narrow_resync_no_op_when_correct(self):
        ...
        result = self._trigger_narrow_resync(ports=[lib.port1["id"]])
    ...
    def test_subnet_narrow_resync_writes_only_when_changed(self):
        ...
        result = self._trigger_narrow_resync(subnets=[subnet["id"]])
        self.assertTrue(result.ok)
    ...
    def test_subnet_narrow_resync_deletes_when_gone(self):
        ...
        self._trigger_narrow_resync(subnets=[subnet["id"]])
        self.recent_writes = {}
    ...
    def test_security_group_narrow_resync_no_op_when_correct(self):
        ...
        self._do_initial_all_resync()
        result = self._trigger_narrow_resync(security_groups=["SGID-default"])
        self.assertTrue(result.ok)
...
class TestLiveMigration(TestPluginEtcdBase):
    ...
    def test_narrow_resync_creates_missing_live_migration(self):
        ...
        result = self._trigger_narrow_resync(ports=[self.port["id"]])
        self.assertTrue(result.ok)
    ...
    def test_narrow_resync_cleans_stale_lm_when_flagged(self):
        ...
        result = self._trigger_narrow_resync(
