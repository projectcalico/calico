...
class TestLiveMigration(TestPluginEtcdBase):
    ...
    def test_narrow_resync_skips_write_when_reread_matches_etcd(self):
        """The recheck after reread suppresses spurious writes when the
        cached Neutron read was stale but a concurrent updater had already
        brought etcd into sync with the fresh Neutron state.  Without the
        recheck we'd CAS-write fresh data on top of identical fresh data
        in etcd, wasting an etcd revision and over-counting ``updated``.
        """
        self._do_initial_resync()
        # Stage a stale variant of the port: same id (so the etcd key
        # matches) but a different mac_address (so endpoint_spec differs).
        stale_port = dict(self.port)
        stale_port["mac_address"] = "00:00:00:00:00:de"
        self.recent_writes = {}
        self.recent_deletes = set()

        # Mock get_ports (used by get_from_neutron) to return the stale
        # variant.  Leave db.get_port alone so the reread returns the real
        # osdb_ports entry, whose spec matches what's already in etcd.
        with mock.patch.object(
            self.db,
            "get_ports",
            side_effect=lambda ctx, filters=None: [stale_port],
        ):
            result = self._trigger_resync(ports=[self.port["id"]])

        self.assertTrue(result.ok)
        wep_key = self._ep_key(self.port["binding:host_id"])
        # Reread matched etcd, so no write should have happened, and the
        # counters should reflect "correct" rather than "updated".
        self.assertNotIn(wep_key, self.recent_writes)
        self.assertEqual(result.phases["endpoints"]["updated"], 0)
        self.assertEqual(result.phases["endpoints"]["correct"], 1)

    def test_narrow_resync_skips_when_reread_finds_port_gone(self):
        """The compare/update path rereads Neutron before writing so a
        concurrent dynamic update doesn't get clobbered.  If the port has
        been deleted between the cached read and the reread, the resync
        skips the entry — it doesn't write stale data and doesn't try to
        delete here either (the next resync sees the port as absent from
        Neutron and deletes the etcd entry on the in-etcd-only branch).
        """
        self._do_initial_resync()
        # Force a mismatch on the WEP so the compare path goes to the
        # reread-and-update branch (rather than no-op'ing as correct).
        wep_key = self._ep_key(self.port["binding:host_id"])
        self.etcd_data[wep_key] = json.dumps(
            {
                "apiVersion": "projectcalico.org/v3",
                "kind": "WorkloadEndpoint",
                "metadata": {
                    "name": wep_key.rsplit("/", 1)[-1],
                    "namespace": self.namespace,
                },
                "spec": {"interfaceName": "different"},
            }
        )
        self.recent_writes = {}
        self.recent_deletes = set()

        # Make db.get_port raise PortNotFound — simulating a concurrent
        # delete after get_from_neutron's bulk query.  get_ports still
        # returns the port so we enter the in_both branch as expected.
        with mock.patch.object(
            self.db,
            "get_port",
            side_effect=mech_calico.n_exc.PortNotFound(port_id=self.port["id"]),
        ):
            result = self._trigger_resync(ports=[self.port["id"]])

        self.assertTrue(result.ok)
        # The stale etcd entry is neither overwritten nor deleted.
        self.assertNotIn(wep_key, self.recent_writes)
        self.assertNotIn(wep_key, self.recent_deletes)
