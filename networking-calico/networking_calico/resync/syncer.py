...
class ResourceSyncer(object):
    ...
    def resync(self, context, scope):
        ...
        ``scope_ids`` is None for an all-resources resync (the
        start-of-day path): we read every resource of this kind from
        both sides, and reconcile in both directions (writing
        missing/incorrect etcd resources and deleting etcd resources
        whose Neutron counterpart is gone).

        ``scope_ids`` may also be a set of Neutron resource IDs, in
        which case the reconcile is restricted to just those
        resources: we read only their Neutron data (in one filtered
        query) and only their etcd entries (one read per name).  The
        decision tree per resource is the same:
        ...
        For the narrow case the "delete" branch is only reached for
        names whose etcd-name is a deterministic function of the
        resource ID (subnets and security groups); see
        :meth:`names_for_missing_ids`.

        Returns a structured summary dict of what happened: per-phase
        timings (etcd_read, neutron_read, compare, create) plus item
        counts (etcd_items, neutron_items, correct, updated, deleted,
        created).  The runner exposes these in the JSON ResyncResult;
        the same numbers are also logged in a single summary line on
        completion.
        """
        ...
        for name in set(etcd_map) | set(neutron_map):
            ...
            if in_neutron and in_etcd:
                # Compare and update if different.  reread=False
                # because we just read this name from Neutron.
