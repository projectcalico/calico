...
from networking_calico.plugins.ml2.drivers.calico.endpoints import EndpointWriter
from networking_calico.resync.endpoint_syncer import EndpointSyncer
from networking_calico.resync.policy_syncer import PolicySyncer
from networking_calico.resync.subnet_syncer import SubnetSyncer
...
class Scope:
    ...
    A Scope with ``is_all=True`` requests a resync of every Calico resource that this
    driver writes to etcd.  Otherwise the resync is limited to the listed IDs, after
    expansion (see ``ExpandedScope``):

      * each network in ``network_ids`` expands to that network plus every subnet and
        port within it;
      * each subnet in ``subnet_ids`` expands to that subnet plus every port on it;
    ...
    def __init__(
        ...
        self.endpoint_writer = endpoint_writer
    ...
    def run(self) -> ResyncResult:
        ...
        try:
            ...
            if self.endpoint_writer is None:
            ...
            endpoint_syncer = EndpointSyncer(self.endpoint_writer, _txn_from_context)
