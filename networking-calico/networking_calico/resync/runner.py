    ...
    try:
        ...
        if scope.is_all:
            phases["subnets"] = _run_phase(
                lambda: subnet_syncer.resync(admin_context)
            )
            phases["policy"] = _run_phase(
                lambda: policy_syncer.resync(admin_context)
            )
...
def _resync_subnets(
    ...
    try:
        subnets_by_id = {
            s["id"]: s
            for s in db_plugin.get_subnets(
...
def _resync_ports(
    ...
    try:
        ports_by_id = {
            p["id"]: p
            for p in db_plugin.get_ports(
                admin_context, filters={"id": port_ids}
            )
        }
        for pid in port_ids:
...
def _resync_security_groups(
    sg_ids, admin_context, policy_syncer
) -> Dict[str, Any]:
    t0 = time.monotonic()
    error = None
    writes = 0
...
def provide_felix_config():
    ...
    while rewrite_felix_config:
        ...
        if not felix_config.get(
            datamodel_v3.ENDPOINT_REPORTING_ENABLED, False
        ):
