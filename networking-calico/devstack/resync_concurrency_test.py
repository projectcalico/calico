...
def time_port_create(conn, network, subnet, sg, host, label):
    ...
    """
    spec = {
        "name": "concurrency-%s-%s" % (label, uuid.uuid4().hex[:8]),
...
def scenario_on_demand(
    ...
    try:
        ...
        # phase.
        in_resync = measure_port_create_burst(
            conn, network, subnet, sg, host, test_ports, "in_resync"
        )
        in_resync_median = median(in_resync)
...
def wait_for_log_text_in_journal(text, since_iso, deadline_secs):
...
def scenario_startup(
    ...
    try:
        t_restart_iso = datetime.datetime.utcnow().isoformat() + "Z"
        ...
        if not wait_for_log_text_in_journal(
            _ENDPOINTS_PHASE_MARKER, t_restart_iso, deadline_secs=60
        ):
            raise RuntimeError(
        ...
        if not wait_for_log_text_in_journal(
            "One-shot resync done", t_restart_iso, deadline_secs=budget_secs
        ):
...
def print_summary(result, ratio_limit):
    ...
    for m in result["in_resync"]:
        LOG.info(
            "  in_resync port %d at %s, API took %.3fs",
            m["iter"],
            m["started_at"],
            m["elapsed_secs"],
        )
